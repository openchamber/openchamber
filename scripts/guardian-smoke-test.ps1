...
if (-not $DataDir) {
    ...
    # Anchor the data dir under `<workspace>/.smoke-logs/` rather
    # than `[System.IO.Path]::GetTempPath()`. The runner's temp
    # dir is ephemeral and GitHub Actions does not preserve it
    # past the job, so the redirect files (`$LogFile` /
    # `$LogErrFile`) would be invisible to the artifact upload
    # step and to `gh run view --log-failed`. Writing inside the
