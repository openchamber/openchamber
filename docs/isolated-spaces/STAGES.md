...
| 1b | Tools volume and the server inside the space | The tools volume with the server, OpenCode, and its plugin, for a released version and for a development build from packed local packages. No UI | The contract and escape suites still pass with the server running inside. The server starts from the read-only tools volume. Confirm that OpenCode inside reports the space path unchanged |
