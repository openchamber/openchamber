...
This directory contains the non-entrypoint implementation for the OpenChamber CLI. `backend/bin/cli.js` should stay thin: it owns bootstrap, command wiring, top-level dispatch, signal/cancel handling, and compatibility exports. Domain logic belongs in these modules.
...
For CLI behavior changes, run the focused CLI suite from `client/web`:
