...
async function main(options = {}) {
  ...
  // Add configured client URL for deployed frontend (e.g., https://your-app.vercel.app)
  const clientUrl = typeof process.env.OPENCHAMBER_CLIENT_URL === 'string' ? process.env.OPENCHAMBER_CLIENT_URL.trim() : '';
  if (clientUrl) {
    packagedClientOrigins.add(clientUrl);
  }
  const isLocalDevClientOrigin = (origin) => /^https?:\/\/(localhost|127\.0\.0\.1):\d+$/.test(origin);
  // Also allow any origin that matches the client URL (handles subdomains, etc.)
  const isConfiguredClientOrigin = (origin) => clientUrl ? origin === clientUrl || origin.startsWith(clientUrl) : false;
  ...
  app.use((req, res, next) => {
    ...
    if (packagedClientOrigins.has(origin) || isLocalDevClientOrigin(origin) || isConfiguredClientOrigin(origin)) {
