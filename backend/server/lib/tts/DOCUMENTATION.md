...
## Purpose
This module provides server-side Text-to-Speech services using OpenAI's TTS API. The historical shared text summarization endpoint now lives in `backend/server/lib/text/` as an API-compatible stub because the previous Zen model provider is unavailable.

## Entrypoints and structure
- `backend/server/lib/tts/index.js`: Public entrypoint imported by `backend/server/index.js`.
- `backend/server/lib/tts/routes.js`: Express route registration for `/api/voice/*`, `/api/tts/*`, and `/api/stt/*` endpoints.
- `backend/server/lib/tts/capability-runtime.js`: runtime helper for probing local macOS `say` TTS voice capability.
- `backend/server/lib/tts/service.js`: TTS service implementation with OpenAI integration.
- `backend/server/lib/text/summarization.js`: Shared text summarization stub and sanitization utilities. It performs no external Zen calls.
- `backend/server/lib/tts/stt.js`: STT proxy for OpenAI-compatible transcription endpoints.
- `backend/server/lib/tts/base-url.js`: shared base URL validation and normalization for custom OpenAI-compatible endpoints.
...
## Usage in web server
The TTS module is used by `backend/server/index.js` for:
...
1. Add new methods to `backend/server/lib/tts/service.js` TTSService class.
2. Export public functions from `backend/server/lib/tts/index.js`.
