export const parseServeCliOptions = ({
  ...
  // Hosting platforms (Render, Heroku, etc.) inject the listen port via PORT.
  // OPENCHAMBER_PORT takes precedence for explicit local overrides. An explicit
  // --port CLI flag still wins over both (handled in the arg loop below).
  const resolveEnvPort = () => {
    const candidates = [env.OPENCHAMBER_PORT, env.PORT];
