...
- `backend/server/lib/github/index.js`: public server entrypoint.
- `backend/server/lib/github/routes.js`: Express route registration for `/api/github/*` endpoints.
- `backend/server/lib/github/auth.js`: auth storage, multi-account support, client id, scope config.
- `backend/server/lib/github/device-flow.js`: OAuth device flow.
- `backend/server/lib/github/octokit.js`: Octokit factory for the current auth.
- `backend/server/lib/github/repo/index.js`: remote URL parsing and directory-to-repo resolution.
- `backend/server/lib/github/pr-status.js`: PR lookup across remotes, forks, and upstreams.
- `backend/server/index.js`: API route layer that calls this module.
- `client/web/src/api/github.ts`: web client wrapper for GitHub endpoints.
...
- The UI asks `github.prStatus(directory, branch, remote?)` from `client/web/src/api/github.ts`.
- That hits `GET /api/github/pr/status` in `backend/server/index.js`.
- The route calls `resolveGitHubPrStatus(...)` in `backend/server/lib/github/pr-status.js`.
...
- `client/ui/src/components/session/SessionSidebar.tsx` reads all PR entries and maps them to `directory::branch`.
- `client/ui/src/components/session/sidebar/SessionGroupSection.tsx` renders the compact badge, PR number, title, checks summary, and GitHub link.
- `client/ui/src/components/views/git/PullRequestSection.tsx` uses the same shared entry for the full PR workflow.
- `client/ui/src/components/ui/MemoryDebugPanel.tsx` reads request counters for debugging.
