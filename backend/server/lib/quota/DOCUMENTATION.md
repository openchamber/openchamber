...
- `backend/server/lib/quota/index.js`: public entrypoint imported by `backend/server/index.js`.
- `backend/server/lib/quota/routes.js`: Express route registration for quota endpoints.
- `backend/server/lib/quota/providers/index.js`: provider registry, configured-provider list, and provider dispatcher.
- `backend/server/lib/quota/providers/google/`: Google-specific auth, API, and transform modules.
- `backend/server/lib/quota/utils/`: shared auth, transform, and formatting helpers.

## Supported provider IDs (dispatcher)

These provider IDs are currently dispatchable via `fetchQuotaForProvider(providerId)` in `backend/server/lib/quota/providers/index.js`.
...
1. Choose module shape based on complexity:
   - Simple providers: create `backend/server/lib/quota/providers/<provider>.js`.
   - Complex providers (multi-source auth, multiple API calls, non-trivial transforms): create `backend/server/lib/quota/providers/<provider>/` with split modules like Google (`index.js`, `auth.js`, `api.js`, `transforms.js`).
2. Export `providerId`, `providerName`, `aliases`, `isConfigured`, and `fetchQuota`.
3. Use shared helpers from `backend/server/lib/quota/utils/index.js` (`buildResult`, `toUsageWindow`, auth/conversion helpers) to keep payload shape consistent.
4. Register the provider in `backend/server/lib/quota/providers/index.js`.
5. If needed for direct use, export a named fetcher from `backend/server/lib/quota/providers/index.js` and `backend/server/lib/quota/index.js`.
