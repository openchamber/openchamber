...
- `backend/server/lib/daytona/config.js`: Environment variable configuration resolution.
- `backend/server/lib/daytona/sandbox-registry.js`: In-memory registry tracking active sandboxes per session.
- `backend/server/lib/daytona/lifecycle.js`: Sandbox creation and destruction via the @daytona/sdk.
- `backend/server/lib/daytona/inactivity-monitor.js`: Periodic check for inactive sandboxes with auto-destroy.
- `backend/server/lib/daytona/proxy.js`: SSE, HTTP, and WebSocket proxy to OpenCode in sandboxes.
- `backend/server/lib/daytona/routes.js`: REST endpoints for sandbox lifecycle management.
- `backend/server/lib/daytona/service.js`: Composition entrypoint that wires all components together.
...
- This module follows the `createXxxService()` factory pattern used by `backend/server/lib/relay/service.js`.
