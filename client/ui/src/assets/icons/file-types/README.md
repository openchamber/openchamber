...
- The UI resolves an icon id in `client/ui/src/lib/fileTypeIcons.ts`.
- Valid icon ids are loaded from `client/ui/src/lib/fileTypeIconIds.ts`.
- `client/ui/src/components/icons/FileTypeIcon.tsx` renders the icon with `<use href="...#icon-id" />` from `sprite.svg`.
...
The command regenerates both `sprite.svg` and `client/ui/src/lib/fileTypeIconIds.ts`.
