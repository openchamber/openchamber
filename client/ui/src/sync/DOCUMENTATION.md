...
This document covers the current client-side session/data architecture in `client/ui/src/sync` and the rules for updating stores safely.
...
2. **Global sessions cache**
   - Owned by `client/ui/src/stores/useGlobalSessionsStore.ts`
