...
Electron desktop runtime for Sharpie on macOS, Windows, and Linux.

This package owns the native shell: windows, menus, deep links, native notifications, auto-updates, host switching, SSH connections, tunnel helpers, and packaged desktop builds. The web UI and Sharpie server logic still live in `packages/web` and shared React UI lives in `packages/ui`.

## How It Runs

Desktop starts the Sharpie web server in the same Electron main process. There is no separate sidecar subprocess for the Sharpie server.
...
| `OPENCHAMBER_SKIP_LOCAL_SERVER=1` | Skips the in-process local Sharpie server and uses the configured default remote instance; Desktop imports this from the user's login-shell environment, and packaged/bundled UI remains available for connection recovery |
...
Electron uses `electron-log`. In development, console logs are also visible in the terminal. In packaged apps, logs are written through the platform log path for the `Sharpie` app name.

Development builds use a separate user data directory named `Sharpie Dev`, so dev state does not overwrite normal packaged app state.
