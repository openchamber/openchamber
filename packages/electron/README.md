...
- Managed-mode SSH servers persist their started remote port under the `desktopSshManagedServers` settings key, separate from `desktopSshInstances` because renderer saves rewrite that array from known fields. Connects probe the instance's preferred port, then the persisted port, and only start a new server when neither answers. A server found on the persisted port is adopted (stopped on disconnect when keep-running is off) and restarted when its reported `openchamberVersion` differs from the app or when its persisted config tag (a truncated hash of the UI password and bind host) no longer matches the instance config, so config edits take effect on the next connect; entries predating config tags are adopted as-is and backfilled. A server on the user-pinned preferred port is reused untouched at any version because it may be externally managed.
