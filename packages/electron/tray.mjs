...
//
// Surfaces a glanceable, always-visible view of Sharpie's live state:
...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'Sharpie — no active sessions';
  const bits = [];
  ...
  return `Sharpie — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    if (isLinux || process.platform === 'win32') {
      ...
      template.push(
        { label: 'Show Sharpie', click: () => onAction({ type: 'show-main-window' }) },
        { type: 'separator' },
        { label: 'Quit Sharpie', click: () => onAction({ type: 'quit' }) },
      );
    }
