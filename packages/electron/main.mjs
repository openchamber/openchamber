...
const PACKAGED_APP_USER_MODEL_ID = 'dev.sharpie.desktop';
const DEV_APP_USER_MODEL_ID = 'dev.sharpie.desktop.dev';
...
// ~/Library/Logs/Sharpie/ (not ~/Library/Logs/@openchamber/electron/).
app.setName('Sharpie');
if (process.platform === 'linux') {
  app.setDesktopName('openchamber.desktop');
}
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'Sharpie Dev'));
}
...
// that output never lands in ~/Library/Logs/Sharpie/main.log and we
...
const quitConfirmationMessage = () => {
  ...
  return `Sharpie detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
// session expires) reuses the same "Sharpie Desktop" record instead of
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Sharpie loading icon">
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'Sharpie',
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to Sharpie server?',
      ...
      if (applyUpdate && process.platform === 'darwin' && typeof app.isInApplicationsFolder === 'function') {
        try {
          if (!app.isInApplicationsFolder()) {
            throw new Error('Desktop update requires Sharpie.app to be installed in /Applications');
          }
        } catch (error) {
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: 'About Sharpie', click: () => dispatchAction('about') },
        {
