...
const waitLocalForwardReady = async (localPort) => {
  ...
  throw new Error('Timed out waiting for forwarded Sharpie health');
};
...
export class ElectronSshManager {
  ...
  async issueClientToken(localUrl, openchamberPassword) {
    ...
    const loginResponse = await fetch(new URL('/auth/session', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({
        ...
        clientLabel: 'Sharpie Desktop SSH',
      }),
    });
    if (!loginResponse.ok) {
      throw new Error(`Configured Sharpie UI password was rejected by forwarded server (status ${loginResponse.status})`);
    }
    ...
    const tokenResponse = await fetch(new URL('/api/client-auth/clients', `${localUrl}/`).toString(), {
      ...
      body: JSON.stringify({ label: 'Sharpie Desktop SSH' }),
    });
  ...
  async installOpenChamberManaged(parsed, controlPath, version, preferred) {
    ...
    throw lastError || new Error('Failed to install Sharpie on remote host');
  }
    ...
    if (isLivenessHttpStatus(infoStatus)) {
      if (isAuthHttpStatus(infoStatus)) {
        if (openchamberPassword && authStatus !== 200) {
          throw new Error(`Remote Sharpie requires UI authentication and configured password was rejected (auth status ${authStatus})`);
        }
        if (isLivenessHttpStatus(healthStatus)) return {};
        throw new Error('Remote Sharpie requires UI authentication on /api/system/info; configure Sharpie UI password');
      }
    } else if (isLivenessHttpStatus(healthStatus)) {
      return {};
    } else {
      throw new Error(`Remote Sharpie probe failed (info status ${infoStatus}, health status ${healthStatus})`);
    }
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    if (instance.remoteOpenchamber.mode === 'external') {
      if (!instance.remoteOpenchamber.preferredPort) {
        throw new Error('External mode requires a preferred remote Sharpie port');
      }
      const port = instance.remoteOpenchamber.preferredPort;
      this.setStatus(instance.id, 'server_detecting', 'Probing external Sharpie server', null, null, port, false, 0, false);
    ...
    this.setStatus(instance.id, 'remote_probe', 'Checking remote Sharpie installation');
    const installedVersion = await this.currentRemoteOpenChamberVersion(parsed, controlPath);
    if (!installedVersion) {
      this.setStatus(instance.id, 'installing', 'Installing Sharpie on remote host');
      ...
      this.setStatus(instance.id, 'updating', `Updating remote Sharpie from ${installedVersion} to ${this.appVersion}`);
      await this.installOpenChamberManaged(parsed, controlPath, this.appVersion, instance.remoteOpenchamber.installMethod);
    }

    this.setStatus(instance.id, 'server_detecting', 'Detecting managed Sharpie server');
    ...
    if (!remotePort) {
      this.setStatus(instance.id, 'server_starting', 'Starting managed Sharpie server');
    ...
    if (!(await this.remoteServerRunning(parsed, controlPath, remotePort, this.configuredOpenChamberPassword(instance)))) {
      throw new Error('Managed Sharpie server failed to become reachable');
    }
