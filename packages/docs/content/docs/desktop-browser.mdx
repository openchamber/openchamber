...
Copy and paste use the computer's clipboard on macOS, Windows, and Linux. A page
must be focused before it can copy. Clipboard reads through JavaScript work only
for a focused page on the literal `localhost` hostname. Pages opened through a
remote development tunnel, external pages, and pages addressed as `127.0.0.1` or
`[::1]` cannot read the clipboard. Native paste still works after you focus a
field. The web and VS Code versions do not provide this browser or its clipboard
behavior.
