...
A scheduled task runs a prompt for you on a schedule — for example, a daily "summarize yesterday's changes" or a weekly cleanup. When it runs, Sharpie starts a new session and sends the prompt on its own. Open the scheduler from the button at the top of the session sidebar.
...
Tasks only fire while the Sharpie server is running. If you close it, scheduled runs are paused until it's back up.
