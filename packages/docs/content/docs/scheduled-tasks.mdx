...
Enable **Archive successful runs** under **Lifecycle** when completed run sessions should leave Recents automatically. Normal runs archive after their session becomes idle; goal runs wait until the goal completes. Failed and blocked runs stay visible for diagnosis, and archived runs remain available from the task link and Archived view.
...
If automatic archival fails, the run keeps its successful status and the task shows a separate archival warning.
