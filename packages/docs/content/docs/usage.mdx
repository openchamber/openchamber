...
Usage works for the providers that publish a quota, including Claude, Codex, Command Code, GitHub Copilot, Google, OpenRouter, Kimi, NanoGPT, z.ai, Zhipu, MiniMax, Ollama Cloud, and Wafer.

Command Code is a best-effort integration over unofficial endpoints and only appears when you connect it through OpenCode, set `COMMAND_CODE_API_KEY`, or run `cmd login`. It shows monthly, five-hour, and weekly usage as percentages. If those endpoints change, only the Command Code result reports an error.
