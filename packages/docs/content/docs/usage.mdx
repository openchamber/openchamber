...
Command Code is a best-effort integration over unofficial endpoints and only appears when you connect it through OpenCode, set `COMMAND_CODE_API_KEY`, or run `cmd login`. If those endpoints change, only the Command Code result reports an error.
