...
| `surface` | şeritte `panel`, iliştirme penceresinde `dialog` |
...
`startSession` geçerli projede bir oturum oluşturur. `worktree: true` ile OpenChamber önce bir git worktree oluşturur ve oturumu orada açar. `text` verilmişse ve bir model seçiliyse ilk mesaj olarak gönderilir. Sonuç `{ sessionId, sent }` olur; `sent` değeri `sent`, `no-model`, `skipped` veya `failed`. `no-model` ve `failed` sonrasında da oturum var olur. Açık proje olmaması, worktree'nin başarısız olması veya oturum oluşturmanın başarısız olması `HOST_REJECTED` hatasıdır.
