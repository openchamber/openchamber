---
title: 環境変数
description: 環境変数で Sharpie と OpenCode 連携を設定します。
---

# 環境変数

Sharpie は起動時にこれらの環境変数を読み取ります。スタートアップサービスでは、`openchamber startup enable` がデフォルトで現在の環境をスナップショットするため、サービスに使わせたい変数を変更した後は再実行してください。

## Sharpie サーバー

### `OPENCHAMBER_HOST`

Sharpie Web サーバーのバインドアドレスです。他のマシンからアクセスできるようにするには `0.0.0.0` を使います。
...
`true` または `1` に設定すると、Sharpie をヘッドレスモードで起動します。デスクトップおよびモバイルクライアント向けの API ルートは利用できますが、ブラウザ UI は配信されません。

### `OPENCHAMBER_DATA_DIR`

Sharpie のデータディレクトリを上書きします。デフォルトは `~/.config/openchamber` です。
...
Sharpie を既存の OpenCode サーバーに接続します。値は明示的なポートを含み、パス、クエリ、ハッシュを含まない `http` または `https` の origin である必要があります。`OPENCODE_HOST` は `OPENCODE_PORT` より優先されます。
...
`true` に設定すると、Sharpie が独自の OpenCode サーバーを起動しません。

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Sharpie が管理する OpenCode サーバーのバインドホスト名です。デフォルトは `127.0.0.1` です。

### `OPENCODE_BINARY`

Sharpie が実行する `opencode` 実行ファイルへのパスです。
...
WSL ディストリビューション選択用の Sharpie 固有エイリアスです。両方が設定されている場合は `OPENCODE_WSL_DISTRO` が優先されます。
...
Sharpie のターミナルセッションで使うシェル実行ファイルです。

### `OPENCHAMBER_GIT_BINARY`

Sharpie の Git 機能で使う Git 実行ファイルです。

### `GIT_BINARY`

代替の Git 実行ファイル上書きです。Sharpie 固有の設定には `OPENCHAMBER_GIT_BINARY` を推奨します。
...
OpenAI 互換サービスを呼び出す Sharpie の音声機能で使う API キーです。
...
Sharpie のトンネルコマンドで使う ngrok 認証トークンです。`ngrok config add-authtoken <token>` で ngrok 側に設定することもできます。

## ランタイムヘルパー

### `BUN_BINARY`

デーモンプロセスを起動するときに Sharpie が使う Bun 実行ファイルです。

### `BUN_INSTALL`

Bun のインストールルートです。Sharpie はデーモン起動と更新のために `bin/bun` を探す際に使います。
