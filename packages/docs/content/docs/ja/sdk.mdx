...
ページは `ctx.surface`(`panel` または `dialog`)でどちらの場所にいるか判断できます。以前に添付した項目をユーザーがクリックすると、同じ場所が再び開き、`ctx.item` にその項目(そこに保存した `data` も含む)がセットされるので、一覧の代わりにその詳細を表示できます。+ メニューやレールのアイコンから開いた場合、`ctx.item` は `null` です。
