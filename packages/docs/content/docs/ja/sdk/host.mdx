...
| `surface` | レール上なら `panel`、添付ウィンドウなら `dialog` |
...
`startSession` は現在のプロジェクトにセッションを作ります。`worktree: true` を付けると、OpenChamber はまず git worktree を作り、そこでセッションを開きます。`text` があってモデルが選択されていれば、最初のメッセージとして送られます。結果は `{ sessionId, sent }` で、`sent` は `sent`、`no-model`、`skipped`、`failed` のいずれかです。`no-model` と `failed` の後もセッションは存在します。開いているプロジェクトがない、worktree の作成に失敗した、セッションの作成に失敗した場合は `HOST_REJECTED` エラーになります。
