...
3. それ以外の場合は、自分でサーバーを起動して管理する

何も設定されていなければ、手順 3 が自動的に実行され、そのまま使えるようになります。
OpenChamber はデフォルトポートの無関係なサーバーに盲目的に接続しません。既存の guardian
子プロセスは、保存された所有者とプロセス ID がこの OpenChamber インスタンスと一致する場合だけ採用されます。
