---
title: OpenCode サーバー
description: Sharpie をローカルまたはリモートの OpenCode サーバーに接続します。
---

# OpenCode サーバー

Sharpie は OpenCode サーバーの上で動作します。デフォルトでは Sharpie がサーバーを起動するため、何もする必要はありません。このページが必要なのは、すでに実行しているサーバーに Sharpie を向けたい場合、または Sharpie が起動するサーバーを管理したい場合だけです。

## Sharpie がサーバーを見つける順序

Sharpie の起動時、次の順序でサーバーを探します。
...
- `OPENCODE_SKIP_START=true` — Sharpie に自分のサーバーを起動しないよう伝えます。

ポートだけを変えたい場合は、`OPENCODE_HOST` ではなく `OPENCODE_PORT` を設定します。

`OPENCODE_HOST` にポートがない、またはパスが含まれている場合、Sharpie はそれを無視し、自分のサーバー起動にフォールバックします。期待した接続が行われなかった場合は、起動ログの `[config]` 警告を確認してください。
...
## ログイン時に Sharpie を起動する

`startup enable` を使うと、ネイティブのユーザーサービスをインストールできます。Sharpie は macOS では `launchd`、Linux では `systemd --user`、Windows では Task Scheduler を使います。
...
別の Sharpie アプリ向けに接続リンクを作成するには、次を使います。
...
サーバーの起動中または再起動中、Sharpie は "OpenCode is restarting" 状態を表示し、準備が整うまでリクエストを一時停止します。これは起動直後や再起動直後には正常です。いつまでも消えない場合は、[OpenCode 接続](/troubleshooting/opencode-connection/) を参照してください。
