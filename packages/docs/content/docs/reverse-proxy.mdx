---
title: Reverse Proxy
description: Configure ALIAS ADE correctly behind Nginx, Nginx Proxy Manager, or another reverse proxy.
---

# Reverse Proxy

Use this page if you run ALIAS ADE behind Nginx, Nginx Proxy Manager, Caddy, Cloudflare, or another reverse proxy.

## Before you proxy it

1. Confirm ALIAS ADE works directly first.
...
- Disable gzip on the proxy if ALIAS ADE is already compressing responses.
...
- ALIAS ADE reachable directly on LAN
...
location ~ ^/api/(event|global/event|notifications/stream|alias-ade/events)$ {
    proxy_pass http://127.0.0.1:3000;
...
- ALIAS ADE compresses HTTP responses with gzip (threshold 1 KB).
...
SSE streaming routes are excluded from compression by ALIAS ADE, but the CDN may still buffer them. Check your CDN documentation for how to disable buffering on SSE paths.
