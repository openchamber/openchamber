...
Sharpie flags a worktree when something's off:

- **folder missing** — the worktree's folder was deleted or moved outside Sharpie; remove the session and create a fresh worktree
...
- on Windows, git uses Unix-style paths (like `/c/Users/...`) — Sharpie handles this, but custom SSH key paths should follow the same form
