...
description: Fix Sharpie not connecting to its OpenCode server.
---

# OpenCode Connection

If Sharpie loads but never gets past "OpenCode is restarting," or chat won't respond, the server it talks to isn't reachable. Work through these.
...
If you set Sharpie to use an existing server, double-check the setup from [OpenCode Server](/opencode-server/):

- `OPENCODE_HOST` must include the port, with no path — for example `http://localhost:4096`
- set `OPENCODE_SKIP_START=true` so Sharpie doesn't also start its own
- if the address is invalid, Sharpie ignores it and starts its own server instead — look for a `[config]` warning in the logs
...
- [OpenCode Server](/opencode-server/) — how Sharpie finds and manages the server
