...
| `surface` | `panel` dans la barre latérale, `dialog` dans la fenêtre d'attache |
...
`startSession` crée une session dans le projet en cours. Avec `worktree: true`, OpenChamber crée d'abord un worktree git et y ouvre la session. Quand `text` est défini et qu'un modèle est sélectionné, il est envoyé comme premier message. Le résultat est `{ sessionId, sent }`, où `sent` vaut `sent`, `no-model`, `skipped` ou `failed`. La session existe même après `no-model` et `failed`. Pas de projet ouvert, un worktree en échec ou une création de session en échec donnent une erreur `HOST_REJECTED`.
