...
description: Connectez Sharpie à un serveur OpenCode local ou distant.
---

# Serveur OpenCode

Sharpie fonctionne par-dessus un serveur OpenCode. Par défaut, il en démarre un pour vous, donc vous n’avez rien à faire. Vous n’avez besoin de cette page que si vous voulez pointer Sharpie vers un serveur que vous exécutez déjà, ou gérer celui qu’il démarre.

## Comment Sharpie trouve un serveur

Au démarrage, Sharpie cherche un serveur dans cet ordre :
...
Définissez ces variables avant de lancer Sharpie :
...
- `OPENCODE_SKIP_START=true` — indique à Sharpie de ne pas démarrer son propre serveur.

Si vous voulez seulement changer le port, définissez `OPENCODE_PORT` au lieu de `OPENCODE_HOST`.

Si `OPENCODE_HOST` n’a pas de port ou contient un chemin, Sharpie l’ignore et revient au démarrage de son propre serveur. Surveillez les logs de démarrage pour un avertissement `[config]` si une connexion attendue ne se fait pas.
...
## Lancer Sharpie à la connexion

Utilisez `startup enable` pour installer un service utilisateur natif. Sharpie utilise `launchd` sur macOS, `systemd --user` sur Linux et le Planificateur de tâches sur Windows.
...
Pour créer un lien de connexion pour une autre application Sharpie, utilisez :
...
Pendant que le serveur démarre ou redémarre, Sharpie affiche l’état « OpenCode redémarre » et met les requêtes en pause jusqu’à ce qu’il soit prêt. C’est normal juste après un lancement ou un redémarrage. Si l’état ne disparaît jamais, consultez [Connexion à OpenCode](/troubleshooting/opencode-connection/).
