---
title: Tunnels
description: Exposez Sharpie en sécurité pour l’accès distant et mobile.
---

# Tunnels

Un tunnel est un lien public vers votre Sharpie, pour y accéder depuis un navigateur ordinaire sur un autre réseau. Utilisez `openchamber tunnel` pour en créer un pour une instance en cours d’exécution.
...
Sharpie démarre la CLI du fournisseur de tunnel sur votre machine. Installez d’abord le fournisseur que vous voulez utiliser :
...
Quand le tunnel est actif, `status` affiche une URL publique. Ouvrez-la, ou scannez le QR code, pour joindre Sharpie depuis n’importe où.

Par défaut, Sharpie affiche un QR code dans les sessions TTY interactives. Utilisez `--qr` pour forcer la sortie QR, ou `--no-qr` pour la désactiver.
...
- un tunnel actif par instance Sharpie (port)
...
- [PWA et mobile](/mobile/) — joindre Sharpie depuis votre téléphone
