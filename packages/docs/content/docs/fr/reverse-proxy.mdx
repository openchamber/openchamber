---
title: Reverse proxy
description: Configurez correctement Sharpie derrière Nginx, Nginx Proxy Manager ou un autre reverse proxy.
---

# Reverse proxy

Utilisez cette page si vous exécutez Sharpie derrière Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou un autre reverse proxy.

## Avant de le proxifier

1. Confirmez d’abord qu’Sharpie fonctionne en accès direct.
...
- Désactivez gzip sur le proxy si Sharpie compresse déjà les réponses.
...
- Sharpie joignable directement sur le LAN
...
- Sharpie compresse les réponses HTTP avec gzip (seuil de 1 Ko).
...
Les routes de streaming SSE sont exclues de la compression par Sharpie, mais le CDN peut encore les bufferiser. Consultez la documentation de votre CDN pour désactiver le buffering sur les chemins SSE.
