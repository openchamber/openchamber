...
description: Ouvrez un serveur de dev en cours d’exécution dans Sharpie.
---

# Aperçu et serveurs de dev

Quand vous démarrez un serveur de dev, Sharpie peut l’ouvrir directement dans l’application au lieu d’un onglet de navigateur séparé — vous voyez ainsi votre site à côté du chat, vous capturez sa console et vous pouvez pointer des éléments pour poser des questions à leur sujet.

## Ouvrir un aperçu

Sharpie surveille la sortie du terminal pour détecter une adresse locale (la ligne `Local:` affichée par des outils comme Vite, Next.js ou Astro). Quand il en trouve une :
