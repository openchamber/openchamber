...
Sharpie donne accès à votre machine et à votre code ; verrouillez-le donc avant que quelqu’un d’autre que vous puisse l’atteindre. Cette page couvre le mot de passe UI, les passkeys et ce qu’il faut savoir avant d’exposer Sharpie sur un réseau.

## Définir un mot de passe UI

Démarrez Sharpie avec un mot de passe et l’UI navigateur le demandera :

```bash
openchamber --ui-password be-creative-here
```

Vous pouvez aussi le définir avec la variable d’environnement `OPENCHAMBER_UI_PASSWORD` au lieu de le mettre en ligne de commande. Après connexion, Sharpie mémorise l’appareil pendant un moment pour ne pas vous le redemander à chaque fois.
...
Une fois un mot de passe défini, vous pouvez ajouter des passkeys (Face ID, Touch ID, clé de sécurité) pour vous connecter plus vite. Ajoutez-les dans **Paramètres → Sharpie → Passkeys**.
...
- Par défaut, Sharpie n’écoute que sur votre propre machine (`127.0.0.1`). Écouter plus largement demande un changement volontaire, et vous devriez définir un mot de passe d’abord.
...
- Si vous placez Sharpie derrière votre propre serveur HTTPS, consultez [Reverse proxy](/reverse-proxy/).
...
- [Reverse proxy](/reverse-proxy/) — exécuter Sharpie derrière votre propre serveur
