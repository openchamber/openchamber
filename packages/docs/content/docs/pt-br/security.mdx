...
O Sharpie dá acesso à sua máquina e ao seu código, então proteja-o antes que qualquer pessoa além de você possa alcançá-lo. Esta página cobre a senha de UI, as passkeys e o que saber antes de expor o Sharpie a uma rede.

## Definir uma senha de UI

Inicie o Sharpie com uma senha e a UI do navegador a solicita:

```bash
openchamber --ui-password be-creative-here
```

Você também pode defini-la com a variável de ambiente `OPENCHAMBER_UI_PASSWORD` em vez de colocá-la na linha de comando. Após o login, o Sharpie lembra o dispositivo por um tempo para que você não seja solicitado toda vez.
...
Uma vez definida uma senha, você pode adicionar passkeys (Face ID, Touch ID, uma chave de segurança) para um login mais rápido. Adicione-as em **Settings → Sharpie → Passkeys**.
...
- Por padrão, o Sharpie só escuta na sua própria máquina (`127.0.0.1`). É preciso uma mudança deliberada para escutar mais amplamente, e você deve definir uma senha primeiro.
...
- Se você colocar o Sharpie atrás do seu próprio servidor HTTPS, veja [Reverse Proxy](/pt-br/reverse-proxy/).
...
- [Reverse Proxy](/pt-br/reverse-proxy/) — execute o Sharpie atrás do seu próprio servidor
