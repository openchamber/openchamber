...
description: Configure a integração do Sharpie e do OpenCode com variáveis de ambiente.
---

# Variáveis de ambiente

O Sharpie lê estas variáveis na inicialização. Para serviços de inicialização, `openchamber startup enable` salva por padrão um snapshot do ambiente atual, então execute de novo após mudar variáveis que o serviço deve usar.

## Servidor Sharpie

### `OPENCHAMBER_HOST`

Endereço onde o servidor web do Sharpie escuta. Use `0.0.0.0` para permitir acesso de outras máquinas.
...
Inicia o Sharpie em modo headless quando definido como `true` ou `1`. As rotas de API continuam disponíveis para clientes desktop e mobile, mas a UI do navegador não é servida.

### `OPENCHAMBER_DATA_DIR`

Altera o diretório de dados do Sharpie. O padrão é `~/.config/openchamber`.
...
Conecta o Sharpie a um servidor OpenCode existente. O valor deve ser um origin `http` ou `https` com porta explícita e sem path, query ou hash. `OPENCODE_HOST` tem prioridade sobre `OPENCODE_PORT`.
...
Impede o Sharpie de iniciar seu próprio servidor OpenCode quando definido como `true`.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Hostname do servidor OpenCode gerenciado pelo Sharpie. O padrão é `127.0.0.1`.

### `OPENCODE_BINARY`

Caminho para o executável `opencode` que o Sharpie deve usar.
...
Alias específico do Sharpie para escolher a distro WSL. `OPENCODE_WSL_DISTRO` tem prioridade quando ambas estão definidas.
...
Executável de shell usado nas sessões de terminal do Sharpie.

### `OPENCHAMBER_GIT_BINARY`

Executável do Git usado pelos recursos Git do Sharpie.

### `GIT_BINARY`

Override alternativo para o executável do Git. Para configuração específica do Sharpie, prefira `OPENCHAMBER_GIT_BINARY`.
...
API key usada por recursos de voz do Sharpie que chamam serviços compatíveis com OpenAI.
...
Token de autenticação do ngrok usado pelos comandos de túnel do Sharpie. Você também pode configurar com `ngrok config add-authtoken <token>`.

## Auxiliares de runtime

### `BUN_BINARY`

Executável do Bun que o Sharpie deve usar ao iniciar processos daemon.

### `BUN_INSTALL`

Raiz da instalação do Bun. O Sharpie usa isso para encontrar `bin/bun` ao iniciar daemons e atualizar.
