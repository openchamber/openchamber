---
title: Proxy reverso
description: Configure o Sharpie corretamente atrás do Nginx, Nginx Proxy Manager ou outro proxy reverso.
---

# Proxy reverso

Use esta página se você executa o Sharpie atrás do Nginx, Nginx Proxy Manager, Caddy, Cloudflare ou outro proxy reverso.

## Antes de colocar um proxy

1. Primeiro confirme que o Sharpie funciona diretamente.
...
- Desative o gzip no proxy se o Sharpie já comprime as respostas.
...
- Sharpie acessível diretamente na LAN
...
- O Sharpie comprime as respostas HTTP com gzip (limite de 1 KB).
...
As rotas de streaming SSE são excluídas da compressão pelo Sharpie, mas a CDN ainda pode armazená-las em buffer. Consulte a documentação da sua CDN sobre como desativar o buffer nas rotas SSE.
