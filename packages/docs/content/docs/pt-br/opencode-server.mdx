...
description: Conecte o Sharpie a um servidor OpenCode local ou remoto.
---

# OpenCode Server

O Sharpie roda sobre um servidor OpenCode. Por padrão, ele inicia um para você, então não precisa fazer nada. Você só precisa desta página se quiser apontar o Sharpie para um servidor que já executa, ou gerenciar o que ele inicia.

## Como o Sharpie encontra um servidor

Quando o Sharpie inicia, ele procura por um servidor nesta ordem:
...
Defina estas variáveis antes de iniciar o Sharpie:
...
- `OPENCODE_SKIP_START=true` — diz ao Sharpie para não iniciar o próprio servidor.

Se você só precisa mudar a porta, defina `OPENCODE_PORT` em vez de `OPENCODE_HOST`.

Se faltar a porta em `OPENCODE_HOST` ou se ele tiver um caminho, o Sharpie o ignora e volta a iniciar o próprio servidor. Fique de olho nos logs de inicialização para um aviso `[config]` caso uma conexão esperada não aconteça.
...
## Iniciar o Sharpie no login

Use `startup enable` para instalar um serviço nativo do usuário. O Sharpie usa `launchd` no macOS, `systemd --user` no Linux e Task Scheduler no Windows.
...
Para criar um link de conexão para outro app Sharpie, use:
...
Enquanto o servidor está iniciando ou reiniciando, o Sharpie mostra um estado "OpenCode is restarting" e pausa as requisições até ele estar pronto. Isso é normal logo após a inicialização ou um reinício. Se nunca sair desse estado, veja [Conexão com o OpenCode](/pt-br/troubleshooting/opencode-connection/).
