...
| `surface` | `panel` na barra lateral, `dialog` na janela de anexar |
...
`startSession` cria uma sessão no projeto atual. Com `worktree: true` o OpenChamber cria primeiro um worktree git e abre a sessão nele. Quando `text` está definido e há um modelo selecionado, ele é enviado como primeira mensagem. O resultado é `{ sessionId, sent }`, onde `sent` é `sent`, `no-model`, `skipped` ou `failed`. A sessão existe mesmo após `no-model` e `failed`. Sem projeto aberto, worktree que falhou ou criação de sessão que falhou, o erro é `HOST_REJECTED`.
