---
title: Túneis
description: Exponha o Sharpie com segurança para acesso remoto e móvel.
---

# Túneis

Um túnel é um link público para o seu Sharpie, para que você possa acessá-lo de um navegador comum em outra rede. Use `openchamber tunnel` para criar um para uma instância em execução.
...
O Sharpie inicia o CLI do provedor de túnel na sua máquina. Instale primeiro o provedor que você quer usar:
...
Quando o túnel estiver no ar, o `status` mostra uma URL pública. Abra-a, ou escaneie o QR code, para acessar o Sharpie de qualquer lugar.

Por padrão, o Sharpie imprime um QR code em sessões TTY interativas. Use `--qr` para forçar a saída do QR, ou `--no-qr` para desativá-la.
...
- um único túnel ativo por instância do Sharpie (porta)
...
- [PWA e Acesso Móvel](/pt-br/mobile/) — acesse o Sharpie pelo celular
