...
OpenChamber also provides control commands that perform app actions instead of expanding a saved prompt. They are not managed in **Settings → Commands**.

- `/side [message]` — opens a disposable chat beside the current desktop or web conversation. Text after the command is sent immediately. Closing the side chat deletes its conversation unless you choose **Promote** first.
- `/btw [message]` — alias for `/side`.

Only one side chat can be open for a conversation. These commands are unavailable on mobile, in VS Code, and inside a side chat.
