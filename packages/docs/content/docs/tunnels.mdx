---
title: Tunnels
description: Expose Sharpie safely for remote and mobile access.
---

# Tunnels

A tunnel is a public link to your Sharpie, so you can reach it from an ordinary browser on another network. Use `openchamber tunnel` to create one for a running instance.
...
Sharpie starts the tunnel provider CLI on your machine. Install the provider you want to use first:
...
When the tunnel is up, `status` shows a public URL. Open it, or scan the QR code, to reach Sharpie from anywhere.

By default, Sharpie prints a QR code in interactive TTY sessions. Use `--qr` to force QR output, or `--no-qr` to disable it.
...
- one active tunnel per Sharpie instance (port)
