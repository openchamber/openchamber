...
description: Configure Sharpie and OpenCode integration with environment variables.
---

# Environment Variables

Sharpie reads these environment variables at startup. For startup services, `openchamber startup enable` snapshots the current environment by default, so rerun it after changing variables you want the service to use.

## Sharpie server

### `OPENCHAMBER_HOST`

Bind address for the Sharpie web server. Use `0.0.0.0` to allow access from other machines.
...
Starts Sharpie in headless mode when set to `true` or `1`. API routes stay available for desktop and mobile clients, but the browser UI is not served.

### `OPENCHAMBER_DATA_DIR`

Overrides the Sharpie data directory. The default is `~/.config/openchamber`.
...
Connects Sharpie to an existing OpenCode server. The value must be an `http` or `https` origin with an explicit port and no path, query, or hash. `OPENCODE_HOST` takes precedence over `OPENCODE_PORT`.
...
Prevents Sharpie from starting its own OpenCode server when set to `true`.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Bind hostname for the OpenCode server managed by Sharpie. The default is `127.0.0.1`.

### `OPENCODE_BINARY`

Path to the `opencode` executable Sharpie should run.
...
Sharpie-specific alias for selecting the WSL distro. `OPENCODE_WSL_DISTRO` takes precedence when both are set.
...
Shell executable used for Sharpie terminal sessions.

### `OPENCHAMBER_GIT_BINARY`

Git executable used by Sharpie Git features.

### `GIT_BINARY`

Alternative Git executable override. Prefer `OPENCHAMBER_GIT_BINARY` for Sharpie-specific configuration.
...
API key used by Sharpie voice features that call OpenAI-compatible services.
...
ngrok auth token used by Sharpie tunnel commands. You can also configure ngrok with `ngrok config add-authtoken <token>`.

## Runtime helpers

### `BUN_BINARY`

Bun executable Sharpie should use when spawning daemon processes.

### `BUN_INSTALL`

Bun installation root. Sharpie uses it to find `bin/bun` for daemon startup and updates.
