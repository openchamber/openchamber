...
3. Pick where you'll use the device:
   ...
   - **Managed tunnel (E2EE)** — when an enabled managed-remote profile is active,
     connects the native app through your Cloudflare hostname with application-layer end-to-end encryption
...
- **Managed-tunnel E2EE is explicit.** It appears only for an enabled active managed-remote profile.
  It does not turn the browser tunnel link into a native connection or silently fall back to plaintext application traffic.

## Browser tunnel, managed E2EE, and Private Relay

- A tunnel's **Browser access** QR opens `/connect` in a browser and creates a tunnel-session cookie.
- **Managed tunnel (E2EE)** is a native Pairing v2 option through your own active Cloudflare hostname.
  Cloudflare can observe connection metadata and ciphertext sizes,
  but not the encrypted application payload.
- **Anywhere** uses the hosted [Private Relay](/private-relay/) and remains available independently when requested.

LAN candidates remain available under the existing local-network choices.
Choosing managed E2EE suppresses only the plaintext native candidate for the same Cloudflare origin;
it does not remove LAN or an explicitly requested hosted-relay candidate.
