---
title: 远程访问
description: 修复隧道、远程实例，以及从另一台设备访问 Sharpie 的问题。
---

# 远程访问

当你无法从手机或另一台机器访问 Sharpie 时，修复方法取决于你的连接方式。
...
- 对于 **任何地方** 配对，检查服务器上的 **Settings → Remote Instances → Sharpie Relay**：它应显示 **已连接**；如果不是，停用后再重新启用
...
当某个 [远程实例](/zh-cn/remote-instances/) 停滞时，Sharpie 会指出失败的步骤：

- **auth** — 你的 SSH 或 UI 密码被拒绝；重新输入它
- **install / start** — Sharpie 无法在远程机器上设置或启动服务器；检查该机器的要求
- **forwarding** — 连接已建立但端口没有到达你；尝试一个不同的本地端口

## 在你自己的服务器后面

如果你把 Sharpie 放在反向代理后面，而它加载异常或无法连接，请参阅 [反向代理](/zh-cn/reverse-proxy/)。
