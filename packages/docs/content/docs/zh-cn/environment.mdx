---
title: 环境变量
description: 使用环境变量配置 Sharpie 和 OpenCode 集成。
---

# 环境变量

Sharpie 会在启动时读取这些变量。对于开机启动服务，`openchamber startup enable` 默认会保存当前环境的 snapshot，所以如果要让服务使用新的变量，请重新运行它。

## Sharpie 服务器

### `OPENCHAMBER_HOST`

Sharpie web 服务器监听的地址。使用 `0.0.0.0` 可允许其他机器访问。
...
设置为 `true` 或 `1` 时，以 headless 模式启动 Sharpie。桌面和移动客户端仍可使用 API 路由，但不会提供浏览器 UI。

### `OPENCHAMBER_DATA_DIR`

覆盖 Sharpie 数据目录。默认是 `~/.config/openchamber`。
...
将 Sharpie 连接到已有的 OpenCode server。值必须是带明确端口、没有 path、query 或 hash 的 `http` 或 `https` origin。`OPENCODE_HOST` 优先于 `OPENCODE_PORT`。
...
设置为 `true` 时阻止 Sharpie 启动自己的 OpenCode server。

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Sharpie 管理的 OpenCode server 绑定 hostname。默认是 `127.0.0.1`。

### `OPENCODE_BINARY`

Sharpie 应该运行的 `opencode` executable 路径。
...
Sharpie 专用的 WSL distro alias。如果两个变量都设置，`OPENCODE_WSL_DISTRO` 优先。
...
Sharpie terminal sessions 使用的 shell executable。

### `OPENCHAMBER_GIT_BINARY`

Sharpie Git 功能使用的 Git executable。

### `GIT_BINARY`

Git executable 的备用 override。Sharpie 专用配置建议使用 `OPENCHAMBER_GIT_BINARY`。
...
Sharpie 语音功能调用 OpenAI-compatible services 时使用的 API key。
...
Sharpie tunnel commands 使用的 ngrok auth token。也可以用 `ngrok config add-authtoken <token>` 配置。

## Runtime helpers

### `BUN_BINARY`

Sharpie 启动 daemon processes 时使用的 Bun executable。

### `BUN_INSTALL`

Bun 安装根目录。Sharpie 使用它查找 `bin/bun` 来启动 daemon 和执行更新。
