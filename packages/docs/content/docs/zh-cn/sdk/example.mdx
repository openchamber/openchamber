...
在该文件夹中用 `bunx openchamber-guest-bundle panel/main.ts panel/main.js` 构建，然后在 设置 → 扩展 中添加这个文件夹。另外四个示例（每个 UI 控件、带会话和提示词的任务列表、一个本地服务、一个令牌集成）在 [github.com/openchamber/openchamber/tree/main/packages/sdk/examples](https://github.com/openchamber/openchamber/tree/main/packages/sdk/examples)。
