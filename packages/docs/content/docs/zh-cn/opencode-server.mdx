...
description: 将 Sharpie 连接到本地或远程的 OpenCode 服务器。
---

# OpenCode Server

Sharpie 运行在 OpenCode 服务器之上。默认情况下，它会为你启动一个，所以你无需做任何事。只有当你想让 Sharpie 指向一个你已经运行的服务器，或者管理它所启动的服务器时，才需要本页内容。

## Sharpie 如何查找服务器

Sharpie 启动时，会按以下顺序查找服务器：
...
- `OPENCODE_SKIP_START=true` — 告诉 Sharpie 不要启动它自己的服务器。

如果你只需要更改端口，请设置 `OPENCODE_PORT` 而不是 `OPENCODE_HOST`。

如果 `OPENCODE_HOST` 缺少端口或带有路径，Sharpie 会忽略它并回退到启动自己的服务器。如果你期望的连接没有发生，请留意启动日志中的 `[config]` 警告。
...
使用 `startup enable` 安装原生用户服务。Sharpie 在 macOS 上使用 `launchd`，在 Linux 上使用 `systemd --user`，在 Windows 上使用 Task Scheduler。
...
当服务器正在启动或重启时，Sharpie 会显示 “OpenCode is restarting” 状态，并暂停请求直到服务器就绪。这在启动或重启后立即出现是正常的。如果它一直无法消失，请参阅 [OpenCode 连接](/zh-cn/troubleshooting/opencode-connection/)。
