...
아무것도 설정하지 않으면 3단계가 자동으로 실행되어 바로 사용할 수 있습니다. OpenChamber는
기본 포트의 관련 없는 서버에 맹목적으로 연결하지 않으며, 저장된 소유자와 프로세스 ID가
현재 OpenChamber 인스턴스와 일치할 때만 기존 guardian 자식을 채택합니다.
