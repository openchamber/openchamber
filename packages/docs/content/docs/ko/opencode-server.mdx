---
title: OpenCode 서버
description: Sharpie를 로컬 또는 원격 OpenCode 서버에 연결하세요.
---

# OpenCode 서버

Sharpie는 OpenCode 서버 위에서 실행됩니다. 기본적으로 Sharpie가 서버를 직접 시작하므로 따로 할 일은 없습니다. 이미 운영 중인 서버를 Sharpie가 사용하도록 지정하거나, Sharpie가 시작한 서버를 관리하려는 경우에만 이 페이지가 필요합니다.

## Sharpie가 서버를 찾는 방법

Sharpie가 시작되면 다음 순서로 서버를 찾습니다.
...
- `OPENCODE_SKIP_START=true` — Sharpie가 자체 서버를 시작하지 않도록 지정합니다.

포트만 변경하면 된다면 `OPENCODE_HOST` 대신 `OPENCODE_PORT`를 설정하세요.

`OPENCODE_HOST`에 포트가 없거나 경로가 붙어 있으면 Sharpie는 이를 무시하고 자체 서버를 시작하는 방식으로 되돌아갑니다. 예상한 연결이 일어나지 않았다면 시작 로그에서 `[config]` 경고를 확인하세요.
...
`startup enable`을 사용해 네이티브 사용자 서비스를 설치하세요. Sharpie는 macOS에서 `launchd`, Linux에서 `systemd --user`, Windows에서 Task Scheduler를 사용합니다.
...
다른 Sharpie 앱용 연결 링크를 만들려면 다음을 사용하세요:
...
서버가 시작되거나 재시작되는 동안 Sharpie는 "OpenCode is restarting" 상태를 표시하고 준비가 끝날 때까지 요청을 일시 중지합니다. 실행 직후나 재시작 직후에는 정상적인 동작입니다. 이 상태가 계속 사라지지 않으면 [OpenCode 연결](/ko/troubleshooting/opencode-connection/)을 참고하세요.
