---
title: 원격 접속
description: 터널, 원격 인스턴스, 다른 기기에서 Sharpie에 접근하는 문제를 해결하세요.
---

# 원격 접속

휴대폰이나 다른 컴퓨터에서 Sharpie에 접근할 수 없을 때, 해결 방법은 연결 방식에 따라 다릅니다.
...
- **어디서나** 페어링이라면 서버의 **Settings → Remote Instances → Sharpie Relay**를 확인하세요. **연결됨**으로 표시되어야 하며, 아니라면 껐다가 다시 켜세요
...
[원격 인스턴스](/ko/remote-instances/)가 멈추면 Sharpie가 실패한 단계를 알려줍니다.

- **auth** — SSH 또는 UI 비밀번호가 거부됨. 다시 입력하세요
- **install / start** — Sharpie가 원격 컴퓨터에서 서버를 설정하거나 시작하지 못함. 그 컴퓨터의 요구 사항을 확인하세요
- **forwarding** — 연결은 되었지만 포트가 도달하지 않음. 다른 로컬 포트를 시도하세요

## 자체 서버 뒤에 있음

Sharpie를 리버스 프록시 뒤에 두었는데 이상하게 로드되거나 연결되지 않으면 [Reverse Proxy](/ko/reverse-proxy/)를 참고하세요.
