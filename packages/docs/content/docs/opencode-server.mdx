...
description: Connect Sharpie to a local or remote OpenCode server.
---

# OpenCode Server

Sharpie runs on top of an OpenCode server. By default it starts one for you, so you don't have to do anything. You only need this page if you want to point Sharpie at a server you already run, or manage the one it starts.

## How Sharpie finds a server

When Sharpie starts, it looks for a server in this order:
...
Set these before starting Sharpie:
...
- `OPENCODE_SKIP_START=true` — tells Sharpie not to start its own server.

If you only need to change the port, set `OPENCODE_PORT` instead of `OPENCODE_HOST`.

If `OPENCODE_HOST` is missing its port or has a path, Sharpie ignores it and falls back to starting its own server. Watch the startup logs for a `[config]` warning if a connection you expected didn't happen.
...
## Start Sharpie at login

Use `startup enable` to install a native user service. Sharpie uses `launchd` on macOS, `systemd --user` on Linux, and Task Scheduler on Windows.
...
To create a connection link for another Sharpie app, use:
...
While the server is starting or restarting, Sharpie shows an "OpenCode is restarting" state and pauses requests until it's ready. This is normal right after launch or a restart. If it never clears, see [OpenCode connection](/troubleshooting/opencode-connection/).
