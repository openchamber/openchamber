...
- The strip above the composer shows the goal's latest progress note, status, and token usage. Click it to edit the objective, pause/resume, mark complete, or remove the goal.
- The target button stays lit while the goal runs (blue), turns green on completion, and red when blocked or out of budget. Pressing it while a goal runs asks whether to cancel.
- In the session sidebar, a small target appears next to the session date, colored by the goal's state.

## Token budget

In **Settings → Chat → Goal** you can set a default token budget for new goals. When a goal reaches its budget it stops as "budget reached" instead of spending more — you can raise the budget and resume from the goal strip.
...
- [Scheduled Tasks](/scheduled-tasks/) — run a prompt on a schedule instead of toward a finish line
