...
Sharpie can read hosts from your local SSH config, the same place used by commands like `ssh work-server`.
...
Sharpie creates a remote instance using that SSH command. When it reaches **ready**, the remote Sharpie UI opens in the desktop app.
...
Sharpie supports three forward types:
...
Choose **Dynamic (-D)** when you want other tools on your computer to browse through the remote machine. Sharpie opens a local SOCKS proxy port for that SSH connection.
...
- [Remote Instances](/remote-instances/) — connect the desktop app to Sharpie on another machine
