...
Your page can tell which one it is in from `ctx.surface` (`panel` or `dialog`). When the user clicks an item you attached earlier, the same surface opens again with `ctx.item` set to that item, including any `data` you stored on it, so you can show its details instead of the list. From the + menu or the rail icon `ctx.item` is `null`.
