...
Before Sharpie can do anything, it needs at least one AI provider connected. This page covers connecting a provider, choosing a model, and tailoring agents.
...
3. Sign in one of two ways, depending on the provider:
   ...
   - **Sign-in (device flow)** — Sharpie shows a link and a short code. Open the link, enter the code, and approve. Sharpie finishes connecting on its own.
...
4. Save — Sharpie writes the provider block to OpenCode config and stores the key in OpenCode auth (literal keys) or records an `{env:VAR}` reference.
...
Provider sign-ins are stored by OpenCode, not Sharpie, so they're shared with the OpenCode CLI. If you set the same provider in more than one place, the most specific setting wins: a per-project setting overrides your personal one.
