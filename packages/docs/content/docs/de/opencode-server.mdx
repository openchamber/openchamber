...
description: Verbinde Sharpie mit einem lokalen oder entfernten OpenCode-Server.
---

# OpenCode-Server

Sharpie läuft auf einem OpenCode-Server auf. Standardmäßig startet es einen für dich, sodass du nichts tun musst. Diese Seite brauchst du nur, wenn du Sharpie auf einen Server zeigen lassen willst, den du bereits betreibst, oder den Server verwalten willst, den es startet.

## Wie Sharpie einen Server findet

Wenn Sharpie startet, sucht es in dieser Reihenfolge nach einem Server:
...
Setze diese Variablen, bevor du Sharpie startest:
...
- `OPENCODE_SKIP_START=true` — weist Sharpie an, keinen eigenen Server zu starten.

Wenn du nur den Port ändern musst, setze `OPENCODE_PORT` statt `OPENCODE_HOST`.

Wenn `OPENCODE_HOST` keinen Port hat oder einen Pfad enthält, ignoriert Sharpie ihn und fällt darauf zurück, den eigenen Server zu starten. Achte beim Start auf eine `[config]`-Warnung in den Logs, wenn eine erwartete Verbindung nicht zustande kam.
...
## Sharpie beim Anmelden starten

Verwende `startup enable`, um einen nativen Benutzerdienst zu installieren. Sharpie nutzt `launchd` auf macOS, `systemd --user` auf Linux und den Taskplaner auf Windows.
...
Um einen Verbindungslink für eine andere Sharpie-App zu erstellen, verwende:
...
Während der Server startet oder neu startet, zeigt Sharpie den Status „OpenCode wird neu gestartet“ an und pausiert Anfragen, bis er bereit ist. Das ist direkt nach dem Start oder einem Neustart normal. Wenn er nie verschwindet, siehe [OpenCode-Verbindung](/troubleshooting/opencode-connection/).
