---
title: Tunnel
description: Stelle Sharpie sicher für Remote- und Mobilzugriff bereit.
---

# Tunnel

Ein Tunnel ist ein öffentlicher Link zu deinem Sharpie, damit du ihn von einem normalen Browser in einem anderen Netzwerk aus erreichen kannst. Nutze `openchamber tunnel`, um einen für eine laufende Instanz zu erstellen.
...
Sharpie startet das CLI des Tunnel-Anbieters auf deinem Rechner. Installiere zuerst den Anbieter, den du verwenden möchtest:
...
Sobald der Tunnel aktiv ist, zeigt `status` eine öffentliche URL. Öffne sie oder scanne den QR-Code, um Sharpie von überall aus zu erreichen.

Standardmäßig druckt Sharpie in interaktiven TTY-Sitzungen einen QR-Code. Mit `--qr` erzwingst du die QR-Ausgabe, mit `--no-qr` deaktivierst du sie.
...
- ein aktiver Tunnel pro Sharpie-Instanz (Port)
