...
description: Konfiguriere die Sharpie- und OpenCode-Integration mit Umgebungsvariablen.
---

# Umgebungsvariablen

Sharpie liest diese Umgebungsvariablen beim Start. Für Startdienste speichert `openchamber startup enable` die aktuelle Umgebung standardmäßig als Schnappschuss, also führe den Befehl nach Änderungen an den Variablen, die der Dienst verwenden soll, erneut aus.

## Sharpie-Server

### `OPENCHAMBER_HOST`

Bind-Adresse für den Sharpie-Webserver. Verwende `0.0.0.0`, um Zugriff von anderen Maschinen zu erlauben.
...
Startet Sharpie im Headless-Modus, wenn auf `true` oder `1` gesetzt. API-Routen bleiben für Desktop- und Mobile-Clients verfügbar, aber die Browser-Oberfläche wird nicht ausgeliefert.

### `OPENCHAMBER_DATA_DIR`

Überschreibt das Sharpie-Datenverzeichnis. Standard ist `~/.config/openchamber`.
...
Verbindet Sharpie mit einem vorhandenen OpenCode-Server. Der Wert muss ein `http`- oder `https`-Origin mit explizitem Port und ohne Pfad, Query oder Hash sein. `OPENCODE_HOST` hat Vorrang vor `OPENCODE_PORT`.
...
Verhindert, dass Sharpie einen eigenen OpenCode-Server startet, wenn auf `true` gesetzt.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Bind-Hostname für den von Sharpie verwalteten OpenCode-Server. Standard ist `127.0.0.1`.

### `OPENCODE_BINARY`

Pfad zur ausführbaren `opencode`-Datei, die Sharpie ausführen soll.
...
Sharpie-spezifischer Alias zur Auswahl der WSL-Distribution. `OPENCODE_WSL_DISTRO` hat Vorrang, wenn beide gesetzt sind.
...
Shell-Executable, die Sharpie für Terminal-Sessions verwendet.

### `OPENCHAMBER_GIT_BINARY`

Git-Executable, die Sharpie-Git-Funktionen verwenden.

### `GIT_BINARY`

Alternative Git-Executable-Überschreibung. Bevorzuge `OPENCHAMBER_GIT_BINARY` für Sharpie-spezifische Konfiguration.
...
API-Schlüssel, den Sharpie-Voice-Funktionen für OpenAI-kompatible Dienste verwenden.
...
ngrok-Auth-Token, den Sharpie-Tunnel-Befehle verwenden. Du kannst ngrok auch mit `ngrok config add-authtoken <token>` konfigurieren.

## Laufzeit-Helfer

### `BUN_BINARY`

Bun-Executable, die Sharpie beim Starten von Daemon-Prozessen verwenden soll.

### `BUN_INSTALL`

Installationswurzel von Bun. Sharpie verwendet sie, um `bin/bun` für Daemon-Start und Updates zu finden.
