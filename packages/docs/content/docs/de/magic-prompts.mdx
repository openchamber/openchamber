...
Einige Prompts haben einen sichtbaren Teil (die Nachricht, die du sehen würdest) und einen Anweisungsteil (versteckte Hinweise für den Agenten). Prompts können `{{placeholders}}` enthalten, die Sharpie einsetzt, zum Beispiel den Diff oder den Issue-Titel — lass diese unverändert.
