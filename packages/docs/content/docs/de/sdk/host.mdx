...
| `surface` | `panel` in der Leiste, `dialog` im Anhängen-Fenster |
...
`startSession` legt eine Session im aktuellen Projekt an. Mit `worktree: true` erstellt OpenChamber zuerst einen Git-Worktree und öffnet die Session dort. Ist `text` gesetzt und ein Modell ausgewählt, wird er als erste Nachricht gesendet. Das Ergebnis ist `{ sessionId, sent }`, wobei `sent` einer der Werte `sent`, `no-model`, `skipped` oder `failed` ist. Die Session existiert auch nach `no-model` und `failed`. Kein offenes Projekt, ein fehlgeschlagener Worktree oder ein fehlgeschlagenes Anlegen der Session ist ein `HOST_REJECTED`-Fehler.
