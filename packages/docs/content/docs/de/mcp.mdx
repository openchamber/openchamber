...
2. Füge einen Server hinzu und wähle seinen Typ:
   - **lokal** — Sharpie führt einen Befehl auf deinem Rechner aus. Du gibst den auszuführenden Befehl an und, falls nötig, Umgebungsvariablen.
   - **entfernt** — Sharpie verbindet sich mit einer URL, die jemand anderes hostet. Du gibst die URL und alle nötigen Header an (zum Beispiel ein Auth-Token).
