---
title: Dostęp zdalny
description: Napraw tunele, zdalne instancje i dostęp do Sharpie z innego urządzenia.
---

# Dostęp zdalny

Gdy nie możesz dotrzeć do Sharpie z telefonu lub innej maszyny, rozwiązanie zależy od tego, jak się łączysz.
...
- przy parowaniu **Wszędzie** sprawdź **Settings → Remote Instances → Sharpie Relay** na serwerze: status powinien brzmieć **Połączono**; jeśli nie, wyłącz i włącz relay ponownie
...
Gdy [zdalna instancja](/pl/remote-instances/) się zatnie, Sharpie wskazuje krok, który zawiódł:

- **auth** — Twoje hasło SSH lub UI zostało odrzucone; wpisz je ponownie
- **install / start** — Sharpie nie mógł skonfigurować lub uruchomić serwera na zdalnej maszynie; sprawdź wymagania tej maszyny
- **forwarding** — połączenie działa, ale port do Ciebie nie dociera; spróbuj innego portu lokalnego

## Za własnym serwerem

Jeśli umieściłeś Sharpie za reverse proxy i ładuje się dziwnie lub nie chce się połączyć, zobacz [Reverse Proxy](/pl/reverse-proxy/).
