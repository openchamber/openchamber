...
description: Napraw brak połączenia Sharpie z serwerem OpenCode.
---

# Połączenie z OpenCode

Jeśli Sharpie się ładuje, ale nigdy nie przechodzi poza „OpenCode uruchamia się ponownie”, albo czat nie odpowiada, serwer, z którym się komunikuje, jest nieosiągalny. Przejdź przez te kroki.
...
Jeśli ustawiłeś Sharpie tak, aby używał istniejącego serwera, sprawdź ponownie konfigurację z [OpenCode Server](/pl/opencode-server/):

- `OPENCODE_HOST` musi zawierać port, bez ścieżki — na przykład `http://localhost:4096`
- ustaw `OPENCODE_SKIP_START=true`, aby Sharpie nie uruchamiał też własnego
- jeśli adres jest nieprawidłowy, Sharpie go ignoruje i uruchamia własny serwer — poszukaj ostrzeżenia `[config]` w logach
...
- [OpenCode Server](/pl/opencode-server/) — jak Sharpie znajduje serwer i nim zarządza
