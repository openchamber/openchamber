---
title: Reverse proxy
description: Skonfiguruj Sharpie poprawnie za Nginx, Nginx Proxy Manager lub innym reverse proxy.
---

# Reverse proxy

Skorzystaj z tej strony, jeśli uruchamiasz Sharpie za Nginx, Nginx Proxy Manager, Caddy, Cloudflare lub innym reverse proxy.

## Zanim ustawisz proxy

1. Najpierw potwierdź, że Sharpie działa bezpośrednio.
...
- Wyłącz gzip na proxy, jeśli Sharpie już kompresuje odpowiedzi.
...
- Sharpie dostępny bezpośrednio w LAN
...
- Sharpie kompresuje odpowiedzi HTTP za pomocą gzip (próg 1 KB).
...
Trasy strumieniowania SSE są wyłączone z kompresji przez Sharpie, ale CDN może je nadal buforować. Sprawdź dokumentację swojego CDN, jak wyłączyć buforowanie na ścieżkach SSE.
