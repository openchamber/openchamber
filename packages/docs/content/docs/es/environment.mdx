...
description: Configura la integración de Sharpie y OpenCode con variables de entorno.
---

# Variables de entorno

Sharpie lee estas variables al arrancar. Para servicios de inicio, `openchamber startup enable` guarda por defecto una copia del entorno actual, así que vuelve a ejecutarlo después de cambiar variables que quieras usar en el servicio.

## Servidor Sharpie

### `OPENCHAMBER_HOST`

Dirección donde escucha el servidor web de Sharpie. Usa `0.0.0.0` para permitir acceso desde otras máquinas.
...
Inicia Sharpie en modo headless cuando vale `true` o `1`. Las rutas API siguen disponibles para clientes de escritorio y móviles, pero no se sirve la UI del navegador.

### `OPENCHAMBER_DATA_DIR`

Cambia el directorio de datos de Sharpie. Por defecto es `~/.config/openchamber`.
...
Conecta Sharpie a un servidor OpenCode existente. Debe ser un origin `http` o `https` con puerto explícito y sin ruta, query ni hash. `OPENCODE_HOST` tiene prioridad sobre `OPENCODE_PORT`.
...
Evita que Sharpie arranque su propio servidor OpenCode cuando vale `true`.

### `OPENCHAMBER_OPENCODE_HOSTNAME`

Hostname para el servidor OpenCode gestionado por Sharpie. Por defecto es `127.0.0.1`.

### `OPENCODE_BINARY`

Ruta al ejecutable `opencode` que debe usar Sharpie.
...
Alias específico de Sharpie para elegir la distro WSL. `OPENCODE_WSL_DISTRO` tiene prioridad si ambas están definidas.
...
Ejecutable de shell para las sesiones de terminal en Sharpie.

### `OPENCHAMBER_GIT_BINARY`

Ejecutable de Git usado por las funciones Git de Sharpie.

### `GIT_BINARY`

Override alternativo para el ejecutable de Git. Para ajustes específicos de Sharpie, prefiere `OPENCHAMBER_GIT_BINARY`.
...
API key para funciones de voz de Sharpie que llaman a servicios compatibles con OpenAI.
...
Token de autenticación de ngrok usado por los comandos de túnel de Sharpie. También puedes configurarlo con `ngrok config add-authtoken <token>`.

## Ayudas de runtime

### `BUN_BINARY`

Ejecutable de Bun que Sharpie debe usar al iniciar procesos daemon.

### `BUN_INSTALL`

Raíz de instalación de Bun. Sharpie la usa para encontrar `bin/bun` al iniciar daemons y actualizar.
