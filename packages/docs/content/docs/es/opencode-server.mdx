...
description: Conecta Sharpie a un servidor de OpenCode local o remoto.
---

# OpenCode Server

Sharpie se ejecuta sobre un servidor de OpenCode. De forma predeterminada inicia uno por ti, así que no tienes que hacer nada. Solo necesitas esta página si quieres apuntar Sharpie a un servidor que ya ejecutas, o gestionar el que él inicia.

## Cómo encuentra Sharpie un servidor

Cuando Sharpie arranca, busca un servidor en este orden:
...
Define esto antes de iniciar Sharpie:
...
- `OPENCODE_SKIP_START=true` — le indica a Sharpie que no inicie su propio servidor.

Si solo necesitas cambiar el puerto, define `OPENCODE_PORT` en lugar de `OPENCODE_HOST`.

Si a `OPENCODE_HOST` le falta el puerto o tiene una ruta, Sharpie lo ignora y recurre a iniciar su propio servidor. Observa los registros de arranque por si aparece una advertencia `[config]` cuando una conexión que esperabas no ocurrió.
...
## Iniciar Sharpie al iniciar sesión

Usa `startup enable` para instalar un servicio nativo de usuario. Sharpie usa `launchd` en macOS, `systemd --user` en Linux y Task Scheduler en Windows.
...
Para crear un enlace de conexión para otra app de Sharpie, usa:
...
Mientras el servidor se inicia o reinicia, Sharpie muestra un estado "OpenCode is restarting" y pausa las solicitudes hasta que esté listo. Esto es normal justo después de iniciar o reiniciar. Si nunca desaparece, consulta [Conexión de OpenCode](/es/troubleshooting/opencode-connection/).
