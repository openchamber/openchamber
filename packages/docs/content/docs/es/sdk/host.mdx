...
| `surface` | `panel` en la barra lateral, `dialog` en la ventana de adjuntar |
...
`startSession` crea una sesión en el proyecto actual. Con `worktree: true` OpenChamber crea primero un worktree de git y abre la sesión ahí. Si `text` está definido y hay un modelo seleccionado, se envía como primer mensaje. El resultado es `{ sessionId, sent }`, donde `sent` es `sent`, `no-model`, `skipped` o `failed`. La sesión existe incluso tras `no-model` y `failed`. Si no hay proyecto abierto, falla el worktree o falla la creación de la sesión, el error es `HOST_REJECTED`.
