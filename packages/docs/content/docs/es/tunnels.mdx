---
title: Túneles
description: Expón Sharpie de forma segura para acceso remoto y móvil.
---

# Túneles

Un túnel es un enlace público a tu Sharpie, para que puedas acceder a él desde un navegador corriente en otra red. Usa `openchamber tunnel` para crear uno para una instancia en marcha.
...
Sharpie inicia el CLI del proveedor de túnel en tu máquina. Instala primero el proveedor que quieras usar:
...
Cuando el túnel está activo, `status` muestra una URL pública. Ábrela, o escanea el código QR, para acceder a Sharpie desde cualquier lugar.

Por defecto, Sharpie imprime un código QR en sesiones TTY interactivas. Usa `--qr` para forzar la salida del QR, o `--no-qr` para desactivarla.
...
- un único túnel activo por instancia de Sharpie (puerto)
...
- [PWA y acceso móvil](/es/mobile/) — accede a Sharpie desde tu teléfono
