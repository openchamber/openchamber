...
description: Налаштуйте Sharpie коректно за Nginx, Nginx Proxy Manager або іншим зворотним проксі.
---

# Зворотний проксі

Скористайтеся цією сторінкою, якщо запускаєте Sharpie за Nginx, Nginx Proxy Manager, Caddy, Cloudflare або іншим зворотним проксі.

## Перш ніж проксувати

1. Спершу переконайтеся, що Sharpie працює напряму.
...
- Вимкніть gzip на проксі, якщо Sharpie уже стискає відповіді.
...
- Sharpie доступний напряму в LAN
...
- Sharpie стискає HTTP-відповіді через gzip (поріг 1 КБ).
...
SSE-маршрути потоків виключені зі стиснення в Sharpie, але CDN усе одно може їх буферизувати. Перевірте документацію свого CDN щодо вимкнення буферизації на SSE-шляхах.
