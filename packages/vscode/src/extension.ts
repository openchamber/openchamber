...
export async function activate(context: vscode.ExtensionContext) {
  outputChannel = vscode.window.createOutputChannel('Sharpie');
  ...
  const attemptMoveChatToRightSidebar = async (): Promise<'moved' | 'unsupported' | 'failed'> => {
    ...
    try {
      ...
      outputChannel?.appendLine(
        `[Sharpie] Failed moving chat view to right sidebar (command=${moveCommandId}): ${error instanceof Error ? error.message : String(error)}`
      );
      return 'failed';
    }
  };
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.openSidebar', async () => {
      ...
      try {
        ...
        outputChannel?.appendLine(`[Sharpie] workbench.view.extension.openchamber failed: ${e}`);
      }

      try {
        await vscode.commands.executeCommand('openchamber.chatView.focus');
      } catch (e) {
        outputChannel?.appendLine(`[Sharpie] openchamber.chatView.focus failed: ${e}`);
        vscode.window.showErrorMessage(t('Sharpie: Failed to open sidebar - {0}', String(e)));
        return false;
      }

      if (!chatViewProvider?.hasResolvedView()) {
        outputChannel?.appendLine('[Sharpie] Chat sidebar focus completed before the webview was resolved');
        vscode.window.showWarningMessage(t('Sharpie: Chat sidebar is not ready'));
        return false;
      }
  ...
  const revealChatViewForPayload = async () => {
    ...
    if (!chatViewProvider?.hasResolvedView()) {
      outputChannel?.appendLine('[Sharpie] Chat sidebar webview was disposed before payload delivery');
      vscode.window.showWarningMessage(t('Sharpie: Chat sidebar is not ready'));
      return false;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.openActiveSessionInEditor', () => {
      if (!activeSessionId) {
        vscode.window.showInformationMessage(t('Sharpie: No active session'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.restartApi', async () => {
      try {
        ...
        vscode.window.showInformationMessage(t('Sharpie: API connection restarted'));
      } catch (e) {
        vscode.window.showErrorMessage(t('Sharpie: Failed to restart API - {0}', String(e)));
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.addToContext', async () => {
      ...
      if (!editor) {
        vscode.window.showWarningMessage(t('Sharpie [Add to Context]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Sharpie [Add to Context]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.attachExplorerToChat', async (resource?: vscode.Uri, resources?: vscode.Uri[]) => {
      ...
      if (attachedFiles.length === 0) {
        vscode.window.showWarningMessage(t('Sharpie: No file selected to mention'));
        return;
      }
      ...
      if (skippedEntries.length > 0) {
        vscode.window.showInformationMessage(t('Sharpie: Some selected entries were skipped (folders or unsupported resources)'));
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.explain', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Sharpie [Explain]: No active editor'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.improveCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(t('Sharpie [Improve Code]: No active editor'));
        return;
      }
      ...
      if (!selectedText) {
        vscode.window.showWarningMessage(t('Sharpie [Improve Code]: No text selected'));
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.newSession', async (directory?: unknown) => {
      ...
      if (!folderPath && candidates.length === 0) {
        vscode.window.showInformationMessage('Sharpie: No folder is open. Open a folder to start a new session.');
        return;
      }
      ...
      if (openCodeManager) {
        ...
        if (!result.success) {
          vscode.window.showErrorMessage(`Sharpie: ${result.error}`);
          return;
        }
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('openchamber.showOpenCodeStatus', async () => {
      ...
      const lines = [
        ...
        `Sharpie version: ${extensionVersion || '(unknown)'}`,
