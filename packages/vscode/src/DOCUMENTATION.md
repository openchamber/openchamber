...
- `gitService.ts`
  ...
  - Splits large selected stage and unstage lists before spawning Git. An explicit `stageFiles` selected commit or an oversized selected commit starts a temporary index from `HEAD` (or empty), writes only selected current index entries into it, and removes selected staged deletions through a NUL-delimited literal pathspec before native `git commit`. This preserves selected staged-only/deleted/untracked content and exact unrelated partial, unmerged, intent-to-add, and newline-path index state without temporarily unstaging/restaging it. A later staging batch failure rejects the bridge operation instead of reporting the full selection as successful.
