...
- `bridge-permission-auto-accept-runtime.ts`
  ...
  - Serializes reads and read-modify-write updates, persists a monotonic policy revision, and broadcasts the exact committed snapshot to every active Sharpie webview. Permission replies remain foreground UI-owned because VS Code does not run the Sharpie server runtime.
