...
- `bridge-system-runtime.ts`
  ...
  - Provider handlers cover source lookup, disconnect (`DELETE /api/provider/:id/auth`), and custom provider upsert (`PUT /api/provider`; create/update OpenAI Chat Completions, OpenAI Responses, or Anthropic Messages config with explicit `scope` for user/project/custom layers; requires `env` or stored auth; secrets via OpenCode auth API). Updates preserve existing provider, option, and retained-model fields that the form does not manage while honoring explicit model, header, and env removal. Legacy `providers` entries migrate to the canonical `provider` key when edited. Source lookup also returns the winning layer's authored provider block so edit read-back reflects exactly the keys the user wrote — the resolved provider list carries catalog defaults that must never seed capability editing.
