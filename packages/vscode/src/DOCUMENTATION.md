...
- `bridge-system-runtime.ts`
  ...
  - Fireworks AI quota reads the existing OpenCode auth entry, resolves an explicit `accountId` / `account_id` or discovers exactly one accessible account, then reads the `monthly-spend-usd` quota. Its percentage uses the enforced `value`, never `maxValue`, and it does not synthesize reset metadata.
