...
- `gitService.ts`
  ...
  - Splits large selected stage and unstage lists before spawning Git. An oversized selected commit without `stageFiles` starts a temporary index from `HEAD` (or empty), writes only selected current index entries into it, and removes selected staged deletions through a NUL-delimited literal pathspec before committing it. This preserves selected staged-only content without serializing or changing unrelated index entries. The explicit `stageFiles` flow keeps its index-managed behavior. A later staging batch failure rejects the bridge operation instead of reporting the full selection as successful.
