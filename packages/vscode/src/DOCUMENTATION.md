...
- `bridge-git-special-runtime.ts`
  ...
  - Git generation uses the legacy SDK for legacy runtimes and the generated opencode2 client for V2. The V2 path waits on `session.wait`; it does not infer completion from `/api/session/active` omission.
...
- `opencode-upgrade-runtime.ts`
  - Owns managed-versus-external capability decisions, latest-version checks, serialized OpenCode self-upgrades, and restart-after-upgrade behavior.
   - `opencode.ts` detects legacy `/global/health` versus V2 `/api/info` with an independent bounded timeout per probe. V2 is not offered the legacy self-upgrade route.
   - CLI discovery prefers `opencode`, with `opencode2` retained as a fallback name. The shared web-runtime `cli-protocol.js` probes the resolved target with `--version` before assigning ownership. Unknown versions fail before managed startup. Windows npm shims launch their referenced native executable directly when available; OpenCode desktop GUI executables remain excluded.
   - V2 uses `Service.discover()` from `@opencode/client/service` as the authoritative URL/auth source for every proxy, SSE stream and watcher. This is connect-only, including when discovery fails. The extension never starts, stops, upgrades or reconfigures the shared daemon.
   - Restarting a V2 connection repeats discovery and publishes disconnect/connect transitions so transports rebind even when the URL stays the same. Deactivation clears only extension-local state. V1 managed children retain direct `serve` startup, generated auth, process registry, orphan cleanup and owned shutdown. An explicit `openchamber.apiUrl` remains authoritative and bypasses CLI probing and service discovery.

- `sessionActivityWatcher.ts`
  - Legacy runtimes reconcile the complete `/session/status` snapshot and retire omitted entries.
  - opencode2 marks sessions returned by `/api/session/active` busy but treats omitted sessions as unknown. Terminal execution events provide the proof needed to retire activity.
  - Legacy global events use the legacy SDK stream. opencode2 subscribes through the generated client at `/api/event`.
