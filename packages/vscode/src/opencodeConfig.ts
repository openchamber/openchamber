...
const normalizePositiveIntegerLimit = (value: unknown, fieldName: string): number | null => {
...
const getUserProviderModelEntry = (
...
const cleanupEmptyProviderModelOverride = (provider: Record<string, unknown>, modelId: string): void => {
...
export const getUserProviderModelContextLimits = (providerId: string) => {
  ...
  const provider = isPlainObject(providers[providerId]) ? providers[providerId] : null;
  const models = provider && isPlainObject(provider.models) ? provider.models : {};
  const result: Record<string, { context: number; output?: number }> = {};

  for (const [modelId, modelEntry] of Object.entries(models)) {
    if (!isPlainObject(modelEntry) || !isPlainObject(modelEntry.limit)) continue;
    const context = modelEntry.limit.context;
    if (typeof context !== 'number' || !Number.isFinite(context)) continue;
  ...
  return {
    providerId,
    models: result,
    source: { scope: 'user', path: layers.paths.userPath },
  };
};

export const updateUserProviderModelContextLimit = (
  providerId: string,
  modelId: string,
  contextLimit: unknown,
  outputLimit: unknown,
  maxContextLimit: unknown,
) => {
  if (!providerId) throw new Error('Provider ID is required');
  ...
  const providers = targetConfig.provider as Record<string, unknown>;
  ...
  const existingModel = getUserProviderModelEntry(targetConfig, providerId, modelId) || {};
  const modelEntry: Record<string, unknown> = { ...existingModel };
  ...
  if (normalizedContext === null) {
    ...
    cleanupEmptyProviderModelOverride(provider, modelId);
  ...
  const nextLimit: Record<string, unknown> = {
    ...existingLimit,
