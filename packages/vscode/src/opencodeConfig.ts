  ...
  for (const { dir, source } of [...getGlobalSnippetDirs(), ...getProjectSnippetDirs(workingDirectory)]) {
    ...
    for (const filename of fs.readdirSync(dir)) {
      ...
      try {
        ...
        console.warn(`[Sharpie][VSCode] Failed to load snippet ${path.join(dir, filename)}:`, error);
      }
    }
  }
  return registry;
...
const parseMdFile = (filePath: string): { frontmatter: Record<string, unknown>; body: string } => {
  ...
  try {
    ...
    console.warn(`[Sharpie][VSCode] Failed to parse frontmatter for ${filePath}, treating as empty:`, error);
    frontmatter = {};
  }
