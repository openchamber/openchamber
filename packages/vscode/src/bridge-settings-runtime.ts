...
export const persistSettings = async (changes: Record<string, unknown>, ctx?: BridgeContext): Promise<Record<string, unknown>> => {
  ...
  for (const key of ['defaultModel', 'defaultVariant', 'defaultAgent', 'defaultDirectory', 'defaultGitIdentityId', 'opencodeBinary', 'smallModelOverride', 'walkthroughModelOverride']) {
