...
import { handleLocalApiRequest } from './localApi';
...
const handleConnectionMessage = (event: MessageEvent) => {
  ...
  if (msg?.type === 'connectionStatus') {
    ...
    const cliAvailable = typeof msg.cliAvailable === 'boolean' ? msg.cliAvailable : prevCliAvailable;
    window.__OPENCHAMBER_CONNECTION__ = { status: payload, error, cliAvailable };
    window.dispatchEvent(new CustomEvent('openchamber:connection-status', { detail: { status: payload, error, cliAvailable } }));
  }
};
