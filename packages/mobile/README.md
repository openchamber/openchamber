...
Capacitor shell for the dedicated Sharpie mobile web surface.
...
- The native app bundles the mobile UI only; it does not embed the Sharpie web server or OpenCode server.
- On first launch in Capacitor, the app shows a connection screen for an existing Sharpie server.
...
- Password-protected Sharpie servers can be unlocked from the mobile app. The app stores the issued client token with the saved connection.
- The Terminal workspace surface runs its PTY on the active Sharpie server over the shared authenticated runtime transport; it never opens a local shell on the phone or tablet. Closing the surface detaches the renderer while the server session remains available for reattachment. On touch devices, dragging scrolls the buffer while long-pressing and dragging selects terminal text.
...
- If connecting to a remote Sharpie server fails from the app while `/health` works in curl, check that the server build includes the packaged-client CORS allowlist for `capacitor://localhost` and local dev origins.
