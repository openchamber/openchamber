...
function getRestorableUserMessageDataFromStore(
  state: ReturnType<DirectoryStoreApi["getState"]>,
  sessionId: string,
  messageId: string,
): RestorableUserMessageData | null {
  const targetMsg = (state.message[sessionId] ?? []).find((message) => message.id === messageId)
...
function getRestorableUserMessageDataFromSession3(message: SessionMessage): RestorableUserMessageData {
  if (message.type !== "user") return EMPTY_RESTORABLE_USER_MESSAGE_DATA
  return {
...
async function getRestorableUserMessageData(
  ...
  if (local && (local.messageText || local.fileParts.length > 0)) return local
  try {
    const fallback = getRestorableUserMessageDataFromSession3(
      await opencodeClient.getSessionMessage(sessionId, messageId, directory),
    )
    if (fallback.messageText || fallback.fileParts.length > 0) return fallback
  } catch {
    // Targeted lookup is best-effort; revert/fork must still proceed.
  }
