...
function subscribeBlockingRequests(
  ...
  const subscribed = bySession
  const ids = [...new Set(sessionIDs)]
  for (const sessionID of ids) {
    let listeners = subscribed.get(sessionID)
    if (!listeners) {
      listeners = new Set()
      subscribed.set(sessionID, listeners)
    }
    listeners.add(listener)
  }
  return () => {
    for (const sessionID of ids) {
      const listeners = subscribed.get(sessionID)
      listeners?.delete(listener)
      if (listeners?.size === 0) subscribed.delete(sessionID)
    }
    if (subscribed.size === 0) subscribersByStore.delete(store)
  }
}
...
export function subscribeDirectoryPermission(
  ...
  return subscribeBlockingRequests(permissionSubscribersByStore, store, [sessionID], listener)
}

/**
 * Subscribe one consumer to the exact question buckets it renders. A sidebar row
 * stands in for its own session plus any descendants hidden by collapse, so the
 * listener is registered per session and coalesced to one call per publication.
 */
...
const notifyChangedBlockingRequests = (
  subscribersByStore: BlockingRequestSubscribers,
  ...
  const changed = new Set<BlockingRequestSubscriber>()
  for (const [sessionID, listeners] of subscribers) {
    if (current[sessionID] === previous[sessionID]) continue
    for (const listener of listeners) changed.add(listener)
  }
