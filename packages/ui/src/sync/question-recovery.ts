...
export type PendingQuestionGap = {
  callID: string
...
/**
 * Find running `question` tool parts in the current session's message records
 * that have no matching store question. A part is covered when the store has a
 * question with the same tool `callID`; when the part has no callID (or the
 * store question carries no tool reference to correlate), a part is still
 * treated as covered as long as the session has any store question — the real
 * QuestionCard is already answerable and we must not retrigger recovery.
 */
export function findPendingQuestionGaps(
  messages: Array<{ info: Message; parts: Part[] }>,
  storeQuestions: QuestionRequest[],
): PendingQuestionGap[] {
  const gaps: PendingQuestionGap[] = []
  ...
  for (const record of messages) {
    for (const part of record.parts) {
      ...
      gaps.push({ callID: part.callID, questions })
    }
  }
