...
The global sessions store persists and hydrates one bounded, runtime-scoped startup snapshot containing only active managed chat sessions. Every global session surface, including the main sidebar and Electron Mini Chat switcher, sees that stale snapshot while the global list is unresolved or failed; the first authoritative global snapshot replaces it. Because the module-init seed and runtime reset read the snapshot before the server-resolved chats root is warm, bootstrap re-hydrates the snapshot once after the warm-up so relocated chat directories are not filtered out of the stale paint. Runtime reset to idle must hydrate rather than erase the destination runtime's snapshot; authoritative empty, archive, and delete updates do persist the resulting empty or reduced list.
