...
Terminal session failures are kept in the live `session-failure-store.ts` alongside the event stream rather than inferred from idle status. A `session.error` event records the failure for the session's directory; a later busy event clears it for a new attempt, deletion clears it with the session, and a runtime switch clears the store. Idle status alone never turns a failed session into a successful one. Failure-store writes also verify the event pipeline's captured runtime key, so cleanup from an old pipeline cannot repopulate or clear the new runtime's state after a switch.
