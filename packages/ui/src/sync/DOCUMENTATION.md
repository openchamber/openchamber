...
VS Code does not run the server permission-auto-accept runtime. The extension host persists and broadcasts the authoritative snapshot (`default` plus explicit `sessions`), while its foreground UI runtime resolves missing child-session lineage through the OpenCode API before deciding whether to suppress and answer a `permission.asked` event. Resolution order is explicit current session -> nearest explicit ancestor -> global default -> false. Enabling either the current session policy or the global default reconciles pending requests in the relevant directory scope, including requests inherited by child sessions. Global-default reconciliation uses authoritative current/live directory sources only: the active directory, initialized sync directories, and VS Code workspace folders. Unknown lineage and exhausted reply retries fail closed and leave the request available for manual action. With every OpenChamber webview closed or suspended no responder runs; this is an intentional VS Code foreground-webview-only limitation.

Other runtimes remain fully server-owned. Their shared UI must keep a `permission.asked` request visible until the authoritative `permission.replied` event removes it, even when the projected auto-accept policy says the server should answer it. A failed server retry is therefore still manually actionable instead of being hidden as if it had succeeded.
