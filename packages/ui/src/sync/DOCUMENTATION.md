...
| `selection-store.ts` | Model/agent/variant selections, sticky `sessionTargets`, ephemeral `pendingHandoffTargets` for cross-harness Send handoff | App UI state |
...
5. In-flight message loads must not be discarded solely because directory bootstrap replaced child-store object identity. Generation and SDK epoch still reject forced refresh, eviction, and runtime switches. A stale empty load schedules bounded forced retries (even after `invalidateDirectory` deletes the loader entry) so Claude overlay data is not lost during startup races. Chat also keeps force-ensuring the selected session while its local message count remains zero. SyncProvider remounts clear module-level `syncSession` inflight coalescing so React Strict Mode cannot leave the remounted tree joined to a disposed loader's empty completion. `SessionMessageLoader.configure` clears the disposed bit so Strict Mode's effect cleanup+re-run cannot permanently stick a live provider on a disposed loader. SyncProvider must not `disposeAll()` child stores from an effect cleanup for the same reason — that wiped hydrated transcripts and left chat on skeletons.
