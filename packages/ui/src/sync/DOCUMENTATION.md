...
- Demand owners are independent. `SessionSidebar` publishes the full project/worktree plan under its own owner. `SyncAppEffects` also runs `useActiveSessionBootstrapDemand`, which publishes a single normalized `selected` / `selected-session` demand for `currentSessionDirectory` under the unique `active-session-effects` owner. That covers non-sidebar surfaces (PWA/mobile, VS Code webview, Electron mini chat) that mount `SyncAppEffects` without the sidebar, so blocking-request state (`question` / permission) still bootstraps for the focused session directory. The active-session owner replaces or clears only its own demand on directory change and unmount; it never clears sidebar demand. Once that directory bootstrap is complete, selecting a session and returning to a visible, focused, or restored page revalidates that session's blocking requests through authoritative `question.list` / `permission.list` reconciliation without forcing another full bootstrap. Revalidation accepts the explicit selected session before directory indexes catch up, coalesces in flight by manager + normalized directory + session ID, limits destructive cleanup to that candidate, preserves newer SSE mutations through signature guards, and preserves existing question or permission state when its corresponding fetch fails.
