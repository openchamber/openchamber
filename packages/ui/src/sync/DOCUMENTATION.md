...
8. Every normal prompt send carries the **complete** `tools` map from `agent-tool-gate.ts`, keyed on the selected model's tool-call capability. OpenCode stores that map on the user message *and* replaces the session's permission ruleset with it, so a partial map would strand OpenChamber's injected `openchamber` tool disabled for the rest of the session. Slash-command and shell sends cannot carry the field — the OpenCode API has no `tools` on those routes.
