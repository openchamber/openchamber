...
5. When automatic empty-project closure is enabled, evaluate it only after a confirmed mutation and a fresh complete global session load. Batch operations coalesce that refresh and close only projects affected by successful items.
