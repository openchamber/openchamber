...
12. A successful tail refresh replaces the authoritative range it fetched instead of union-merging omitted messages forever. A complete response covers the whole transcript. A paginated response covers the range from its oldest returned message through the tail and preserves older loaded pages. Reconciliation removes only records that existed when the request began, so a delayed response cannot erase messages delivered by live events while it was in flight; removed messages lose their part buckets in the same commit.
