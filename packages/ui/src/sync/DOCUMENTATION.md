...
5. Composer and queued sends carry their captured runtime, directory, and session through asynchronous preparation. A runtime change cancels the send instead of re-resolving it against the new runtime. Outside VS Code the queue itself is server-owned (`packages/web/server/lib/message-queue/`). The UI captures send configuration, resolved text, attachments, and context at queue time. Manual composer sends claim those items without removal, await `optimisticSend.onMessageID` to bind their exact optimistic ID, and route the SDK-generated request through the queue server's dispatch fence. `takeForSend` remains the destructive edit-to-input operation. See the `messageQueueStore.ts` section in `stores/DOCUMENTATION.md`.
