...
| `global-session-status.ts` | Incremental non-idle session status index reconciled from events and authoritative directory snapshots, plus a reference-stable active-ID membership collection and directory-scoped freshness state (`unavailableDirectories`, `acceptEventUpdates`) maintained from the same mutations | All known directories in the active runtime |
...
- `SessionNodeItem.tsx`
- `Header.tsx`
- agent/session activity surfaces using `useSessionDisplayStatus()` for presentation and `useSessionKnownInactive()` for control gates; raw `useGlobalSessionStatus()` / `useAllSessionStatuses()` remain for consumers that need the last-known value
...
Status freshness is directory-scoped. A failed `/session/status` request is not an authoritative empty snapshot: `global-session-status.ts` preserves the last known busy/retry entries and adds only that directory to `unavailableDirectories`. The flag is freshness, not an event gate, so live status events keep updating the index while it is set. Only an authoritative reconcile clears it, inside `applyGlobalSessionStatusSnapshot` in the same state update that applies the snapshot and only for that directory; a monotonic success never clears it, because dropping the flag on a pass that still contradicts preserved busy/retry presents that data as confirmed activity until a second, authoritative fetch settles it. A concurrent authoritative success for one directory cannot make another directory's preserved entries look fresh. While a directory is marked unavailable, the sync requests authoritative snapshots for it (`statusFetchModeForDirectory`), so its next successful fetch clears the flag atomically with fresh data; fresh directories keep the monotonic pass, which confirms or raises busy/retry but can never lower them to idle. `isSessionStatusFresh(sessionId, directory)` is the exported directory-freshness check, and its `directory` argument is required because absence from `statusById` means "last known idle", never "idle now" while the directory is unavailable.

A transport disconnect or transport switch is transport-wide: `markTransportStatusUnavailable` adds every known directory (the `statusById` directories plus the child-store directories the sync provider passes, including idle-only ones), and each directory is freshened only by its own next authoritative snapshot. The reconnect resync is authoritative even for a directory with no active candidates, and the watchdog's bounded status-only recovery fetch keeps requesting authoritative snapshots for a flagged empty directory on the normal poll cadence. Freshness keys are normalized like the status entries.

A runtime replacement is a different boundary. `resetAppForRuntimeEndpointChange` calls `resetGlobalSessionStatus({ blockEventUpdates: true })`, destroying the previous runtime's status index, clearing every freshness flag, and blocking `session.status`, `session.idle`, and `session.error` events until the new runtime's first authoritative snapshot re-enables them. `handleEvent` also drops payloads whose captured runtime key is no longer current, so late old-runtime status events cannot re-add confirmed activity to the new runtime.

Presentation and control use separate hooks. `useSessionDisplayStatus(sessionId, directory?)` derives `busy`/`retry` from fresh data, `reconnecting` from preserved busy/retry whose directory is unavailable, and `idle` otherwise, keeping the raw last-known status in `rawStatus`. `useSessionKnownInactive(sessionId, directory)` is the stricter control predicate: true only when the directory is fresh and the session is idle, `error`, or absent from the busy/retry index, false for every unavailable directory. `reconnecting` means the current truth is unknown, not inactive, so control paths such as move-to-worktree fail closed while a directory is unavailable.
...
Streaming lifecycle derivation has two paths. Directory attach, switch, bootstrap, and reconnect may perform a full reconciliation; a reconnect or transport-switch resync uses every locally known session (session list, status keys, and message keys) as candidate coverage, so a successful authoritative snapshot that omits a session settles it instead of leaving a stale streaming projection. Normal store publications reconcile only sessions whose `session_status` or `message` bucket changed; part-only events update the affected streaming message heartbeat directly and must not rescan all busy sessions.
...
When an assistant `message.updated` event carries `time.completed` and the store still believes the session busy, sync schedules one deferred status check (`maybePollStatusAfterMessageCompletion`, ~750ms). The status is re-read when the timer fires, so a normal turn whose `session.idle` lands inside that window issues no request at all; only a still-busy session spends a directory status poll, sharing the watchdog's one-in-flight-per-directory guard. The check picks its mode the same way the watchdog does: a directory marked unavailable is fetched authoritatively so its flag clears atomically with the snapshot, while an available directory uses the monotonic pass, which confirms or raises active status and never lowers it and escalates to an authoritative resync only when the snapshot disagrees with a store that still believes the session busy. This narrows the stuck-spinner window after a lost `session.idle` from a watchdog interval to one round-trip; the 5s watchdog poll remains the backstop.
