...
| `global-session-status.ts` | Incremental non-idle session status index reconciled from events and authoritative directory snapshots, plus derived parent activity while a background child is busy/retrying | All known directories in the active runtime |
