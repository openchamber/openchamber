...
The pipeline resolves the wrapped SDK client's protocol on every connection attempt. OpenCode V2 always uses the adapter's `global.event` SSE stream because its stateful mapper owns session text, reasoning, tool, and message translation; legacy clients retain the configured WebSocket/SSE/auto selection. Protocol detection failure is a stream failure and follows normal disconnect and retry handling rather than falling back to a legacy transport.
