...
9. Message arrays use the server's chronological order: `time.created` first and `id` only as a same-millisecond tie-breaker. OpenCode's 48-bit encoded ID clock wraps every `2^36` milliseconds, so message merges, optimistic insertion, and revert boundaries must not treat raw ID order as chronology.
