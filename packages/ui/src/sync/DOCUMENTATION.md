...
The directory store records `session_status_ready` only after an authoritative `/session/status` snapshot. Because that endpoint omits idle sessions, a missing entry means idle only after this marker is true. Before the first authoritative snapshot, the trailing assistant message may be used as a narrowly scoped fallback for a missed live status event; historical message data must not keep a revisited session marked busy after an authoritative empty snapshot.
