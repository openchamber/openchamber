...
| `global-session-status.ts` | Incremental non-idle session status index reconciled from events and authoritative directory snapshots, plus derived parent activity and a reference-stable active-ID membership collection maintained from the same mutations | All known directories in the active runtime |
...
A parent session whose background child is busy/retrying stays logically active even while the parent's own raw status is idle. `global-session-status.ts` learns the parent-child relation from authoritative complete session records (`session.updated`/`session.created` `info.parentID` and the session records handed to the snapshot applier for cold start). A complete record omits `parentID` exactly for root sessions, so an omitted parent detaches any prior relation (clearing the former parent's derived activity); an explicit `parentID: null` detaches the same way, while partial payloads without record `info` never mutate relations. The relation index tracks each session's record recency (`time.updated ?? time.created`) and skips a stale complete `session.updated`/`created` record that is older than the last applied one — a delayed old event cannot move a busy child away from its newer parent; authoritative snapshots reset the baseline to their records' recency, mirroring the directory reducer's freshness rules. Activity is transitive — a busy grandchild keeps the child and every ancestor derived-active — and relation moves/null detach/deletion settle every former ancestor immediately. The derived entry carries a `derived` flag; an authoritative raw busy/retry always replaces a matching synthetic entry (`derived:false`) so child settlement cannot delete a raw-active parent. The parent's raw status in the child stores is never rewritten, and the derived entry disappears as soon as the last active descendant settles. A relation that would form an indirect parent cycle is rejected without mutating the maps, and recomputation skips malformed back-edges, so derived entries can never self-sustain without a raw active descendant. Deletion tombstones the deleted session id: a delayed same-recency relation record cannot reconnect a busy child (or the deleted session) to it — only an authoritative strictly-newer record may re-create the relation. A complete `session.updated` record with `info.time.archived` is treated as terminal deletion the same way (matching the directory reducer), using the archived record time as the tombstone baseline. A terminal record is stale — and ignored — when it is not strictly newer than the session's tombstone baseline or the newest accepted recency of the session's own records and currently related children referencing it as parent, so a delayed archive/deletion echo cannot clean up a parent that a newer child edge just restored; a strictly-newer terminal record still terminates it. Unversioned `session.status` / `session.idle` / `session.error` updates for a tombstoned session id are ignored (no raw or derived resurrection) until a strictly-newer complete record recreates the session — and a status snapshot cannot re-enable a tombstoned id either. A newer child→parent edge may restore the edge relation (the child derives the parent again) but never clears the parent's tombstone/status block: the parent's own unversioned status stays blocked until the parent itself has a strictly-newer complete record. Authoritative snapshots re-derive the parent from the children present in the same snapshot, so a reconnect never overwrites child-derived parent activity with a false idle. Relation omission pruning runs only when the caller proves the `knownSessions` list is authoritative and complete for the directory (only an explicit full authoritative list fetch/commit proves it — the bootstrap post-load reapply and authoritative-source resyncs; the `live` event-updated source and the `roots-only` failed-children-fetch fallback are NOT completeness proofs, and early bootstrap/partial lists never prune): a complete list omission prunes every relation owned by that directory (ownership is tracked from the complete session records independently of live status, so busy and idle children both prune) — a delayed busy status cannot recreate a removed relation, while other directories' relations and unavailable/partial lists are untouched. Ordering and turn-timing stay driven by raw status events only.
