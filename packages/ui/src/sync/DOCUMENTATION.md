...
- agent/session activity surfaces using `useGlobalSessionStatus()` / `useAllSessionStatuses()`
  ...
  Cross-directory selectors subscribe to the narrow child-store field they aggregate. Session aggregation listens to `state.session`. Live busy/retry state is also maintained in `global-session-status.ts`, where each row subscribes to one session ID instead of scanning every child store. Events update the index incrementally; authoritative per-directory status snapshots seed it, clear sessions omitted as idle, and reconcile missed events. Runtime/connection boundaries block old status events until a new complete snapshot arrives. Tray status polls capture the active runtime generation and recheck it immediately before either unavailable cleanup or snapshot application, so an old completion cannot populate the new global status store or re-enable event updates. A failed status fetch is not an empty snapshot: a transient HTTP failure, 502, network/relay interruption, or temporary SSE disconnect preserves the last known busy/retry status data in `statusById` (it does not delete entries, lower them to idle, or clear session ordering as if idle) and marks only that directory unavailable via `markDirectoryStatusUnavailable(directory)`, which adds the directory to the `unavailableDirectories` set. Freshness is **directory-scoped, not a global boolean**: a failed fetch for `/repo-a` marks only `/repo-a` unavailable, so a concurrent successful snapshot for `/repo-b` cannot freshen `/repo-a`. A transport-wide disconnect or transport switch uses `markTransportStatusUnavailable()` to add every directory with a `statusById` entry (plus any explicitly passed known directories) to `unavailableDirectories`. Each directory becomes fresh again only after its own next successful authoritative snapshot: `applyGlobalSessionStatusSnapshot` clears the snapshot's own directory from `unavailableDirectories` and re-enables event updates, but never another directory's flag, so completion order does not matter and each directory recovers freshness independently. `statusById` stores only busy/retry entries, so absence means "no active busy/retry entry" (idle), not "definitely idle right now while the directory is unavailable"; control paths that need confirmed inactivity must additionally know the directory's freshness via `isSessionStatusFresh(sessionId, directory)`. Presentation consumers read `useSessionDisplayStatus(sessionId, directory?)`, which combines raw status with directory freshness: `unavailable + last known busy/retry → 'reconnecting'` (the preserved data stays available as `rawStatus`), `fresh + busy/retry → 'busy'/'retry'`, and `fresh + no entry → 'idle'`. Control consumers that require the session to be definitely inactive use `useSessionKnownInactive(sessionId, directory)`, which returns `true` only when fresh + idle and fails closed (`false`) while the directory is unavailable — `unavailable + no status entry` is unknown, not confirmed idle. A real OpenCode runtime replacement (runtime key change) is distinct: `resetGlobalSessionStatus({ blockEventUpdates: true })` destroys stale data and blocks old events so activity from the old runtime cannot be presented as current (issue #2421). Unrelated streaming events such as `message.part.delta` must not trigger global session/status scans.
...
Streaming lifecycle derivation has two paths. Directory attach, switch, bootstrap, and reconnect may perform a full reconciliation after the authoritative session list is loaded. Normal store publications reconcile only sessions whose `session_status` or `message` bucket changed; part-only events update the affected streaming message heartbeat directly and must not rescan all busy sessions. If OpenCode is unavailable, live status/stream projections are cleared without deleting durable messages or session history; a later successful snapshot repopulates them.
