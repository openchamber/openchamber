...
| `question-answer-draft.ts` | Submitted interview answers and request-scoped pending state until question reply acknowledgement | One runtime, session, and question request |
...
11. Interview answers and message-history actions keep their visible state until the SDK request resolves. Interview submissions retain their answer snapshot across session switches. Revert updates the session marker and composer only after acknowledgement; fork switches sessions only after acknowledgement.
