...
Status polling is owned per directory, and the owner is whether a child store exists. A directory with an initialized child store belongs to `sync-context.tsx`: bootstrap seeds its status, the event stream feeds both the child store and the global index, and the active-session watchdog polls and escalates its active candidates. The desktop tray (`hooks/tray-status-poll.ts`) therefore polls only the tray-visible directories that have **no** child store, which is exactly the set nothing else covers. It previously polled every visible directory on the same 5s cadence, so each open project paid for the same `/session/status` request twice and the server answered with sustained file and Git reads. When the sync provider has not mounted, no directory is claimed and the tray polls all of them, as before.
