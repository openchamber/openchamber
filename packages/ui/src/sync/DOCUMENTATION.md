...
An implicit draft opened from an unregistered live directory remains a managed Chat. Delayed stale-directory recovery only repairs a project draft, so it cannot replace that Chat target or its remembered project target.
