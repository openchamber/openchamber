...
| `work-status/useSubagentCostRollup.ts` | Recursive active subtree cost aggregate (session + descendants via `parentID`) from OpenCode's persisted `Session.cost` values | Shared by cost readouts and subagent rows |
