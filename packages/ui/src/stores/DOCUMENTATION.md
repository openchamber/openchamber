...
`synchronizeFromSettings` adopts the persisted project list and active project, but an open session owns the working directory: while `currentSessionDirectory` is set, an active-project change from another window does not redirect `opencodeClient` or `useDirectoryStore`. When a project list change later makes the current session/current directory resolvable to its owning project, `useConfigStore` re-runs `activateDirectory` even though the directory key did not change, so providers/agents load and the resolved default model/agent apply. It skips that call when the directory is already active with providers, agents, and fresh runtime timestamps.
