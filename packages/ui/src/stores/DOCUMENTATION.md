...
Queue entries are `local`, `pending-admission`, `admitted`, `admission-failed`, or `admission-unknown`. Only local entries may be sent, removed, or reordered by the client. Admission-pending entries are submitted to the server's durable queue; admitted entries and their attachment metadata are server-owned history and cannot be locally resent or deleted. An interrupted pending admission becomes unknown after hydration. The user may dismiss it or recover its original input, and either action removes the empty target key. Hard deletion removes the queue, in-flight markers, and durable tombstones for the exact runtime/directory/session identity. Queue target and per-state message limits are independent, and target eviction never drops the target currently being written.
