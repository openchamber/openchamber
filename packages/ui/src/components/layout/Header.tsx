...
const getActiveContextMode = (
	panelState:
		...
		| undefined,
): "diff" | "file" | "context" | "plan" | "chat" | null => {
	if (
		!panelState?.isOpen ||
...
export const Header: React.FC<HeaderProps> = ({
	...
	const isContextUsageResolvedForSession =
		!currentSessionId || currentSessionMessages !== undefined;
	...
	const githubAvatarUrl = githubAuthStatus?.connected
		? githubAuthStatus.user?.avatarUrl
		: null;
	const githubLogin = githubAuthStatus?.connected
		? githubAuthStatus.user?.login
		: null;
	...
	React.useEffect(() => {
		...
		const runOnce = async () => {
			...
			const homeDir = resolveTilde(
				buildHomePlansDirectory(),
				homeDirectory || null,
			);
	...
	const desktopHeaderIconButtonClass =
	...
	const tabs: TabConfig[] = React.useMemo(() => {
		if (isMobile) {
			const base: TabConfig[] = [
				{ id: "chat", label: t("nav.chat"), icon: RiChat4Line },
			];

			if (showPlanTab) {
				base.push({ id: "plan", label: t("nav.plan"), icon: RiFileTextLine });
			}

			base.push(
				{ id: "diff", label: t("nav.diff"), icon: "diff" },
				{ id: "files", label: t("nav.files"), icon: RiFolder6Line },
				{ id: "terminal", label: t("nav.terminal"), icon: RiTerminalBoxLine },
			);
	...
	const servicesTabs = React.useMemo(() => {
		...
		base.push(
			{ value: "usage", label: "Usage", icon: RiTimerLine },
			{
				value: "mcp",
				label: "MCP",
				icon: McpIcon as unknown as RemixiconComponentType,
			},
		);
		return base;
	}, [isDesktopApp]);
		...
		return [
			{
				id: "usage",
				label: "Usage",
				icon: <RiTimerLine className="h-3.5 w-3.5" />,
			},
			{
				id: "mcp",
				label: "MCP",
				icon: <RiCommandLine className="h-3.5 w-3.5" />,
			},
		];
	}, []);
						...
						<TooltipContent>
							<p>
								{isDesktopApp
									...
									: "Services"}{" "}
								({shortcutLabel("toggle_services_menu")}; next tab{" "}
								{shortcutLabel("cycle_services_tab")})
							</p>
						</TooltipContent>
					</Tooltip>
					<DropdownMenuContent
					...
					>
						...
						{isDesktopApp && desktopServicesTab === "instance" && (
							<DesktopHostSwitcherDialog
								embedded
								open={
									isDesktopServicesOpen && desktopServicesTab === "instance"
								}
								onOpenChange={() => {}}
						...
						{desktopServicesTab === "mcp" && (
							<McpDropdownContent
								active={isDesktopServicesOpen && desktopServicesTab === "mcp"}
							/>
						)}
						...
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<button
									...
									title={
										githubLogin ? `GitHub: ${githubLogin}` : "GitHub connected"
									}
								...
								>
									{githubAvatarUrl ? (
										<img
											src={githubAvatarUrl}
											alt={
												githubLogin ? `${githubLogin} avatar` : "GitHub avatar"
											}
							...
							<div className="flex w-max items-center gap-1 pr-1">
								...
								>
									{tabs.map((tab) => {
										const isActive = activeMainTab === tab.id;
										const isDiffTab = tab.icon === "diff";
										const Icon = isDiffTab
											? null
											: (tab.icon as RemixiconComponentType);
										return (
