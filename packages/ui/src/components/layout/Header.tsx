...
export const Header: React.FC<HeaderProps> = ({
  ...
  const [currentInstanceLabel, setCurrentInstanceLabel] = React.useState(t('header.local'));
  ...
  const refreshCurrentInstanceLabel = React.useCallback(async () => {
    ...
    try {
      ...
      if (locationMatchesHost(currentHref, localOrigin)) {
        setCurrentInstanceLabel(t('header.local'));
        return;
      }
      ...
      setCurrentInstanceLabel(t('header.instance'));
    } catch {
      setCurrentInstanceLabel(t('header.local'));
    }
  }, [isDesktopApp, t]);
  ...
  const currentSessionTitle = React.useMemo(() => {
    ...
    return trimmedTitle && trimmedTitle.length > 0 ? trimmedTitle : t('header.untitledSession');
  ...
  const currentSessionPrLabel = React.useMemo(() => {
    ...
    switch (currentSessionPr.visualState) {
      case 'merged':
        return `PR #${currentSessionPr.number} ${t('header.merged')}`;
      case 'closed':
        return `PR #${currentSessionPr.number} ${t('header.closed')}`;
      case 'draft':
        return `PR #${currentSessionPr.number} ${t('header.draft')}`;
      case 'blocked':
        return `PR #${currentSessionPr.number} ${t('header.blocked')}`;
      case 'open':
        if (currentSessionPr.canMerge === true || currentSessionPr.checksState === 'success') {
          return `PR #${currentSessionPr.number} ${t('header.ready')}`;
        }
        return `PR #${currentSessionPr.number} ${t('header.open')}`;
      default:
        return null;
    }
  }, [currentSessionPr, t]);
  ...
  const servicesTabs = React.useMemo(() => {
    ...
    base.push(
      { value: 'usage', label: t('settings.pages.usage'), icon: RiTimerLine },
  ...
  const mobileServicesTabItems = React.useMemo<SortableTabsStripItem[]>(() => {
    return [
      { id: 'usage', label: t('settings.pages.usage'), icon: <RiTimerLine className="h-3.5 w-3.5" /> },
    ...
    <>
      {showPlanTab && (
        <Tooltip delayDuration={500}>
          ...
          <TooltipContent>
            <p>{`${t('header.tabs.plan')} (${shortcutLabel('toggle_context_plan')})`}</p>
          </TooltipContent>
        </Tooltip>
      )}
      ...
      >
        <Tooltip delayDuration={500}>
            ...
            <DropdownMenuTrigger asChild>
              <button
                ...
                aria-label={isDesktopApp
                  ? `${t('header.services')} / ${t('header.instance')} / MCP (${currentInstanceLabel})`
                  : `${t('header.services')} / ${t('settings.pages.usage')} / MCP`}
                className={cn(
                  desktopHeaderIconButtonClass,
          ...
          <TooltipContent>
            <p>
              {isDesktopApp
                ? `${t('header.currentInstance', { label: currentInstanceLabel })} (${shortcutLabel('toggle_services_menu')}; ${t('header.nextTab', { shortcut: shortcutLabel('cycle_services_tab') })})`
                : t('header.services')}
            </p>
          </TooltipContent>
        </Tooltip>
      ...
      <Tooltip delayDuration={500}>
        ...
        <TooltipContent>
          <p>{t('header.terminalPanel', { shortcut: shortcutLabel('toggle_terminal') })}</p>
        </TooltipContent>
      </Tooltip>
      <Tooltip delayDuration={500}>
        <TooltipTrigger asChild>
        ...
        <TooltipContent>
          <p>{t('header.rightSidebar', { shortcut: shortcutLabel('toggle_right_sidebar') })}</p>
        </TooltipContent>
      </Tooltip>
      ...
      <div className={cn('flex min-w-0 flex-1 items-center', !isSidebarOpen && 'pl-3')}>
        {!isLeftSidebarOpen ? (
          <Tooltip delayDuration={500}>
            ...
            <TooltipContent>
              <p>{`${t('header.newSession')} (${shortcutLabel('new_chat')})`}</p>
            </TooltipContent>
          </Tooltip>
        ) : null}
