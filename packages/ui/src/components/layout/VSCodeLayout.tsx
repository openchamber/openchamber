...
export const VSCodeLayout: React.FC = () => {
  ...
  const activeSessionTitle = React.useMemo(() => {
    ...
    return sessions.find((session) => session.id === currentSessionId)?.title || m.lySession();
    ...
    <div ref={containerRef} className="h-full w-full bg-background text-foreground flex flex-col">
      {viewMode === 'editor' ? (
        ...
        <div className="flex flex-col h-full">
          <VSCodeHeader
            title={sessions.find((session) => session.id === currentSessionId)?.title || m.lyChat()}
            showMcp
            showContextUsage
          />
        ...
        <div className="flex h-full">
          ...
          >
            ...
            <div
              ...
              aria-label={m.lyResizeSessionsSidebar()}
            />
          </div>
          {/* Chat content */}
          <div className="flex-1 flex flex-col min-w-0">
            <VSCodeHeader
              title={newSessionDraftOpen && !currentSessionId
                ? m.lyNewSession()
                : sessions.find((session) => session.id === currentSessionId)?.title || m.lyChat()}
              showMcp
              showContextUsage
            />
            <div className="flex-1 overflow-hidden">
    ...
    <div className="flex items-center gap-1.5 pl-3 pr-2 py-1 border-b border-border bg-background shrink-0">
      ...
      {onAgentManager && (
        <button
          ...
          className="inline-flex h-9 w-9 items-center justify-center p-2 text-muted-foreground hover:text-foreground transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          aria-label={m.lyOpenAgentManager()}
        >
        ...
        >
          ...
          >
                ...
                <div className="flex items-center gap-1">
                  <div className="flex items-center rounded-md border border-[var(--interactive-border)] p-0.5">
                    ...
                    <button
                      ...
                      aria-label={m.lyShowRemainingQuota()}
                    >
                      {m.lyRemaining()}
                    </button>
                  </div>
                  <button
                    ...
                    disabled={isQuotaLoading}
                    aria-label={m.lyRefreshRateLimits()}
                  >
            ...
            <div className="border-b border-[var(--interactive-border)] px-2 pb-2 typography-micro text-muted-foreground text-[10px]">
              {m.lyLastUpdated().replace('{time}', formatTime(quotaLastUpdated))}
            </div>
            {!hasRateLimits && (
              <DropdownMenuItem className="cursor-default" onSelect={(event) => event.preventDefault()}>
                <span className="typography-ui-label text-muted-foreground">{m.lyNoRateLimitsAvailable()}</span>
              </DropdownMenuItem>
            )}
                 ...
                 {group.entries.length === 0 ? (
                  ...
                  >
                    <span className="typography-ui-label text-muted-foreground">
                      {group.error ?? m.lyNoRateLimitsReported()}
                    </span>
                  </DropdownMenuItem>
                ) : (
