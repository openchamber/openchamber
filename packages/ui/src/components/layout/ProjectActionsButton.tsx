...
export const ProjectActionsButton = ({
    ...
    for (const { directory: tabDirectory, state } of watchedTerminalStates) {
      ...
      for (const tab of state?.tabs ?? []) {
        ...
        const action = displayActions.find((entry) => entry.id === actionId)
          ?? (actionId === AUTO_DISCOVER_ACTION_ID ? autoDiscoverAction : undefined);
  ...
  }, [autoDiscoverAction, displayActions, setTabIconKey, setTabLabel, watchedTerminalStates]);
  ...
  }, [executionDirectoryFor, projectActionRuns, resolvedSelected, runAction, stopAction]);
  ...
  const previewRun = resolvedSelected
    ? projectActionRuns[toProjectActionRunKey(executionDirectoryFor(resolvedSelected), resolvedSelected.id)]
    : null;
  ...
  const selectedIconName = resolvedSelected ? resolveProjectActionIconName(resolvedSelected) : 'play';
  const selectedRunKey = resolvedSelected
