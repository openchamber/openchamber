...
import { clampContextEditorTreeWidth, useUIStore, type ContextPanelMode, type PendingDiffScope, type SavedServerBrowserSelection } from '@/stores/useUIStore';
  ...
  return (
    ...
    >
      ...
      <div className={cn('relative min-h-0 flex-1 overflow-hidden', isResizing && 'pointer-events-none')}>
        ...
        {activeTab?.mode !== 'chat' && !isFileTabActive && activeTab?.mode !== 'browser' && activeTab?.mode !== 'server-browser' && activeTab?.mode !== 'diff' && activeTab?.mode !== 'terminal' && activeTab?.mode !== 'walkthrough' && !(activeTab && isPluginContextPanelMode(activeTab.mode)) ? activeNonChatContent : null}
      </div>
      </div>
    </aside>
  );
