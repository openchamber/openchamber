...
import { nodeContainsSessionId, nodeHasPinnedMembershipChange, selectQuestionScopes } from './sessionNodeItemUtils';
...
function SessionNodeItemComponent(props: Props): React.ReactNode {
  ...
  // Collapsed rows stand in for the descendants they hide; expanded rows leave
  // each child's count to the child row so a total is never counted twice.
  const questionScopes = React.useMemo(
    () => selectQuestionScopes(node, isExpanded, sessionDirectory),
    [isExpanded, node, sessionDirectory],
  );
  const pendingQuestionCount = useSessionQuestionCount(questionScopes);
