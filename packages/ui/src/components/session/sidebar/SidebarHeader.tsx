              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    aria-label={t('sessionsSidebar.addProject')}
                  >
                    <RiFolderAddLine className={headerActionIconClass} />
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom" sideOffset={4}><p>{t('sessionsSidebar.addProject')}</p></TooltipContent>
              </Tooltip>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={headerActionButtonClass}
                    aria-label={t('sessionsSidebar.newSession')}
                  >
                    <RiChatNewLine className={headerActionIconClass} />
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom" sideOffset={4}><p>{t('sessionsSidebar.newSession')}</p></TooltipContent>
              </Tooltip>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={headerActionButtonClass}
                    aria-label={t('sessionsSidebar.newMultiRun')}
                    disabled={!canOpenMultiRun}
                  >
                    <ArrowsMerge className={headerActionIconClass} />
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom" sideOffset={4}><p>{t('sessionsSidebar.newMultiRun')}</p></TooltipContent>
              </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={headerActionButtonClass}
                      aria-label={t('sessionsSidebar.projectNotes')}
                      disabled={!activeProjectRefForHeader}
                    >
                      <RiStickyNoteLine className={headerActionIconClass} />
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="bottom" sideOffset={4}><p>{t('sessionsSidebar.projectNotes')}</p></TooltipContent>
                </Tooltip>
              ) : (
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    ...
                    className={headerActionButtonClass}
                     aria-label={t('sessionsSidebar.searchSessions')}
                    aria-expanded={isSessionSearchOpen}
                  >
                    <RiSearchLine className={headerActionIconClass} />
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom" sideOffset={4}><p>{t('sessionsSidebar.searchSessions')}</p></TooltipContent>
              </Tooltip>
              ...
              <DropdownMenu>
                  ...
                  <TooltipTrigger asChild>
                    <DropdownMenuTrigger asChild>
                      <button
                        type="button"
                        className={headerActionButtonClass}
                         aria-label={t('sessionsSidebar.displayMode')}
                      >
                        <RiEqualizer2Line className={headerActionIconClass} />
                      </button>
                    </DropdownMenuTrigger>
                  </TooltipTrigger>
                  <TooltipContent side="bottom" sideOffset={4}><p>{t('sessionsSidebar.displayMode')}</p></TooltipContent>
                </Tooltip>
                <DropdownMenuContent align="end" className="min-w-[160px]">
                  ...
                  >
                    <span>{t('sessionsSidebar.defaultDisplay')}</span>
          ...
          {isSessionSearchOpen ? (
            <div className="pb-1">
              <div className="mb-1 flex items-center justify-between px-0.5 typography-micro text-muted-foreground/80">
                {hasSessionSearchQuery ? (
                  <span>{t('sessionsSidebar.matchCount', { count: searchMatchCount })}</span>
                ) : <span />}
                <span>{t('sessionsSidebar.escToClear')}</span>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={t('sessionsSidebar.searchSessionsPlaceholder')}
                ...
                {sessionSearchQuery.length > 0 ? (
                  <button
                    ...
                    className="absolute right-1 top-1/2 inline-flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md text-muted-foreground hover:bg-interactive-hover/60 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
                     aria-label={t('sessionsSidebar.clearSearch')}
                  >
