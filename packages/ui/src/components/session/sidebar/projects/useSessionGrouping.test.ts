...
import { shouldHardDeleteSession } from '../sessions/sessionNodeItemUtils';
import { createSessionDirectoryResolver } from '../sessions/sessionOwnership';
import { indexSessionsByParent, subtreeHasLiveSession } from './useSessionGrouping';

const makeSession = (id: string, options: { parentID?: string; archived?: boolean } = {}): Session => {
  const time: Session['time'] = { created: 1, updated: 1 };
  if (options.archived) time.archived = 2;
  return {
    id,
    slug: id,
    projectID: 'project',
...
describe('resolveSessionDirectoryFromLineage', () => {
  test('inherits the archived parent directory for a newly spawned active child', () => {
    const parent = { ...makeSession('parent', { archived: true }), directory: '/project/worktree' };
    const child = makeSession('child', { parentID: 'parent' });
    Reflect.deleteProperty(child, 'directory');

    const resolveDirectory = createSessionDirectoryResolver(
      new Map([[parent.id, parent], [child.id, child]]),
    );
    expect(resolveDirectory(child)).toBe('/project/worktree');
  });
});
