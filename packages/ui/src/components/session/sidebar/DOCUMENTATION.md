...
- `SessionNodeItem.tsx`: Renders one session row/tree node with a single-line layout, inline branch label, indicators, menu actions, and nested children. Pending-question counts stay per-session while expanded and roll up the descendants collapse hides, grouped by the directory store that owns each session, so a cross-worktree descendant still reports and no total is counted twice. Rows do not initiate directory bootstrap on mount.
