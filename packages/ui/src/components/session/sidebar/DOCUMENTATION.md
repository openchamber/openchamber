...
- Pending approval badges follow rendered tree ownership: each visible row keeps its own requests and rolls up only descendant branches hidden by collapse or search. Visible child branches own their counts, so every request remains discoverable without duplicate ancestor totals. Metadata-only session updates do not wake row permission subscriptions.
