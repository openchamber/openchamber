...
- **Activity is a spinner plus a counter; the static dot means "finished, unread".** The row's left gutter spins (`loader-4`, `animate-spin`, primary) while the session runs (`busy`/`retry`) and shows a static info dot once the result is waiting to be read. The two states differ in shape, not colour alone, because the counter beside them cannot carry that distinction: it renders in both states, it is absent when no turn start was recorded, and it fades while the row is hovered. The metadata slot on the right swaps the goal/branch/date group for the elapsed time of the turn (`SessionActivityDuration`, ticking once per second) and takes the marker's colour — primary while running, info once it is waiting to be read — so the pair reads as one indicator. The spinner animates `transform` only, so it stays on the compositor; that cost is accepted deliberately as the primary signal of a running turn. The counter follows the unread marker's lifetime exactly: it survives the turn ending, disappears when the session is read, and never lingers on the session being watched (which is marked read as it goes idle). Aggregate indicators for collapsed groups, folders, and projects spin the same way but carry no counter — a group may hold several running turns, so a single counter would have nothing to count. The same treatment applies to the mobile sessions sheet and session switcher rows. The worktree-move indicator is also a spinner: it marks a short user-initiated operation, not a session state.
