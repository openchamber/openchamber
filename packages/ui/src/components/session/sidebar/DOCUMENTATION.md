...
- Project headers are sticky background "zone" bands (`SortableProjectItem`); the `recent` section header uses the same band styling. Zone bands expose pinned state as `data-oc-stuck` (projects via `useStickyProjectHeaders`, `recent` via its own top sentinel) so the macOS vibrancy backing paints only while a band masks rows scrolling under it; collapsed zones never take the backing. Collapsed projects show an aggregated busy/unseen indicator (`ProjectAggregateStatusIndicator`), derived from the live status index and notification store scoped to the project's directories.
