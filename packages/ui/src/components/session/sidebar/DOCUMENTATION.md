...
- Archived group membership derives **only** from the authoritative `time.archived` field (`resolveSessionGroupKey`). A live session whose directory matches neither the project root nor a registered worktree — a repo subdirectory, or a worktree that was since removed — falls back to the project root group. It must never fall through to the archived bucket: that bucket is unrendered outside VS Code and is bulk deletable, so misrouting hides a live session and then destroys it. The archived bulk delete independently re-checks `time.archived` rather than trusting group membership.
- Every hard-delete UI entry point routes through `SessionDialogs`, which loads complete active and archived pages from the server before confirming instead of trusting rendered or cached sessions. Archive-page and archived-bucket requests additionally require every selected root to remain authoritatively archived, so stale UI classification cannot delete a live root. Desktop and mobile worktree removal use the same authoritative cascade before archiving linked sessions. The cascade is loaded again on confirmation; a changed descendant set updates the dialog and requires another confirmation, while unrelated sessions do not invalidate it. Confirmed descendants execute before their parents using their authoritative directories. A failed descendant blocks its ancestors without aborting unrelated roots, and the result reports every failed or blocked session.
- `requiresDeleteConfirmation` is the single rule for the `showDeletionDialog` opt-out: "never ask" and shift-click quick delete suppress confirmation only when the authoritative cascade contains one session. A server-discovered descendant always forces disclosure and confirmation. Archiving remains non-destructive and keeps the plain opt-out.
