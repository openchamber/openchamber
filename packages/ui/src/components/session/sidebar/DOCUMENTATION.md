...
- Root session right-click and overflow menus expose `Move to worktree`: a submenu listing existing primary and linked worktree destinations, the current target greyed and disabled, plus a `New worktree...` action. The submenu force-refreshes the owning project's worktree topology on open so externally created worktrees appear without a full reload. Moving to an existing or new destination transfers the full idle subtree; only the root session carries uncommitted changes.
