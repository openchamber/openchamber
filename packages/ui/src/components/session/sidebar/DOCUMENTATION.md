...
- Project sort modes `recent` and `date-added` promote projects that own an authoritative non-idle session above projects without one, on the desktop sidebar and the mobile sessions sheet alike. `recent` ranks by `max(lastOpenedAt, latest session lifecycle value)`; `manual`, `a-z`, and `z-a` are unchanged.
