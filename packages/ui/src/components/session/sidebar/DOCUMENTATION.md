...
- Parent expansion separates persisted manual choices from the current session's bounded auto-expanded parent path. Navigation replaces the auto path instead of persisting historical paths; the manual storage key is `v3` because older state mixed both sources and cannot be migrated safely. Auto expansion commits on the next animation frame with cancellation so child-session visibility does not add a second sidebar commit to the session-selection event. A manual collapse suppresses auto expansion for that render-context key until the selected parent changes, and `recent` invalidates its memoized top content when expansion state changes.
- Sidebar selection holds the clicked row's viewport position across the navigation and deferred auto-expansion frames. Wheel or touch input cancels the hold immediately, so programmatic compensation never fights intentional scrolling.
