...
import {
  filterArchivedFolderSessions,
  getStaleArchivedFolderSessionIds,
} from './useArchivedAutoFolders';
...
describe('filterArchivedFolderSessions', () => {
  ...
  test('keeps one deterministic root for a malformed archived cycle', () => {
    const sessions = [
      makeSession('first', { parentID: 'second' }),
      makeSession('second', { parentID: 'first' }),
    ];

    expect(ids(filterArchivedFolderSessions(sessions))).toEqual(['first']);
  });
});

describe('getStaleArchivedFolderSessionIds', () => {
  test('removes persisted descendants and sessions that are no longer archived', () => {
    const roots = [makeSession('parent'), makeSession('independently-archived-child', { parentID: 'active-parent' })];
    const folders = [
      { id: 'one', name: 'Project', sessionIds: ['parent', 'nested-child', 'active-child'] },
      { id: 'two', name: 'Worktree', sessionIds: ['nested-child', 'independently-archived-child'] },
    ];

    expect(getStaleArchivedFolderSessionIds(roots, folders)).toEqual(['nested-child', 'active-child']);
  });
});
