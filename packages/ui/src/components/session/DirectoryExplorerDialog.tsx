...
export const DirectoryExplorerDialog: React.FC<DirectoryExplorerDialogProps> = ({
    ...
    try {
      ...
      if (isDesktop) {
        ...
        if (!accessResult.success) {
          toast.error(t('directoryExplorerDialog.unableToAccessDirectory'), {
            description: accessResult.error || t('directoryExplorerDialog.desktopDeniedDirectoryAccess'),
          });
          return;
        }
        ...
        if (!startResult.success) {
          toast.error(t('sessionDialogs.failedToOpenDirectory'), {
            description: startResult.error || t('sessionDialogs.desktopCouldNotGrantFileAccess'),
          });
          return;
        }
      }
      ...
      toast.error(t('projectsSidebar.failedToSelectDirectory'), {
        description: error instanceof Error ? error.message : t('directoryExplorerDialog.unknownErrorOccurred'),
      });
    } finally {
      setIsConfirming(false);
    }
  ...
  const showHiddenToggle = (
    ...
    >
      ...
      {t('directoryExplorer.showHidden')}
    </button>
  );

  const dialogHeader = (
    <DialogHeader className="flex-shrink-0 px-4 pb-2 pt-[calc(var(--oc-safe-area-top,0px)+0.5rem)] sm:px-0 sm:pb-3 sm:pt-0">
      <DialogTitle>{t('directoryExplorer.addProjectDirectory')}</DialogTitle>
      <div className="hidden sm:flex sm:items-center sm:justify-between sm:gap-4">
        <DialogDescription className="flex-1">
          {t('directoryExplorer.chooseFolderToAddProject')}
        </DialogDescription>
        {showHiddenToggle}
      </div>
    </DialogHeader>
  ...
  const pathInputSection = (
    <div className="relative">
      <Input
        ...
        placeholder={t('directoryExplorer.enterPathOrSelect')}
  ...
  const renderActionButtons = () => (
    <>
      ...
      >
        {isConfirming ? t('directoryExplorer.adding') : t('navigation.addProject')}
      </Button>
    </>
  );
  ...
  if (isMobile) {
    return (
      <MobileOverlayPanel
        ...
        title={t('directoryExplorer.addProjectDirectory')}
