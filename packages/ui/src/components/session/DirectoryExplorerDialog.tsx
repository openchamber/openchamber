...
export const DirectoryExplorerDialog: React.FC<DirectoryExplorerDialogProps> = ({
  ...
  const renderPinButton = (path: string) => {
    ...
    return (
      ...
      >
        {isPinned ? <Icon name="pushpin-2-fill" className="h-4 w-4 text-primary" /> : <Icon name="pushpin-2" className="h-4 w-4" />}
      </button>
    );
  };
      ...
      <div className="max-h-[min(28rem,58vh)] overflow-y-auto p-2">
        {pinnedDirectories.length > 0 ? (
          <div className="mb-2">
            ...
            >
              {isPinnedSectionExpanded ? <Icon name="arrow-down-s" className="h-3.5 w-3.5" /> : <Icon name="arrow-right-s" className="h-3.5 w-3.5" />}
