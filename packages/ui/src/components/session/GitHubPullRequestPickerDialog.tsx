...
export function GitHubPullRequestPickerDialog({
  ...
  const refresh = React.useCallback(async () => {
    if (!projectDirectory) {
      setResult(null);
      setError(t('githubPullRequestPickerDialog.noActiveProject'));
      return;
    }
    ...
    if (!github?.prsList) {
      setResult(null);
      setError(t('githubPullRequestPickerDialog.runtimeApiUnavailable'));
      return;
    }
  ...
  const loadMore = React.useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('githubPullRequestPickerDialog.failedToLoadMorePrs'), { description: message });
    } finally {
    ...
    const results = await Promise.all(
      unknown.map(async (head) => {
        const validation = await validateWorktreeCreate(projectRef, {
          ...
          branchName: head,
          worktreeName: head,
        }).catch(() => ({ ok: false, errors: [{ code: 'validation_failed', message: t('githubPullRequestPickerDialog.validationFailed') }] }));
  ...
  }, [projectRef, existingBranchHeads, t]);
  ...
  const startSession = React.useCallback(async (number: number) => {
    if (!projectDirectory) {
      toast.error(t('githubPullRequestPickerDialog.noActiveProject'));
      return;
    }
    if (!github?.prContext) {
      toast.error(t('githubPullRequestPickerDialog.runtimeApiUnavailable'));
      return;
    }
    ...
    try {
      ...
      if (prContext.connected === false) {
        toast.error(t('githubPullRequestPickerDialog.notConnected'));
        return;
      }
      if (!prContext.repo) {
        toast.error(t('githubPullRequestPickerDialog.repoNotResolvable'), {
          description: t('githubPullRequestPickerDialog.originRemoteMustBeGithubUrl'),
        });
        return;
      }
      if (!prContext.pr) {
        toast.error(t('githubPullRequestPickerDialog.prNotFound'));
        return;
      }
...
- None`;
      ...
      void opencodeClient.sendMessage({
        ...
        const message = e instanceof Error ? e.message : String(e);
        toast.error(t('githubPullRequestPickerDialog.failedToSendPrContext'), { description: message });
      });
      ...
      toast.error(
        createInWorktree
          ? t('githubPullRequestPickerDialog.prWorktreeFailed')
          : t('githubPullRequestPickerDialog.failedToStartSession'),
        { description: message }
      );
    } finally {
      setStartingNumber(null);
      ...
      <DialogContent className="max-w-2xl max-h-[70vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('githubPullRequestPicker.newSessionFromPr')}
          </DialogTitle>
          <DialogDescription>
            {t('githubPullRequestPicker.description')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="relative mt-2">
          ...
          <Input
            placeholder={t('githubPullRequestPicker.searchPlaceholder')}
            value={query}
        ...
        <div className="flex-1 overflow-y-auto mt-2">
          {!projectDirectory ? (
            <div className="text-center text-muted-foreground py-8">{t('githubPullRequestPicker.noActiveProject')}</div>
          ) : null}

          {!github ? (
            <div className="text-center text-muted-foreground py-8">{t('githubPullRequestPicker.runtimeUnavailable')}</div>
          ) : null}

          {isLoading ? (
            <div className="text-center text-muted-foreground py-8 flex items-center justify-center gap-2">
              <RiLoader4Line className="h-4 w-4 animate-spin" />
              {t('githubPullRequestPickerDialog.loadingPullRequests')}
            </div>
          ) : null}

          {connected === false ? (
            <div className="text-center text-muted-foreground py-8 space-y-3">
              <div>{t('githubPullRequestPicker.notConnected')}</div>
          ...
          {directNumber && projectDirectory && github && connected ? (
            ...
            >
              ...
              <p className="flex-1 min-w-0 typography-small text-foreground truncate ml-0.5">
                {t('githubPullRequestPickerDialog.usePrNumber', { number: directNumber })}
              </p>
          ...
          {filtered.length === 0 && !isLoading && connected && github && projectDirectory ? (
            <div className="text-center text-muted-foreground py-8">
              {query ? t('githubPullRequestPickerDialog.noPrsFound') : t('githubPullRequestPickerDialog.noOpenPrsFound')}
            </div>
          ) : null}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {createInWorktree && disabledByWorktree ? (
                    <p className="typography-micro text-muted-foreground mt-0.5 ml-0.5">
                      {t('githubPullRequestPickerDialog.prWorktreeDisabled', { head: pr.head })}
                    </p>
                  ) : null}
                </div>
                <div className="flex-shrink-0 h-5 flex items-center mr-2 pt-0.5">
                  {startingNumber === pr.number ? (
                    ...
                    <a
                      ...
                      aria-label={t('githubPullRequestPicker.openInGithub')}
                    >
              ...
              >
                {isLoadingMore ? (
                  <span className="inline-flex items-center gap-2">
                    <RiLoader4Line className="h-4 w-4 animate-spin" />
                    {t('githubPullRequestPickerDialog.loading')}
                  </span>
                ) : (
                  t('githubPullRequestPickerDialog.loadMore')
                )}
              </button>
            </div>
          ) : null}
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:gap-4">
              ...
              >
                <button
                  ...
                  aria-label={t('githubPullRequestPicker.toggleWorktree')}
                ...
                <span className="typography-meta text-muted-foreground">{t('githubPullRequestPicker.createInPrWorktree')}</span>
              </div>
              ...
              >
                <button
                  ...
                  aria-label={t('githubPullRequestPicker.toggleDiff')}
                  className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-muted-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                >
                  {includeDiff ? (
                    <RiCheckboxLine className="h-4 w-4 text-primary" />
                ...
                <span className="typography-meta text-muted-foreground">{t('githubPullRequestPicker.includeFullDiff')}</span>
              </div>
            </div>
