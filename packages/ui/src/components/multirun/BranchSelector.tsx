  ...
  return (
    <div>
      ...
      >
        ...
        >
          <SelectValue placeholder={isLoading ? t('multirun.loadingBranches') : t('newWorktreeDialog.selectSourceBranchPlaceholder')} />
        </SelectTrigger>
        <SelectContent className="max-h-[280px] max-w-[320px]">
          {isLoading ? (
            <div className="px-2 py-4 text-center typography-meta text-muted-foreground">
              {t('multirun.loadingBranches')}
            </div>
          ) : localBranches.length === 0 && remoteBranches.length === 0 ? (
            <div className="px-2 py-4 text-center typography-meta text-muted-foreground">
              {t('multirun.noBranchesFound')}
            </div>
          ) : (
            <>
              {localBranches.length > 0 && (
                <SelectGroup>
                  <SelectLabel className="font-semibold text-foreground">{t('newWorktreeDialog.localBranches')}</SelectLabel>
              ...
              {remoteBranches.length > 0 && (
                <SelectGroup>
                  <SelectLabel className="font-semibold text-foreground">{t('newWorktreeDialog.remoteBranches')}</SelectLabel>
      ...
      {isGitRepository === false && (
        <p className="typography-micro text-muted-foreground/70 mt-2">{t('multirun.notInGitRepository')}</p>
      )}
    </div>
  );
};
