...
const SkillsInstalledPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (!skillName) {
      toast.error(m.skToastNameRequired2());
      return;
    }
    ...
    if (!description.trim()) {
      toast.error(m.skToastDescriptionRequired());
      return;
    }

    if (isNewSkill && skills.some((s) => s.name === skillName)) {
      toast.error(m.skToastDuplicate());
      return;
    }
  ...
  const handleEditFile = async (filePath: string) => {
    ...
    try {
      ...
      toast.error(m.skToastLoadFileFailed());
      setNewFileContent('');
      setOriginalFileContent('');
    } finally {
  ...
  const handleSaveFile = async () => {
    if (!newFileName.trim()) {
      toast.error(m.skToastFileNameRequired());
      return;
    }
    ...
    if (success) {
      ...
      toast.error(isEditing ? m.skToastUpdateFailed() : m.skToastCreateFailed());
    }
  };
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center text-muted-foreground">
          ...
          <p className="typography-body">{m.skSelectFromSidebar()}</p>
          <p className="typography-meta mt-1 opacity-75">{m.skOrCreateNew()}</p>
        </div>
      </div>
    );
  }
  ...
  if (isLoading) {
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center text-muted-foreground">
          <p className="typography-body">{m.skLoadingDetails()}</p>
        </div>
      </div>
    );
  }
    ...
    <ScrollableOverlay keyboardAvoid outerClassName="h-full" className="w-full">
      <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
        ...
        <div className="mb-4">
          <div className="min-w-0">
            <h2 className="typography-ui-header font-semibold text-foreground truncate flex items-center gap-2">
              {isNewSkill ? m.skNewSkill() : selectedSkillName}
              {selectedSkill?.source === 'claude' && (
                <span className="typography-micro font-normal bg-[var(--surface-muted)] text-muted-foreground px-1.5 py-0.5 rounded">
                  {m.skClaudeCompatible()}
                </span>
              )}
            </h2>
            <p className="typography-meta text-muted-foreground truncate">
              {selectedSkill ? `${locationLabel(selectedSkill.scope, selectedSkill.source)} ${m.skEditSkill()}` : m.skNewSkillDesc()}
            </p>
          </div>
        </div>
          ...
          <section className="px-2 pb-2 pt-0 space-y-0">
            ...
            {isNewSkill && (
              <div className="py-1.5">
                <span className="typography-ui-label text-foreground">{m.skSkillNameLocation()}</span>
                <span className="typography-meta text-muted-foreground ml-2">{m.skNameFormat()}</span>
                <div className="flex items-center gap-2 mt-1.5">
                  <Input
                    value={draftName}
                    onChange={(e) => setDraftName(e.target.value.toLowerCase().replace(/\s+/g, '-'))}
                    placeholder={m.skillsNamePlaceholder()}
            ...
            <div className="py-1.5">
              <span className="typography-ui-label text-foreground">{m.skDescription()} <span className="text-[var(--status-error)]">*</span></span>
              <span className="typography-meta text-muted-foreground ml-2">{m.skDescriptionHint()}</span>
              <div className="mt-1.5">
                <Textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder={m.skDescriptionPlaceholder()}
                  rows={2}
        ...
        <div className="mb-8">
          ...
          <section className="px-2 pb-2 pt-0">
            <Textarea
              ...
              placeholder={m.skInstructionsPlaceholder()}
        ...
        <div className="mb-2">
          <div className="mb-1 px-1 flex items-center gap-2">
            ...
            <Button variant="outline" size="xs" className="!font-normal gap-1" onClick={handleAddFile}>
              <RiAddLine className="h-3.5 w-3.5" /> {m.skAddFile()}
            </Button>
          </div>
        ...
        <div className="px-2 py-1">
          ...
          >
            {isSaving ? m.skSaving() : isNewSkill ? m.skCreateSkill() : m.skSaveChanges()}
          </Button>
        </div>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{m.skDeleteFile()}</DialogTitle>
            <DialogDescription>
              {m.skDeleteFileConfirm().replace('{name}', deleteFilePath || '')}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <Dialog open={isFileDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col" keyboardAvoid>
          <DialogHeader className="flex-shrink-0">
            <DialogTitle>{editingFilePath ? m.skEditFile() : m.skAddFileTitle()}</DialogTitle>
            <DialogDescription>
              {editingFilePath ? m.skEditFileDesc() : m.skAddFileDesc()}
            </DialogDescription>
          </DialogHeader>
          {isLoadingFile ? (
            <div className="flex-1 flex items-center justify-center py-8">
              <span className="typography-meta text-muted-foreground">{m.skLoadingFile()}</span>
            </div>
          ) : (
            <div className="space-y-4 flex-1 min-h-0 flex flex-col pt-2">
              <div className="space-y-2 flex-shrink-0">
                ...
                <Input
                  ...
                  placeholder={m.skillsFilePlaceholder()}
              ...
              <div className="space-y-2 flex-1 min-h-0 flex flex-col">
                ...
                <Textarea
                  ...
                  placeholder={m.skillsContentPlaceholder()}
