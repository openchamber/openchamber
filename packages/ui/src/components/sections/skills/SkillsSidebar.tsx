...
export const SkillsSidebar: React.FC<SkillsSidebarProps> = ({ onItemSelect }) => {
  ...
  const handleConfirmDeleteSkill = async () => {
    ...
    if (success) {
      ...
      toast.error(m.skToastDeleteFailed());
    }
  ...
  const handleDuplicateSkill = async (skill: DiscoveredSkill) => {
    ...
    if (!detail) {
      toast.error(m.skToastDuplicationFailed());
      return;
    }
  ...
  const handleRenameSkill = async () => {
    ...
    if (!sanitizedName) {
      toast.error(m.skToastNameRequired());
      return;
    }
    ...
    if (skills.some((s) => s.name === sanitizedName)) {
      toast.error(m.skToastDuplicateName());
      return;
    }
    ...
    if (success) {
      ...
      if (deleteSuccess) {
        toast.success(m.skToastRenamed().replace('{name}', sanitizedName));
        setSelectedSkill(sanitizedName);
      } else {
        toast.error(m.skToastRenameFailed());
      }
    } else {
      toast.error(m.skToastRenameFailed());
    }
  ...
  return (
    <div className={cn('flex h-full flex-col', bgClass)}>
      <div className="border-b px-3 pt-4 pb-3">
        <h2 className="text-base font-semibold text-foreground mb-3">{m.skTitle()}</h2>
        <SettingsProjectSelector className="mb-3" />
        <div className="flex items-center justify-between gap-2">
          <span className="typography-meta text-muted-foreground">{m.skTotal()} {skills.length}</span>
          <Button size="sm"
            variant="ghost"
      ...
      <ScrollableOverlay outerClassName="flex-1 min-h-0" className="space-y-1 px-3 py-2 overflow-x-hidden">
        {skills.length === 0 ? (
          <div className="py-12 px-4 text-center text-muted-foreground">
            ...
            <p className="typography-ui-label font-medium">{m.skNoSkills()}</p>
            <p className="typography-meta mt-1 opacity-75">{m.skNoSkillsHint()}</p>
          </div>
        ) : (
          <>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{m.skDeleteSkill()}</DialogTitle>
            <DialogDescription>
              {m.skDeleteConfirm().replace('{name}', deleteDialogSkill?.name || '')}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <Dialog open={renameDialogSkill !== null} onOpenChange={(open) => !open && setRenameDialogSkill(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{m.skRenameSkill()}</DialogTitle>
            <DialogDescription>
              {m.skRenamePrompt().replace('{name}', renameDialogSkill?.name || '')}
            </DialogDescription>
          </DialogHeader>
          <Input
            value={renameNewName}
