    ...
    <div className="mb-8">
      <div className="mb-1 px-1">
        <div className="flex items-center gap-2">
          <h3 className="typography-ui-header font-medium text-foreground">{t('memoryLimitsSettings.title')}</h3>
          <Tooltip delayDuration={1000}>
            <TooltipTrigger asChild>
              <RiInformationLine className="h-3.5 w-3.5 text-muted-foreground/60 cursor-help" />
            </TooltipTrigger>
            <TooltipContent sideOffset={8} className="max-w-xs">
              {t('memoryLimitsSettings.descriptionLine1')}<br />
              {t('memoryLimitsSettings.descriptionLine2')}
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      <section className="px-2 pb-2 pt-0">
        <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-8">
          <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
            <span className="typography-ui-label text-foreground">{t('memoryLimitsSettings.messageLimit')}</span>
          </div>
          <div className="flex items-center gap-2 sm:w-fit">
            <NumberInput
              ...
              max={MAX_LIMIT}
              step={10}
              aria-label={t('memoryLimitsSettings.messageLimit')}
            ...
            <ButtonSmall
              ...
              aria-label={t('common.resetButton')}
