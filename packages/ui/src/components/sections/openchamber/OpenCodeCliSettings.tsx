...
export const OpenCodeCliSettings: React.FC = () => {
  ...
  const handleSaveAndReload = React.useCallback(async () => {
    ...
    try {
      ...
      await reloadOpenCodeConfiguration({ message: m.ocCliRestarting(), mode: 'projects', scopes: ['all'] });
    } finally {
      setIsSaving(false);
    }
  }, [value]);
        ...
        <div className="flex items-center gap-2">
          ...
          <Tooltip delayDuration={1000}>
            ...
            <TooltipContent sideOffset={8} className="max-w-xs">
              {m.ocCliTooltip()} <code className="font-mono text-xs">opencode</code>.
            </TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      <section className="px-2 pb-2 pt-0 space-y-0.5">
        <div className="flex flex-col gap-2 py-1.5 sm:flex-row sm:items-center sm:gap-3">
          <div className="flex min-w-0 flex-col shrink-0">
            <span className="typography-ui-label text-foreground">{m.ocCliBinaryPath()}</span>
          </div>
          <div className="flex min-w-0 items-center gap-2 sm:w-[20rem]">
            <Input
              value={value}
              onChange={(e) => setValue(e.target.value)}
              placeholder={m.ocCliPlaceholder()}
        ...
        <div className="py-1.5">
          <div className="typography-micro text-muted-foreground/70">
            {m.ocCliTip()} <span className="font-mono">OPENCODE_BINARY</span> env var, but this setting persists in <span className="font-mono">~/.config/openchamber/settings.json</span>.
          </div>
        </div>
        ...
        <div className="flex justify-start py-1.5">
          ...
          >
            {isSaving ? m.ocCliSaving() : m.ocCliSaveReload()}
          </Button>
        </div>
      </section>
    </div>
