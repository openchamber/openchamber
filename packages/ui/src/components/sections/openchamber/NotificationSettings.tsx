...
export const NotificationSettings: React.FC = () => {
  ...
  const defaultNotificationTemplates = React.useMemo(() => ({
    completion: {
      title: t('notificationSettings.defaultCompletionTitle'),
      message: t('notificationSettings.defaultCompletionMessage'),
    },
    error: {
      title: t('notificationSettings.defaultErrorTitle'),
      message: t('notificationSettings.defaultErrorMessage'),
    },
    question: {
      title: t('notificationSettings.defaultQuestionTitle'),
      message: t('notificationSettings.defaultQuestionMessage'),
    },
    subtask: {
      title: t('notificationSettings.defaultSubtaskTitle'),
      message: t('notificationSettings.defaultSubtaskMessage'),
    },
  }), [t]);
    ...
    if (checked && typeof Notification !== 'undefined' && Notification.permission === 'default') {
      try {
        ...
        if (permission === 'granted') {
          ...
          toast.error(t('notificationSettings.permissionDeniedTitle'), {
            description: t('notificationSettings.permissionDeniedBrowser'),
          });
        }
      } catch (error) {
  ...
  const handleEnableBackgroundNotifications = async () => {
    if (!pushSupported) {
      toast.error(t('notificationSettings.pushNotSupportedTitle'));
      return;
    }

    const apis = getRegisteredRuntimeAPIs();
    if (!apis?.push) {
      toast.error(t('notificationSettings.pushApiNotAvailable'));
      return;
    }
    ...
    try {
      ...
      if (!key?.publicKey) {
        toast.error(t('notificationSettings.failedLoadPushKey'));
        return;
      }
      ...
      if (!ok?.ok) {
        toast.error(t('notificationSettings.enableBackgroundFailed'));
        return;
      }

      setPushSubscribed(true);
      toast.success(t('notificationSettings.backgroundEnabled'));
    } catch (error) {
  ...
  const handleDisableBackgroundNotifications = async () => {
    ...
    try {
      ...
      setPushSubscribed(false);
      toast.success(t('notificationSettings.backgroundDisabled'));
    } finally {
      setPushBusy(false);
    }
  };
    ...
    <div className="space-y-8">
        ...
        <div className="mb-8">
          ...
          <section className="px-2 pb-2 pt-0 space-y-0.5">
            ...
            >
              <Checkbox
                ...
                ariaLabel={t('notificationSettings.enableNotifications')}
              />
              <span className="typography-ui-label text-foreground">{t('notificationSettings.enableNotifications')}</span>
            </div>
            ...
            {nativeNotificationsEnabled && canShowNotifications && (
              ...
              >
                <Checkbox
                  ...
                  ariaLabel={t('notificationSettings.notifyWhileFocused')}
                />
                <span className="typography-ui-label text-foreground">{t('notificationSettings.notifyWhileFocused')}</span>
              </div>
            )}
          </section>
            ...
            <div className="mt-1 px-2">
              ...
              {notificationPermission === 'granted' && !nativeNotificationsEnabled && (
                <p className="typography-meta text-muted-foreground/70 mt-1">
                  {t('notificationSettings.permissionGrantedDisabled')}
                </p>
              )}
            </div>
          )}
          {isVSCode && (
            <div className="mt-1 px-2">
              <p className="typography-meta text-muted-foreground/70">
                {t('notificationSettings.vscodeNotification')}
              </p>
            </div>
          )}
        </div>
          ...
          <>
            ...
            <div className="mb-8">
              ...
              <section className="px-2 pb-2 pt-0 space-y-0.5">
                ...
                >
                  <Checkbox checked={notifyOnCompletion} onChange={setNotifyOnCompletion} ariaLabel={t('notificationSettings.agentCompletion')} />
                  <span className="typography-ui-label text-foreground">{t('notificationSettings.agentCompletion')}</span>
                </div>
                ...
                >
                  <Checkbox checked={notifyOnSubtasks} onChange={setNotifyOnSubtasks} ariaLabel={t('notificationSettings.subagentCompletion')} />
                  <span className="typography-ui-label text-foreground">{t('notificationSettings.subagentCompletion')}</span>
                </div>
                ...
                >
                  <Checkbox checked={notifyOnError} onChange={setNotifyOnError} ariaLabel={t('notificationSettings.agentErrors')} />
                  <span className="typography-ui-label text-foreground">{t('notificationSettings.agentErrors')}</span>
                </div>
                ...
                >
                  <Checkbox checked={notifyOnQuestion} onChange={setNotifyOnQuestion} ariaLabel={t('notificationSettings.agentQuestions')} />
                  <span className="typography-ui-label text-foreground">{t('notificationSettings.agentQuestions')}</span>
                </div>
              </section>
            </div>
                ...
                {(['completion', 'subtask', 'error', 'question'] as const).map((event) => (
                  <section key={event} className="p-2">
                    <span className="typography-ui-label text-foreground font-normal capitalize block">
                      {event === 'subtask'
                        ? t('notificationSettings.subagentCompletion')
                        : event === 'completion'
                          ? t('notificationSettings.agentCompletion')
                          : event === 'error'
                            ? t('notificationSettings.agentErrors')
                            : t('notificationSettings.agentQuestions')}
                    </span>
                    <div className="mt-1.5 space-y-2">
                      <div>
                        <label className="typography-micro text-muted-foreground block mb-1">{t('notificationSettings.templateTitle')}</label>
                        <Input
                          value={notificationTemplates[event].title}
                          onChange={(e) => updateTemplate(event, 'title', e.target.value)}
                          className="h-7"
                          placeholder={defaultNotificationTemplates[event].title}
                        />
                      </div>
                      <div>
                        <label className="typography-micro text-muted-foreground block mb-1">{t('notificationSettings.templateMessage')}</label>
                        <Input
                          value={notificationTemplates[event].message}
                          onChange={(e) => updateTemplate(event, 'message', e.target.value)}
                          className="h-7"
                          placeholder={defaultNotificationTemplates[event].message}
                        />
                      </div>
                    </div>
                  </section>
            ...
            <div className="mb-8">
              <div className="mb-1 px-1">
                <h3 className="typography-ui-header font-medium text-foreground">
                  {t('notificationSettings.summarization')}
                </h3>
              </div>
              ...
              <section className="px-2 pb-2 pt-0 space-y-0.5">
                ...
                >
                  <Checkbox
                    ...
                    ariaLabel={t('notificationSettings.summarizeLastMessage')}
                  />
                  <span className="typography-ui-label text-foreground">{t('notificationSettings.summarizeLastMessage')}</span>
                </div>

                <div className={cn("flex flex-col gap-2 py-1 sm:flex-row sm:items-center sm:gap-8")}>
                  <div className="flex min-w-0 flex-col sm:w-56 shrink-0">
                    <div className="flex items-center gap-2">
                      <span className="typography-ui-label text-foreground">{t('notificationSettings.summarizationModel')}</span>
                      <Tooltip delayDuration={1000}>
                        <TooltipTrigger asChild>
                          <RiInformationLine className="h-3.5 w-3.5 text-muted-foreground/60 cursor-help" />
                        </TooltipTrigger>
                        <TooltipContent sideOffset={8} className="max-w-xs">
                          {t('notificationSettings.modelInfo')}
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </div>
                  <div className="flex min-w-0 flex-1 items-center gap-2 sm:w-fit sm:flex-initial">
                    ...
                    >
                      <SelectTrigger className="w-fit min-w-[220px]">
                        <SelectValue placeholder={t('notificationSettings.notSelected')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value={UTILITY_NOT_SELECTED_VALUE}>{t('notificationSettings.notSelected')}</SelectItem>
                ...
                {summarizeLastMessage ? (
                  <>
                    <div className="flex items-center gap-8 py-1.5 mt-1 border-t border-[var(--surface-subtle)]">
                      <div className="flex min-w-0 flex-col w-56 shrink-0">
                        <span className="typography-ui-label text-foreground">{t('notificationSettings.threshold')}</span>
                        <span className="typography-meta text-muted-foreground">{t('notificationSettings.thresholdDesc')}</span>
                      </div>
                      <div className="flex items-center gap-2 w-fit">
                        ...
                        <Button size="sm"
                          ...
                          aria-label={t('notificationSettings.reset')}
                          title={t('notificationSettings.reset')}
                        >
                    ...
                    <div className="flex items-center gap-8 py-1.5">
                      <div className="flex min-w-0 flex-col w-56 shrink-0">
                        <span className="typography-ui-label text-foreground">{t('notificationSettings.length')}</span>
                        <span className="typography-meta text-muted-foreground">{t('notificationSettings.lengthDesc')}</span>
                      </div>
                      <div className="flex items-center gap-2 w-fit">
                        <NumberInput
                          value={summaryLength}
                  ...
                  <div className={cn("py-1.5 mt-1 border-t border-[var(--surface-subtle)]", isMobile ? "flex flex-col gap-3" : "flex items-center gap-8")}>
                    <div className={cn("flex min-w-0 flex-col", isMobile ? "w-full" : "w-56 shrink-0")}>
                      <span className="typography-ui-label text-foreground">{t('notificationSettings.maxLength')}</span>
                      <span className="typography-meta text-muted-foreground">{t('notificationSettings.maxLengthDesc')}</span>
                    </div>
        ...
        {isBrowser && (
          <div className="mb-8">
            <div className="mb-1 px-1">
              <h3 className="typography-ui-header font-medium text-foreground">
                {t('notificationSettings.backgroundPush')}
              </h3>
            </div>
            ...
            <section className="px-2 pb-2 pt-0">
              <div className="flex items-start gap-2 py-1.5">
                <Checkbox
                  ...
                  ariaLabel={t('notificationSettings.enablePushNotifications')}
                />
                <div className="flex min-w-0 flex-col">
                  <span className={cn("typography-ui-label", !pushSupported ? "text-muted-foreground" : "text-foreground")}>{t('notificationSettings.enablePushNotifications')}</span>
                  <span className="typography-meta text-muted-foreground">
                    {!pushSupported
                      ? t('notificationSettings.pushNotSupported')
                      : t('notificationSettings.pushAlerts')}
                  </span>
                </div>
                {pushBusy && (
