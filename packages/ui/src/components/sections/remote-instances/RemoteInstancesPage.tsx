...
const phaseLabel = (phase?: string): string => {
  switch (phase) {
    case 'config_resolved':
      return m.riSshInstancesStatusConfigResolved();
    case 'auth_check':
      return m.riSshInstancesStatusCheckingAuth();
    case 'master_connecting':
      return m.riSshInstancesStatusEstablishingSsh();
    case 'remote_probe':
      return m.riSshInstancesStatusProbingRemote();
    case 'installing':
      return m.riSshInstancesStatusInstallingOc();
    case 'updating':
      return m.riSshInstancesStatusUpdatingOc();
    case 'server_detecting':
      return m.riSshInstancesStatusDetectingServer();
    case 'server_starting':
      return m.riSshInstancesStatusStartingServer();
    case 'forwarding':
      return m.riSshInstancesStatusForwardingPorts();
    case 'ready':
      return m.riSshInstancesStatusReadyShort();
    case 'degraded':
      return m.riSshInstancesStatusReconnecting();
    case 'error':
...
const forwardTypeDescription = (type: DesktopSshPortForwardType): string => {
  switch (type) {
    case 'remote':
      return m.riSshInstancesRemoteForwardTypeRemote();
    case 'dynamic':
      return m.riSshInstancesRemoteForwardTypeDynamic();
    default:
      return m.riSshInstancesRemoteForwardTypeLocal();
  }
};
...
export const RemoteInstancesPage: React.FC = () => {
  ...
  const handleSave = React.useCallback(async () => {
    ...
    if (!normalized.sshCommand.trim()) {
      toast.error(m.riSshInstancesSshCommandRequired());
      return;
    }
    ...
    if (
      ...
      const store = window.confirm(m.riSshInstancesStoreSshPassword());
    ...
    if (
      ...
      const store = window.confirm(m.riSshInstancesOcPasswordStoreConfirm());
    ...
    try {
      ...
      toast.success(m.riSshInstancesSshInstanceSaved());
    } catch (error) {
      toast.error(m.riSshInstancesFailedSaveSshInstance(), {
  ...
  const createImportedInstance = React.useCallback(
    async (host: string, destination: string): Promise<boolean> => {
      ...
      try {
        ...
        setSelectedId(id);
        toast.success(m.riSshInstancesInstanceCreated());
        return true;
      } catch (error) {
  ...
  const handlePatternCreate = React.useCallback(async () => {
    ...
    if (!destination) {
      toast.error(m.riSshInstancesDestinationRequired());
      return;
    }
  ...
  const handleCopyAllLogs = React.useCallback(() => {
    if (!logLinesText.trim()) {
      toast.error(m.riSshInstancesNoLogsToCopy());
      return;
    }
    void copyTextToClipboard(logLinesText).then((result) => {
      if (result.ok) {
        toast.success(m.riSshInstancesLogsCopied());
      }
    });
  }, [logLinesText]);
  ...
  const handleClearLogs = React.useCallback(async () => {
    ...
    try {
      ...
      setLogDialogLines([]);
      toast.success(m.riSshInstancesLogsCleared());
    } catch (error) {
      toast.error(m.riSshInstancesFailedClearLogs(), {
        description: error instanceof Error ? error.message : String(error),
      });
    }
  }, [draft]);
  ...
  const handleOpenCurrentInstance = React.useCallback(async () => {
    if (!status?.localUrl) {
      toast.error(m.riSshInstancesLocalUrlNotAvailable());
      return;
    }
  ...
  const retryButtonLabel = isConnecting
    ? m.riSshInstancesConnecting()
    : isReconnecting
      ? reconnectAppearsStuck
        ? m.riSshInstancesReconnectNow()
        : m.riSshInstancesReconnecting()
      : m.riSshInstancesRetry();
  ...
  const primaryButtonLabel = isReady ? m.commonKeyDisconnect() : canDisconnect ? m.commonKeyCancel() : m.commonKeyConnect();
      ...
      <SettingsPageLayout>
        <div className="mb-8">
          <div className="mb-1 px-1 space-y-0.5">
            <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesManageTitle()}</h3>
            <p className="typography-meta text-muted-foreground">{m.riSshInstancesManageDesc()}</p>
          </div>
          <section className="px-2 pb-2 pt-0 space-y-3">
            <p className="typography-meta text-muted-foreground">{m.riSshInstancesSelectInstance()}</p>
          </section>
        </div>

        <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
          <div className="mb-1 px-1 space-y-0.5">
            <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesImportTitle()}</h3>
          </div>
          <section className="px-2 pb-2 pt-0">
          {isImportsLoading ? (
            <p className="typography-meta text-muted-foreground">{m.riSshInstancesLoadingHosts()}</p>
          ) : importCandidates.length === 0 ? (
            <p className="typography-meta text-muted-foreground">{m.riSshInstancesNoHosts()}</p>
          ) : (
              ...
              {importCandidates.map((candidate) => (
                <div key={`${candidate.source}:${candidate.host}`} className="flex items-center justify-between gap-3 rounded-md border border-[var(--interactive-border)] px-3 py-2">
                 ...
                 <div className="typography-ui-label text-foreground truncate">
                       {candidate.host}
                       {candidate.pattern ? ` (${m.riSshInstancesPattern()})` : ''}
                     </div>
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>{m.riSshInstancesPatternDialogTitle()}</DialogTitle>
              <DialogDescription>
                {patternHost ? `${patternHost} requires a concrete destination.` : m.riSshInstancesPatternEnterDestination()}
              </DialogDescription>
            </DialogHeader>
            <form
              className="space-y-3"
            ...
            >
              <Input
                ...
                placeholder={m.riSshInstancesPatternPlaceholder()}
                autoFocus
              />
    ...
    <SettingsPageLayout>
      <div className="mb-6 px-1">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 typography-meta text-muted-foreground">
          ...
          {reconnectAppearsStuck ? <span>{m.riSshInstancesReconnectStale()}</span> : null}
        </div>
      </div>

      <div className="mb-8">
        <div className="mb-1 px-1 space-y-0.5">
          <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesActionsTitle()}</h3>
          <p className="typography-meta text-muted-foreground">{m.riSshInstancesActionsDesc()}</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                const ok = window.confirm(m.riSshInstancesRemoveInstance());
                if (!ok) return;
                void removeInstance(draft.id)
                  .then(() => {
                    setSelectedId(null);
                    toast.success(m.riSshInstancesRemove());
                  })
                  .catch((err) => {
          ...
          {status?.localUrl ? (
            <div className="flex flex-wrap items-center gap-2 typography-meta text-muted-foreground">
              <span>{m.riSshInstancesCurrentLocalUrl()}</span>
      ...
      <div className="mb-8">
        <div className="mb-1 px-1 space-y-0.5">
          <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesInstanceSection()}</h3>
          <p className="typography-meta text-muted-foreground">{m.riSshInstancesCoreSettingsDesc()}</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">{m.riSshInstancesSshCommand()}</span>
            <Input
              ...
              placeholder={m.riSshInstancesSshCommandRequired()}
            />
          </div>
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">{m.riSshInstancesNickname()}</span>
            <Input
              ...
              placeholder={m.riSshInstancesNickname()}
            />
          </div>
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">{m.riSshInstancesConnectionTimeout()}</span>
            <NumberInput
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        <div className="mb-1 px-1 space-y-0.5">
          <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesTitleRemoteServer()}</h3>
          <p className="typography-meta text-muted-foreground">{m.riSshInstancesDescRemoteServer()}</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
              <HintLabel
                label={m.riSshInstancesMode()}
            ...
            >
              <SelectTrigger className="h-7 w-fit min-w-[140px]">
                <SelectValue placeholder={m.riSshInstancesModeHint()} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="managed">{m.riSshInstancesModeManaged()}</SelectItem>
                <SelectItem value="external">{m.riSshInstancesModeExternal()}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
              <HintLabel
                label={m.riSshInstancesPreferredRemotePort()}
                hint={m.riSshInstancesPreferredRemotePortHint()}
              />
            </div>
            <NumberInput
              containerClassName="w-fit"
          ...
          {isManagedMode ? (
            <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
              <div className="w-56 shrink-0">
                <HintLabel
                  label={m.riSshInstancesInstallMethod()}
              ...
              >
              <SelectTrigger className="h-7 w-fit min-w-[140px]">
                <SelectValue placeholder={m.riSshInstancesInstallMethodHint()} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="bun">{m.riSshInstancesInstallBun()}</SelectItem>
                <SelectItem value="npm">{m.riSshInstancesInstallNpm()}</SelectItem>
                <SelectItem value="download_release">{m.riSshInstancesInstallDownload()}</SelectItem>
                <SelectItem value="upload_bundle">{m.riSshInstancesInstallUpload()}</SelectItem>
              </SelectContent>
              </Select>
            </div>
          ) : null}
          ...
          {isManagedMode ? (
            <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
              <div className="w-56 shrink-0">
                <HintLabel
                  label={m.riSshInstancesKeepServerRunning()}
                  hint={m.riSshInstancesKeepServerRunningHint()}
                />
              </div>
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        <div className="mb-1 px-1 space-y-0.5">
          <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesTitleMainTunnel()}</h3>
          <p className="typography-meta text-muted-foreground">{m.riSshInstancesDescMainTunnel()}</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
              <HintLabel
                label={m.riSshInstancesBindHost()}
            ...
            >
              <SelectTrigger className="h-7 w-fit min-w-[140px]">
                <SelectValue placeholder={m.riSshInstancesBindHostHint()} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="127.0.0.1">127.0.0.1</SelectItem>
                <SelectItem value="localhost">{m.riSshInstancesBindHostLocalhost()}</SelectItem>
          ...
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <div className="w-56 shrink-0">
              <HintLabel
                label={m.riSshInstancesPreferredLocalPort()}
                hint={m.riSshInstancesPreferredLocalPortHint()}
              />
            </div>
            <div className="flex w-full items-center gap-2 md:max-w-sm">
             ...
             />
               <Button
                ...
                title={m.riSshInstancesRandomPortTitle()}
                onClick={() =>
                  updateDraft((current) => ({
                    ...current,
                    localForward: {
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        <div className="mb-1 px-1 space-y-0.5">
          <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesTitleAuthentication()}</h3>
          <p className="typography-meta text-muted-foreground">{m.riSshInstancesDescAuthentication()}</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-3">
          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">{m.riSshInstancesSshPassword()}</span>
            <Input
              className="h-7 md:max-w-sm"
              type="password"
              ...
              placeholder={m.riSshInstancesSshPasswordPlaceholder()}
            />
          </div>

          <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
            <span className="typography-ui-label text-foreground w-56 shrink-0">{m.riSshInstancesOpenchamberPassword()}</span>
            <Input
              className="h-7 md:max-w-sm"
              type="password"
              ...
              placeholder={m.riSshInstancesOpenchamberPasswordPlaceholder()}
            />
          </div>
        </section>
      </div>
      ...
      <div className="mb-8 border-t border-[var(--surface-subtle)] pt-8">
        <div className="mb-1 px-1 space-y-0.5">
          <h3 className="typography-ui-header font-medium text-foreground">{m.riSshInstancesTitlePortForwards()}</h3>
          <p className="typography-meta text-muted-foreground">{m.riSshInstancesDescPortForwards()}</p>
        </div>
        <section className="px-2 pb-2 pt-0 space-y-2">
          {draft.portForwards.length === 0 ? (
            <p className="typography-micro text-muted-foreground/80">{m.riSshInstancesNoExtraForwards()}</p>
          ) : null}
          ...
          {draft.portForwards.map((forward, index) => {
            ...
            const localLabel = forward.type === 'remote' ? m.riSshInstancesLocalTarget() : m.riSshInstancesLocalListen();
            const localHint = forward.type === 'remote'
              ? m.riSshInstancesLocalTargetHint()
              : m.riSshInstancesLocalListenHintAlt();
            const remoteLabel = forward.type === 'remote' ? m.riSshInstancesRemoteListen() : m.riSshInstancesRemoteListenAlt();
            const remoteHint = forward.type === 'remote'
              ? m.riSshInstancesRemoteListenHintAlt()
              : m.riSshInstancesRemoteTargetHint();
            ...
            const typeLabel = forward.type === 'local' ? m.riSshInstancesTypeLocal() : forward.type === 'remote' ? m.riSshInstancesTypeRemote() : m.riSshInstancesTypeDynamic();
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch checked={forward.enabled} onCheckedChange={(checked) => updateForward((item) => ({ ...item, enabled: checked }))} aria-label={m.riSshInstancesEnableForwardAria()} />
                    <Button
                      type="button"
                      variant="ghost"
                      size="xs"
                ...
                <CollapsibleContent className="pt-2">
                  <div className="space-y-0 pb-2">
                    ...
                    <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
                      <div className="w-56 shrink-0">
                        <HintLabel
                          label={m.riSshInstancesForwardType()}
                      ...
                      >
                        <SelectTrigger className="h-7 w-fit min-w-[140px]">
                          <SelectValue placeholder={m.riSshInstancesForwardType()} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="local">{m.riSshInstancesTypeLocal()}</SelectItem>
                          <SelectItem value="remote">{m.riSshInstancesTypeRemote()}</SelectItem>
                          <SelectItem value="dynamic">{m.riSshInstancesTypeDynamic()}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
                      ...
                      <div className="flex items-center gap-1.5">
                        <Input
                          ...
                          placeholder={m.riSshInstancesLocalListenHint()}
                        />
                      ...
                      <div className="flex flex-col gap-1.5 py-1.5 md:flex-row md:items-center md:gap-8">
                        ...
                        <div className="flex items-center gap-1.5">
                          <Input
                            ...
                            placeholder={m.riSshInstancesRemoteListenHint()}
                          />
                          <span className="text-muted-foreground">:</span>
                          <NumberInput
                            containerClassName="w-fit"
            ...
            <Button
              ...
              onClick={() => {
                void openExternalUrl(localEndpointUrl).then((opened) => {
                  if (!opened) {
                    toast.error(m.riSshInstancesFailedOpen());
                  }
                });
              }}
            >
              <RiExternalLinkLine className="h-3.5 w-3.5" />
              {m.riSshInstancesOpen()}
            </Button>
                      ) : null}
                    </div>
                  </div>
              ...
              <Button
                ...
                onClick={() => {
                  void copyTextToClipboard(status.localUrl || '').then((result) => {
                    if (result.ok) {
                      toast.success(m.riSshInstancesLocalUrlCopied());
                    }
                  });
                 }}
               >
      ...
      <Dialog open={logDialogOpen} onOpenChange={setLogDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{m.riSshInstancesSshLogsTitle()}</DialogTitle>
            <DialogDescription>
              {draft?.nickname?.trim() || draft?.sshParsed?.destination || draft?.id || m.riSshInstancesSelectedInstance()}
            </DialogDescription>
          </DialogHeader>
          <div className="flex items-center justify-end gap-2">
            <Button type="button" variant="outline" size="xs" className="!font-normal" onClick={handleCopyAllLogs} disabled={logDialogLoading || !logLinesText.trim()}>
              <RiFileCopyLine className="h-3.5 w-3.5" />
              {m.riSshInstancesCopyAll()}
            </Button>
            <Button type="button" variant="outline" size="xs" className="!font-normal" onClick={() => void handleClearLogs()} disabled={logDialogLoading}>
              <RiDeleteBinLine className="h-3.5 w-3.5" />
              {m.riSshInstancesClear()}
            </Button>
          </div>
          {logDialogLoading ? (
            <div className="typography-meta text-muted-foreground">{m.riSshInstancesLoadingLogs()}</div>
          ) : logDialogError ? (
