...
const normalizeMcpAuthErrorMessage = (error: unknown, fallback: string): string => {
  ...
  if (/oauth state required/i.test(message)) {
    return 'Authorization session expired or was cleared during reload. Return to Sharpie and click Authorize again.';
  }
  return message;
};
...
export const McpOAuthCallbackPage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void (async () => {
      try {
        if (!code) {
          throw new Error('Missing OAuth authorization code. Start authorization again from MCP Settings or paste the returned code into Sharpie manually.');
        }
        ...
        if (!pendingContext?.name) {
          throw new Error('Authorization session details were not available. Start authorization again from MCP Settings or paste the returned code into Sharpie manually.');
        }
        ...
        setMessage('Authorization completed. You can close this tab and return to Sharpie.');
      } catch (authError) {
        if (callbackStateKey) {
          await runtimeFetch(`/api/mcp/auth/pending?state=${encodeURIComponent(callbackStateKey)}`, { method: 'DELETE' }).catch(() => undefined);
        }
