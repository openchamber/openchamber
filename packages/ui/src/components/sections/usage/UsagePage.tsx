  ...
  if (!selectedProviderId) {
    return (
      <div className="flex h-full items-center justify-center text-muted-foreground">
        <p className="typography-body">{m.usSelectProvider()}</p>
      </div>
    );
  }
  ...
  return (
    <ScrollableOverlay keyboardAvoid outerClassName="h-full" className="w-full">
      <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="typography-ui-header font-semibold text-foreground truncate">
              {m.usProviderUsage().replace('{provider}', providerName)}
            </h2>
            <p className="typography-meta text-muted-foreground truncate">
              {isLoading ? (
                <span className="animate-pulse">{m.usRefreshing()}</span>
              ) : (
                m.usLastUpdated().replace('{time}', formatTime(lastUpdated))
              )}
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 px-2">
          ...
          >
            <Checkbox
              ...
              ariaLabel={m.usShowInHeaderMenu()}
            />
            <div className="flex min-w-0 items-center gap-1.5">
              <span className="typography-ui-label text-foreground">{m.usShowInHeaderMenu()}</span>
              <Tooltip delayDuration={1000}>
                <TooltipTrigger asChild>
                  <RiInformationLine className="h-3.5 w-3.5 text-muted-foreground/60 cursor-help" />
                </TooltipTrigger>
                <TooltipContent sideOffset={8} className="max-w-xs">
                  {m.usShowInHeaderTooltip()}
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
        ...
        {!selectedResult && (
          <div className="mb-8 px-2">
            <p className="typography-ui-label text-foreground">{m.usNoData()}</p>
          </div>
        )}

        {error && (
          <div className="mb-8 rounded-lg border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-4 py-3">
            <p className="typography-ui-label font-medium text-[var(--status-error)]">{m.usRefreshFailed()}</p>
        ...
        {selectedResult && !selectedResult.configured && (
          <div className="mb-8 rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-background)] px-4 py-3">
            <p className="typography-ui-label font-medium text-[var(--status-warning)]">{m.usProviderNotConfigured()}</p>
        ...
        {providerModels.length > 0 && (
          <div className="mb-8">
            <div className="mb-1 px-1">
              <h3 className="typography-ui-header font-medium text-foreground">{m.usModelQuotas()}</h3>
            </div>
                ...
                return (
                  <section className="p-2">
                    ...
                    >
                      <CollapsibleTrigger className="flex w-full items-center justify-between py-0.5 group">
                        <div className="flex items-center gap-1.5 text-left">
                          <span className="typography-ui-label font-normal text-foreground">{m.usOtherModels()}</span>
                          <span className="typography-micro text-muted-foreground">
                            ({otherModels.length})
                          </span>
                        </div>
        ...
        {selectedResult?.configured && usage && Object.keys(usage.windows ?? {}).length === 0 &&
          providerModels.length === 0 && (
          <div className="mb-8 px-2">
            <p className="typography-ui-label text-foreground">{m.usNoQuotaWindows()}</p>
            <p className="typography-meta text-muted-foreground mt-1">{m.usNoQuotaWindowsDesc()}</p>
          </div>
        )}
