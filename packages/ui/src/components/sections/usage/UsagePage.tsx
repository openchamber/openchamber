...
export const UsagePage: React.FC = () => {
  ...
  const providerName = providerMeta?.name ?? selectedProviderId ?? t('usagePage.fallbackProviderName');
  ...
  if (!selectedProviderId) {
    return (
      <div className="flex h-full items-center justify-center text-muted-foreground">
        <p className="typography-body">{t('usagePage.selectProvider')}</p>
      </div>
    );
  }
  ...
  return (
    <ScrollableOverlay keyboardAvoid outerClassName="h-full" className="w-full">
      <div className="mx-auto w-full max-w-3xl p-3 sm:p-6 sm:pt-8">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="typography-ui-header font-semibold text-foreground truncate">
              {t('usagePage.providerUsage', { provider: providerName })}
            </h2>
            <p className="typography-meta text-muted-foreground truncate">
              {isLoading ? (
                <span className="animate-pulse">{t('usagePage.refreshingUsage')}</span>
              ) : (
                `${t('usagePage.lastUpdated')} ${formatTime(lastUpdated)}`
              )}
            </p>
          </div>
        </div>
        ...
        <div className="mb-8 px-2">
          ...
          >
            <Checkbox
              ...
              ariaLabel={t('usagePage.showInHeaderMenuAria')}
            />
            <div className="flex min-w-0 items-center gap-1.5">
              <span className="typography-ui-label text-foreground">{t('usagePage.showInHeaderMenu')}</span>
              <Tooltip delayDuration={1000}>
                <TooltipTrigger asChild>
                  <RiInformationLine className="h-3.5 w-3.5 text-muted-foreground/60 cursor-help" />
                </TooltipTrigger>
                <TooltipContent sideOffset={8} className="max-w-xs">
                  {t('usagePage.showInHeaderMenuTooltip')}
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
        ...
        {!selectedResult && (
          <div className="mb-8 px-2">
            <p className="typography-ui-label text-foreground">{t('usagePage.noUsageData')}</p>
          </div>
        )}

        {error && (
          <div className="mb-8 rounded-lg border border-[var(--status-error-border)] bg-[var(--status-error-background)] px-4 py-3">
            <p className="typography-ui-label font-medium text-[var(--status-error)]">{t('usagePage.failedToRefreshUsage')}</p>
        ...
        {selectedResult && !selectedResult.configured && (
          <div className="mb-8 rounded-lg border border-[var(--status-warning-border)] bg-[var(--status-warning-background)] px-4 py-3">
            <p className="typography-ui-label font-medium text-[var(--status-warning)]">{t('usagePage.providerNotConfigured')}</p>
            <p className="typography-meta text-[var(--status-warning)]/80 mt-1">
              {t('usagePage.providerNotConfiguredDesc')}
            </p>
          </div>
        )}
        ...
        {providerModels.length > 0 && (
          <div className="mb-8">
            <div className="mb-1 px-1">
              <h3 className="typography-ui-header font-medium text-foreground">{t('usagePage.modelQuotas')}</h3>
            </div>
                ...
                return (
                  <section className="p-2">
                    ...
                    >
                      <CollapsibleTrigger className="flex w-full items-center justify-between py-0.5 group">
                        <div className="flex items-center gap-1.5 text-left">
                          <span className="typography-ui-label font-normal text-foreground">{t('usagePage.otherModels')}</span>
                          <span className="typography-micro text-muted-foreground">
                            ({otherModels.length})
                          </span>
                        </div>
        ...
        {selectedResult?.configured && usage && Object.keys(usage.windows ?? {}).length === 0 &&
          providerModels.length === 0 && (
          <div className="mb-8 px-2">
            <p className="typography-ui-label text-foreground">{t('usagePage.noQuotaWindows')}</p>
            <p className="typography-meta text-muted-foreground mt-1">{t('usagePage.noQuotaWindowsDesc')}</p>
          </div>
        )}
