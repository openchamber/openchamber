  ...
  const handleMerge = React.useCallback(
    async (branch: string) => {
      ...
      try {
        ...
        addOperationLog(t('gitView.operation.mergingBranch', { branch, currentBranch: currentBranch ?? '' }), 'running');
        ...
        if (result.conflict) {
          ...
          updateLastLog('done', t('gitView.mergedIntoBranch', { branch, currentBranch: currentBranch ?? '' }));
          clearConflictState();
  ...
  const handleRebase = React.useCallback(
    async (branch: string) => {
      ...
      try {
        ...
        addOperationLog(t('gitView.operation.rebasingBranch', { currentBranch: currentBranch ?? '', branch }), 'running');
        ...
        if (result.conflict) {
          ...
          updateLastLog('done', t('gitView.rebasedOntoBranch', { currentBranch: currentBranch ?? '', branch }));
          clearConflictState();
          addOperationLog(t('gitView.operation.refreshingRepositoryStatus'), 'running');
          await refreshStatusAndBranches();
          await refreshLog();
      ...
      try {
        ...
        if (operation === 'merge') {
          ...
          if (result.conflict) {
            ...
            toast.success(t('gitView.mergedIntoBranch', { branch, currentBranch: currentBranch ?? '' }));
          }
        } else {
          const result = await git.rebase(currentDirectory, { onto: branch });
          if (result.conflict) {
            ...
            operationSucceeded = true;
            toast.success(t('gitView.rebasedOntoBranch', { currentBranch: currentBranch ?? '', branch }));
          }
        }
    ...
    <div className={cn('flex h-full flex-col overflow-hidden', isSidebarMode ? 'bg-transparent' : 'bg-background')} data-keyboard-avoid="true">
          ...
          <div className={cn('min-w-0 min-h-0 h-full flex flex-col', isSidebarMode ? 'bg-transparent border-t border-border/40' : 'bg-muted/10')}>
            <div className="px-3 py-1.5">
              <AnimatedTabs<ActionTab>
                ...
                tabs={[
                  { value: 'commit', label: t('gitView.tabs.commit'), icon: RiGitCommitLine },
                  { value: 'branch', label: t('gitView.tabs.update'), icon: RiGitMergeLine },
                  { value: 'pr', label: 'PR', icon: RiGitPullRequestLine },
                  { value: 'worktree', label: t('gitView.tabs.worktree'), icon: RiSplitCellsHorizontal },
                ]}
              />
            </div>
            ...
            >
              ...
              {actionTab === 'worktree' ? (
                integrateCommitsProps ? (
                  ...
                  <div className="space-y-1">
                    <div className="typography-ui-header font-semibold text-foreground">{t('integrateCommits.reintegrateCommits')}</div>
              ...
              {actionTab === 'pr' ? (
                pullRequestProps ? (
                  ...
                  <div className="space-y-1">
                    <div className="typography-ui-header font-semibold text-foreground">{t('gitView.pullRequestTitle')}</div>
                    <div className="typography-micro text-muted-foreground">
      ...
      <Dialog open={isHistoryDialogOpen} onOpenChange={setIsHistoryDialogOpen}>
        <DialogContent className="max-w-5xl max-h-[80vh] flex flex-col overflow-hidden">
          <DialogHeader>
            <DialogTitle>{t('gitView.historyDialogTitle')}</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={isGitmojiPickerOpen} onOpenChange={setIsGitmojiPickerOpen}>
        <DialogContent className="max-w-md p-0 overflow-hidden">
          <DialogHeader className="px-4 pt-4">
            <DialogTitle>{t('gitView.gitmojiDialogTitle')}</DialogTitle>
          </DialogHeader>
          <Command className="h-[420px]">
            <CommandInput
              placeholder={t('gitView.gitmojiSearchPlaceholder')}
              value={gitmojiSearch}
