...
export const ConflictDialog: React.FC<ConflictDialogProps> = ({
  ...
  React.useEffect(() => {
    ...
    getConflictDetails(directory)
      ...
      .catch((err) => {
        const message = err instanceof Error ? err.message : m.gitConflictError();
  ...
  const handleResolveInCurrentSession = async () => {
    ...
    if (!context) {
      toast.error(m.gitConflictError());
      return;
    }

    if (!currentSessionId) {
      toast.error(m.gitConflictError(), { description: m.gitResolveInNewSession() });
      return;
    }
        ...
        <div className="flex flex-col gap-4 overflow-hidden">
          <DialogHeader>
            <div className="flex items-center gap-2">
              ...
              <DialogTitle>{m.gitConflictsDetected(operation)}</DialogTitle>
            </div>
            <DialogDescription>
              {m.gitConflictsDetectedDescription(operation)}
            </DialogDescription>
          </DialogHeader>
          ...
          {displayFiles.length > 0 && (
            <div className="space-y-2 overflow-hidden">
              <div className="flex items-center justify-between">
                <p className="typography-meta text-muted-foreground">{m.gitConflictedFiles()}</p>
          ...
          {conflictDetails?.headInfo && (
            <div className="space-y-1 overflow-hidden">
              <p className="typography-meta text-muted-foreground">{m.gitHeadInformation()}</p>
