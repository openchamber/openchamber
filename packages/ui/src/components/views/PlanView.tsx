...
import { parseProjectPlanMarkdown } from '@/lib/aliasAdeConfig';
...
export const PlanView: React.FC<PlanViewProps> = ({ targetPath = null }) => {
  ...
  const MD_VIEWER_MODE_KEY = 'aliasAde:plan:md-viewer-mode';
  ...
  const handleConfirmPlanSend = React.useCallback(
    async (execution: TodoSendExecution) => {
      ...
      try {
        ...
        if (pendingPlanSend.target === 'worktree') {
          ...
          initializeNewAliasAdeSession(sessionResult.id, useConfigStore.getState().agents ?? []);
        }
    ...
    [canCreateWorktree, createSession, currentProjectRef, initializeNewAliasAdeSession, pendingPlanSend, resolvedPath, routeToChat, sendMessage, sendPromptTitle, setCurrentSession]
  );
