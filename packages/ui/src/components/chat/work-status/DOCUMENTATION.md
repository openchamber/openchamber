...
| Subagents | child sessions from `useAllLiveSessions` (`parentID`) + `useAllSessionStatuses`; terminal failures from the live session failure store; per-row cost from `useSubagentCostRollup`'s `perChildCost` (each child's own subtree total, so nested subagent-of-subagent cost rolls up under its immediate parent row) | |
