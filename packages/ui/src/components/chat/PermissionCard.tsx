  ...
  const renderToolContent = () => {
        ...
        <>
          ...
          {workingDir && (
            <div className="typography-meta text-muted-foreground mb-2">
              <span className="font-semibold">{t('permissionCard.workingDirectory')}:</span> <code className="px-1 py-0.5 bg-muted/30 rounded">{workingDir}</code>
            </div>
          )}
          {timeout && (
            <div className="typography-meta text-muted-foreground mb-2">
              <span className="font-semibold">{t('permissionCard.timeout')}:</span> {timeout}ms
            </div>
          )}
          {}
          {command && (
    ...
    if (tool === 'edit' || tool === 'multiedit' || tool === 'str_replace' || tool === 'str_replace_based_edit_tool') {
      ...
      return (
        <>
          {replaceAll && (
            <div className="typography-meta text-muted-foreground mb-2">
              <span className="font-semibold">{t('permissionCard.replaceAllOccurrences')}</span>
            </div>
          )}
          {changes && (
    ...
    if (tool === 'webfetch' || tool === 'fetch' || tool === 'curl' || tool === 'wget') {
      ...
      return (
        <>
          {url && (
            <div className="mb-2">
              <div className="typography-meta text-muted-foreground mb-1">{t('permissionCard.request')}:</div>
          ...
          {headers && Object.keys(headers).length > 0 && (
            <div className="mb-2">
              <div className="typography-meta text-muted-foreground mb-1">{t('permissionCard.headers')}:</div>
          ...
          {body && (
            <div className="mb-2">
              <div className="typography-meta text-muted-foreground mb-1">{t('permissionCard.body')}:</div>
    ...
    return (
      <>
        ...
        {genericContent && (
          <div className="mb-2">
            <div className="typography-meta text-muted-foreground mb-1">{t('permissionCard.action')}:</div>
            <ScrollableOverlay outerClassName="max-h-32" className="p-0">
        ...
        {Object.keys(permission.metadata).length > 0 && !genericContent && !description && (
          <div>
            <div className="typography-meta text-muted-foreground mb-1">{t('permissionCard.details')}:</div>
            <ScrollableOverlay outerClassName="max-h-32" className="p-0">
              <pre className="typography-meta font-mono px-2 py-1 bg-muted/30 rounded whitespace-pre-wrap break-all">
        ...
        <div className="-mt-1 border border-border/30 rounded-xl bg-muted/10">
          ...
          <div className="px-2 py-2">
            {permission.patterns.length > 0 && (
              <div className="mb-2">
                <div className="typography-meta text-muted-foreground mb-1">{t('permissionCard.patterns')}:</div>
