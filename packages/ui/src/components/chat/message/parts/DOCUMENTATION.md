...
- Activity color coding (the "Color-code tools & activity" setting, default on) tints each tool or reasoning row's icon and title with a per-category theme accent: `lib/activityColors.ts` maps the normalized tool name (or the reasoning/justification part kind) to one of ten categories — thinking, read, edit, write, shell, search, web, question, agent, system — and the theme CSS generator emits one `--tools-activity-<category>` variable per colored category from existing theme tokens (no hard-coded palette), overridable via `colors.tools.activity` in a theme JSON. `system` (todo, plan, OpenChamber plugin, MCP and unknown tools) resolves to no variable, so those rows keep the regular muted tool styling. Task rows keep their per-agent colors; error rows keep `--status-error`. Only icon/title colors change: descriptions, the expanded body, and row structure are untouched, and turning the setting off restores the single neutral tone.
