...
- Exception: `openchamber` calls with `action: 'vision.run'` render a dedicated result card — the described image's thumbnail (opens the standard image-preview popup), the describing model, and the description text, parsed from the result envelope by `visionToolModel.ts`. Failed calls surface the envelope's error message; unparseable output falls back to the neutral JSON views.
