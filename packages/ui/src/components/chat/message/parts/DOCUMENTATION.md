...
- Selecting a JSON summary, tree, or raw view saves that mode as a client-wide preference. New and refreshed JSON tool outputs read the saved mode across sessions. Same-origin embedded chat frames use one safe-storage adapter owned by the top frame, so they also share its in-memory fallback when durable storage is unavailable; missing or invalid preferences use Summary.
