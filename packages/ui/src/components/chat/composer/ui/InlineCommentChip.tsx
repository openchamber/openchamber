/**
 * One pending code comment, shown above the composer.
 *
 * Attached context is summarized by a count, but a comment is something the
 * user wrote against a specific range, so each gets its own chip: the text is
 * editable in place and removable on its own, without clearing the others.
 *
 * The editor opens by itself for a freshly added draft. The VS Code "Add
 * Comment" flow creates the draft empty and expects the user to land in the
 * editor, rather than in a native single-line input box.
 */
...
interface InlineCommentChipProps {
    ...
    // Drafts are owned by a runtime/directory/session tuple, not a bare session id.
    target: InlineCommentDraftTarget;
}

export const InlineCommentChip: React.FC<InlineCommentChipProps> = React.memo(({ draft, target }) => {
    const { t } = useI18n();
    ...
    // Some sources already carry the range in the label (the VS Code editor
    // thread sends `file.ts:12`), so appending it again read as `file.ts:12:12`.
    const fileName = draft.fileLabel.replace(/:\d+(-\d+)?$/, '');
    const label = draft.startLine === draft.endLine
        ? `${fileName}:${draft.startLine}`
        : `${fileName}:${draft.startLine}-${draft.endLine}`;
    ...
    const handleSave = () => {
        ...
        updateDraft(target, draft.id, { text: trimmed });
        setOpen(false);
    };
        ...
        <Popover.Root open={open} onOpenChange={handleOpenChange}>
            ...
            >
                ...
                <button
                    ...
                    onClick={() => removeDraft(target, draft.id)}
