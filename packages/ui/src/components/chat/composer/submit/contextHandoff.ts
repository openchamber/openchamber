...
type ConsumeComposerContextOptions = {
    /** Isolated composers can consume inline drafts without claiming parent synthetic context. */
    consumeSynthetic?: boolean;
};
...
export const consumeComposerContext = (
    ...
    options?: ConsumeComposerContextOptions,
    ...
    context.syntheticParts = options?.consumeSynthetic === false
        ? []
        : useInputStore.getState().consumePendingSyntheticParts(target ?? undefined) ?? [];
...
const restoreQueuedMessageContexts = (
    ...
    for (const message of messages) {
        // `context` is the current queue contract. Older local queue entries
        // only have `capturedContext`, so keep that as the fallback for
        // migration rather than combining the two and restoring duplicates.
        for (const part of message.context ?? message.capturedContext ?? []) {
            if ('kind' in part) {
                if (part.kind === 'instruction') continue;
                if (part.kind === 'synthetic') {
                    syntheticParts.push({ text: part.text, synthetic: true });
                    continue;
                }
                ...
                if (draft) inlineComments.push(draft);
                    ...
                    if (part.instructions) syntheticParts.push({ text: part.instructions, synthetic: true });
                    syntheticParts.push({ text: part.text, synthetic: true, metadata: part.metadata });
                }
                continue;
            }
            const payload = readContextPart({ type: 'text', metadata: part.metadata });
