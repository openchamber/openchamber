...
// A single session row — sized for comfortable touch (≥56px tall).
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isCurrent ? "bg-[var(--interactive-selection)]/40" : "hover:bg-[var(--interactive-hover)]"
      )}
    >
...
// A project filter pill — sized for touch (≥40px tall). Selecting it filters
  ...
  const renderHeader = React.useCallback((closeButton: React.ReactNode) => (
    <div className="shrink-0">
      ...
      <div className="flex justify-center pt-2.5 pb-1">
        <div className="h-1 w-9 rounded-full bg-[var(--surface-mutedForeground)]/40" />
      </div>
      ...
      {projects.length > 1 && (
        <div
          className="flex items-center gap-2 overflow-x-auto border-t border-border/40 px-4 py-2.5 scrollbar-none"
