...
`navigate_session_back` and `navigate_session_forward` traverse chronological session visits, independently of adjacent-tab switching. Their defaults are Cmd+[ / Cmd+] on macOS and Ctrl+[ / Ctrl+] elsewhere, with strict platform modifiers. Favorite-model cycling keeps Ctrl+[ / Ctrl+] on macOS and uses Ctrl+Alt+[ / Ctrl+Alt+] elsewhere. Effective defaults yield to conflicting explicit assignments without rewriting persisted overrides. Header tooltips, Help and Settings resolve the same effective binding. Electron Back/Forward menu clicks invoke these registered commands; keyboard dispatch remains in the renderer.
