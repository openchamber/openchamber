...
export const SETTINGS_PAGE_METADATA: readonly SettingsPageMeta[] = [
  ...
  { slug: 'connection', title: 'Connection', group: 'projects', kind: 'single', keywords: ['connection', 'host', 'remote', 'local', 'server', 'url', 'desktop'], isAvailable: (ctx) => ctx.isDesktop && !ctx.isWeb && !ctx.isVSCode },
] as const;
