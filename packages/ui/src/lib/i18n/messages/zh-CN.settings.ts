...
export const settingsDict = {
  ...
  'settings.page.vision.title': '视觉',
  'settings.page.vision.description': '让无法查看图片的模型通过配置好的视觉模型来描述图片。',
  'settings.vision.section.title': '视觉工具',
  'settings.vision.section.info': '当当前模型无法查看图片时，智能体可以调用视觉工具读取图片文件，并请配置好的视觉模型描述它——内容、布局、风格以及所有可见文字。描述会返回给你的模型，让它无需亲眼看图也能推理。每次调用都按此处选择的视觉模型计费。',
  'settings.vision.model.label': '视觉模型',
  'settings.vision.model.info': '为视觉工具描述图片的模型。仅列出支持图片输入的模型；目录无法判断的模型仍会显示，服务器会在每次运行前重新验证。',
  'settings.vision.model.notSelected': '选择视觉模型',
  'settings.vision.model.required': '请选择一个视觉模型以启用视觉工具。',
  'settings.vision.prompt.label': '描述提示词',
  'settings.vision.prompt.info': '随每张图片一起发送给视觉模型的提示词。好的提示词会要求内容、布局、风格、颜色和逐字文本，让你的模型获得完整信息。留空则使用内置默认提示词。',
  'settings.vision.prompt.placeholder': '留空以使用默认提示词。',
  'settings.vision.prompt.save': '保存',
  'settings.vision.prompt.reset': '恢复默认',
  'settings.vision.toast.saved': '视觉设置已保存',
  'settings.vision.toast.saveFailed': '保存视觉设置失败',
  'settings.vision.toast.loadFailed': '加载视觉设置失败',
