export const settingsDict = {
  ...
  'settings.page.vision.title': '視覺',
  'settings.page.vision.description': '讓無法查看圖片的模型透過設定好的視覺模型來描述圖片。',
  'settings.vision.section.title': '視覺工具',
  'settings.vision.section.info': '當目前模型無法查看圖片時，代理可以呼叫視覺工具讀取圖片檔案，並請設定好的視覺模型描述它——內容、版面、風格以及所有可見文字。描述會傳回給你的模型，讓它不必親眼看到圖片也能推理。每次呼叫都按此處選擇的視覺模型計費。',
  'settings.vision.model.label': '視覺模型',
  'settings.vision.model.info': '為視覺工具描述圖片的模型。僅列出支援圖片輸入的模型；目錄無法判斷的模型仍會顯示，伺服器會在每次執行前重新驗證。',
  'settings.vision.model.notSelected': '選擇視覺模型',
  'settings.vision.model.required': '請選擇一個視覺模型以啟用視覺工具。',
  'settings.vision.prompt.label': '描述提示詞',
  'settings.vision.prompt.info': '隨每張圖片一起傳送給視覺模型的提示詞。好的提示詞會要求內容、版面、風格、顏色和逐字文字，讓你的模型獲得完整資訊。留空則使用內建預設提示詞。',
  'settings.vision.prompt.placeholder': '留空以使用預設提示詞。',
  'settings.vision.prompt.save': '儲存',
  'settings.vision.prompt.reset': '恢復預設',
  'settings.vision.toast.saved': '視覺設定已儲存',
  'settings.vision.toast.saveFailed': '儲存視覺設定失敗',
  'settings.vision.toast.loadFailed': '載入視覺設定失敗',
