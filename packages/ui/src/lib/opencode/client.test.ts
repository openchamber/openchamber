...
mock.module('@/lib/runtime-fetch', () => ({
  runtimeFetch: mock(async (path: string, init?: { query?: DirectoryProbeQuery }) => {
    runtimeFetchCalls.push({ path, query: init?.query });
    const next = runtimeFetchResults.shift();
    if (next instanceof Error) throw next;
    return next ?? new Response(JSON.stringify([]), { headers: { 'Content-Type': 'application/json' } });
  }),
}));
