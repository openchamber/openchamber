...
import { appendInlineComments, buildInlineCommentParts, isCommentCardSource } from "./inlineComments"
...
describe("buildInlineCommentParts (OpenCode Desktop card parity)", () => {
  ...
  test("pr-comment/pr-check are attached context, not comment cards", () => {
    const parts = buildInlineCommentParts([
      draft({ source: "pr-comment", fileLabel: "#42" }),
      draft({ source: "pr-check", fileLabel: "build" }),
    ])
    for (const part of parts) {
      expect(part.synthetic).toBe(true)
      expect(part.metadata).toBe(undefined)
    }
    expect(parts[0].text).toContain("Attached GitHub PR comment")
    expect(parts[1].text).toContain("Attached failed GitHub PR check")
  })
})

describe("isCommentCardSource", () => {
  test("code comment sources render as cards", () => {
    for (const source of ["file", "diff", "plan"] as const) {
      expect(isCommentCardSource(source)).toBe(true)
    }
  })

  test("terminal and attached-context sources do not", () => {
    const notCards = [
      "terminal",
      "preview-console",
      "preview-annotation",
    ...
    for (const source of notCards) {
      expect(isCommentCardSource(source)).toBe(false)
    }
  })
})
