...
export const isSessionStatusAuthoritative = (
  status: { type?: string } | undefined,
  snapshotReady: boolean,
): boolean => status !== undefined || snapshotReady;

/**
 * Determines if a session is actively working.
 * Checks session_status and, only before an authoritative status snapshot is
 * available, falls back to the trailing assistant message when its completion
...
function useSessionActivity(sessionId: string | null | undefined, directory?: string): SessionActivityResult {
  ...
  return React.useMemo<SessionActivityResult>(() => {
    ...
    const hasAuthoritativeStatus = isSessionStatusAuthoritative(status, statusSnapshotReady);
    ...
    const hasPendingAssistant = !hasAuthoritativeStatus && Boolean(
      lastMessage
  ...
  }, [sessionId, status, statusSnapshotReady, messages, permissions, questions]);
}
