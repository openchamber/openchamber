...
async function resolveTargetPort(options = {}, deps = {}) {
  ...
  if (ports.length > 1) {
    ...
    throw new TunnelCliError(
      `Multiple Sharpie instances are running (ports: ${ports.join(', ')}). Choose one with --port <port>.`,
  ...
  throw new TunnelCliError(
    'No running Sharpie server found. Start one with `openchamber serve`, or pass --port <port>.',
