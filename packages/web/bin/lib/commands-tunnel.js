...
async function resolveTargetInstance({
  ...
  if (options.all && requireAll) {
    if (running.length === 0) {
      throw new Error('No running Sharpie instance found. Start one with `openchamber serve`.');
    }
    return running;
  }
      ...
      if (rejectDesktopRuntime) {
        ...
        if (!attachability.attachable) {
          if (attachability.reason === 'desktop') {
            throw new Error(
              `Port ${options.port} is used by Sharpie Desktop app. Tunnel attach requires a CLI instance from \`openchamber serve\`.`
            );
          }
          throw new Error(
            `Port ${options.port} is not an attachable Sharpie tunnel instance. Ensure it is healthy and running Sharpie CLI runtime.`
          );
        }
      }
      return found;
  ...
  if (rejectDesktopRuntime) {
    ...
    if (attachableEntries.length > 1) {
      ...
      throw new Error(`Multiple attachable Sharpie instances found: ${ports}. Use --port <port> or --all.`);
    }
    ...
    if (sawDesktop) {
      throw new Error('Only Sharpie Desktop instance(s) detected. Tunnel attach requires a CLI instance from `openchamber serve`.');
    }

    throw new Error('No attachable Sharpie instance found. Start one with `openchamber serve`.');
  }
  ...
  throw new Error(`Multiple Sharpie instances found: ${ports}. Use --port <port> or --all.`);
}
    ...
    switch (subcommand) {
          ...
          if (isReady) {
            ...
            if (isManagedRemote && (hasPortOrOriginIssue || hasTokenIssue)) {
              troubleshootingHints.push({
                ...
                code: '[PORT_MISMATCH]',
                lines: [
                  'Cloudflare target must match the active Sharpie CLI port.',
      ...
      case 'start': {
        ...
        if (!options.explicitPort && canPrompt(options)) {
          ...
          if (runningInstances.length > 1) {
            ...
            if (safeInstances.length === 0) {
              throw new TunnelCliError(
                'All discovered Sharpie instance ports are browser-unsafe. Start or target a safe port (3000, 5173, 8080, or high ephemeral).',
                EXIT_CODE.USAGE_ERROR,
              );
            }
            ...
            if (attachableSafeInstances.length === 0) {
              throw new TunnelCliError(
                'No attachable Sharpie CLI instances found on safe ports. Start one with `openchamber serve --port 3000`.',
                EXIT_CODE.USAGE_ERROR,
              );
            }

            const selectedPort = await clackSelect({
              message: 'Select Sharpie instance port',
        ...
        if (instance?.autoStarted) {
          ...
          healthProgress?.start(`Waiting for Sharpie on port ${instance.port} to become healthy (up to 60s)...`);
          ...
          const healthy = await waitForServerHealth(instance.port, {
            ...
            onTick({ elapsedMs, complete }) {
              ...
              if (delta > 0) {
                ...
                healthProgress.message(`Waiting for Sharpie health (${progressedSeconds}s / 60s)...`);
              }
          ...
          if (!healthy) {
            healthProgress?.stop('Sharpie is still starting');
            throw new Error(
              `Sharpie on port ${instance.port} is still starting after 60s. Startup time can vary by machine performance. ` +
