...
function createSettingsAccessors() {
  const settingsPath = path.join(getSharpieDataDir(), SETTINGS_FILE_NAME);
...
function createCliPairingRuntime() {
  const dataDir = getSharpieDataDir();
...
function createConnectUrlCommand({ serveCommand }) {
  return async function connectUrlCommand(options = {}) {
    assertSafeBrowserPort(options.port, { context: 'Sharpie connect-url' });
    ...
    clackIntro('Sharpie pairing link');
    if (serverState.autoStarted) {
      logStatus('success', `started Sharpie on port ${options.port}`);
    }
    ...
    if (resolvedServerUrl.source === 'lan-detected') {
      clackLog.info('Detected a LAN address because Sharpie is bound to all interfaces. Use --server to override it.');
    } else if (resolvedServerUrl.source === 'loopback-fallback') {
      clackLog.warn('Sharpie is bound to all interfaces, but no LAN address was detected. Use --server to provide a reachable URL.');
      ...
      if (options.relay) {
        logStatus('warn', '[LAN_UNREACHABLE]', 'Sharpie only listens on this machine, so devices will always connect through the relay. Restart with --lan to allow direct home-network connections.');
      } else {
        logStatus('warn', '[LAN_UNREACHABLE]', 'Sharpie only listens on this machine, so other devices cannot use this link. Restart with --lan, or use --server to provide a reachable URL.');
      }
    }
    clackLog.info('Scan or paste this link into another Sharpie client. It is single-use and expires.');
    if (options.qr === true) {
