...
- `createOpenCodeLifecycleRuntime(dependencies)`: creates lifecycle runtime for managed/external OpenCode process orchestration. The optional `onOpenCodeRestarted` dependency (default `null`) is fired after a successful managed restart. `index.js` rebinds event-stream readers to the possibly-new port (#2638), then calls `interruptBusySessionsAfterRestart()` and broadcasts one `opencode-restart-interrupted` UI notification when interrupted turns exist (#2943). An unexpected exit of a *ready* managed OpenCode process fires `onOpenCodeRestarted` immediately (so busy sessions and running tools settle instead of waiting for the next health check) and then restarts the child (`managed-process-exited`). Intentional `close()`, pre-ready crashes, and deaths while `startOpenCode` is still in-flight (listen URL printed, `waitForReady` not yet returned) do not take this path: the start retry loop owns recovery so exit handling cannot spawn a second orphaned server (#3732).
- Returned API:
  - `startOpenCode()`
  - `restartOpenCode()`
...
- Shutdown stops guest services (extension child processes) first via the injected `stopAllGuestServices` dependency, best-effort: every exit path that reaches `gracefulShutdown` (daemon signal handlers, `POST /api/system/shutdown`, embedded `stop()`) stops them, so a graceful host quit leaves no guest running. A hard kill (SIGKILL, OOM, power loss) never reaches here and still leaves them, because the guest registry is memory-only with no boot reaper. The daemon-mode paths never reach `stop()`, which is why the call lives here and not only there.
