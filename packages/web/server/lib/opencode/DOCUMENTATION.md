...
- `upsertProviderConfig(providerId, config, workingDirectory, scope?, options?)`: Validates and writes a custom OpenAI Chat Completions, OpenAI Responses, or Anthropic Messages provider block (`npm`, `name`, `options.baseURL`, `models`, optional `env`/`headers`) into the user/project/custom config layer. The `npm` adapter is restricted to the three supported protocol packages and defaults to OpenAI-compatible Chat Completions when omitted for legacy callers. Model persistence supports `reasoning`, `attachment`, `tool_call`, `modalities.input/output`, `limit.context/input/output`, and `variants` in addition to `name`. Does not store API keys. Requires `config.env` or `options.hasStoredAuth` (auth already written via OpenCode `auth.set`). Edit flows must pass the provider's effective existing layer (`custom` > `project` > `user`) so updates do not create a global user override.
- `validateCustomProviderConfig(providerId, config, options?)`: Structural validation and allowlist normalization for custom provider payloads (id format, http(s) base URL, models, credentials via `env` or `hasStoredAuth`). Unknown model keys and invalid modality/limit values are dropped. Variant objects preserve recursively JSON-serializable values while dropping unsupported values and unsafe `__proto__`, `constructor`, and `prototype` variant IDs or nested keys. This contract must remain equivalent to the VS Code implementation in `packages/vscode/src/opencodeConfig.ts`.
...
- `registerOpenCodeRoutes(app, dependencies)`: Registers OpenCode-owned HTTP routes and internal module runtime:
  ...
  - `PUT /api/provider` (create/update custom OpenAI Chat Completions, OpenAI Responses, or Anthropic Messages provider config in OpenCode user/project/custom layers via `scope`; secrets stay in auth via the OpenCode auth API)
