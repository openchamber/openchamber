...
Background subagent aggregation: child session records (`session.updated` / `session.created` carrying `info.parentID`) teach the runtime the parent-child relation, and a child `session.status` `busy`/`retry` keeps the parent's activity phase `busy` even while the parent's own raw status is `idle`. Each child session tracks its last applied record recency (`time.updated ?? time.created`), and a stale complete record older than that baseline is skipped entirely — a delayed older update cannot overwrite a newer reparent/detach relation or re-adopt a busy child to a stale ancestor (the same authority rule the UI relation index applies); recency entries are timestamped and, when the session id is no longer referenced, retained by the hourly cleanup for the same deliberate 24-hour stale-event window as tombstones; they are cleared with the relation maps on deletion/reset/dispose. Deletion tombstones the deleted session id: a delayed same-recency record cannot reconnect a busy child (or the deleted session) to it — only an authoritative strictly-newer record may re-create the relation. `session.idle` / `session.error` terminal events are normalized to idle before lifecycle settlement, so a busy child settled by either cools its parent and drops the operational count. A complete `session.updated` record with `info.time.archived` is terminal like a deletion (matching the directory reducer): the shared deletion pathway clears the session's raw state, attention state, activity phase, and pending cooldown timer, orphans outbound relations, and tombstones the id with the archived record time as baseline. Terminal events are staleness-gated like records: an explicit `session.deleted` or a complete archived `session.updated` whose record recency is not strictly newer than the session's deletion tombstone baseline — or is not strictly newer than the newest accepted relation/session record recency (the session's own record or a child record referencing it as parent; an equal-recency terminal is stale too) — is ignored, so a delayed terminal after a newer child edge restored the relation cannot re-terminate it; a strictly-newer terminal still cleans up, and an unversioned terminal event is honored unless the session is already tombstoned. Unversioned `session.status` / `session.idle` / `session.error` updates for a tombstoned id are ignored (no raw/phase/count resurrection) until a strictly-newer complete record recreates the session. A newer child→parent edge may restore the edge relation (the child derives the parent again) but never clears the parent's tombstone/status block: the parent's own unversioned status stays blocked until the parent itself has a strictly-newer complete record (session recreation is split from edge admission — only the parent's own record moves the parent's relation recency above the deletion baseline). A relation that would form an indirect parent cycle (the proposed parent is already a descendant of the child) is rejected before any map mutation, so synthetic activity can never self-sustain after the only raw active session settles. Activity is transitive: a busy grandchild keeps the child and every ancestor presentation-active, and relation moves/null detach/deletion/stale eviction settle every former ancestor immediately. A complete record omits `parentID` exactly for root sessions, so an omitted parent detaches any prior relation the same way an explicit `parentID: null` does; partial payloads without record `info` never mutate relations, and an empty-string `parentID` is not a mutation. The parent enters cooldown only once every active descendant has settled and its own raw status is idle; `sessionStates` (and the `openchamber:session-status` events it produces) remain raw per-session truth. The derived phase is exposed through `getSessionActivitySnapshot()` / `openchamber:session-activity`; relation maps are pruned by the hourly cleanup and cleared on dispose/reset. The hourly cleanup also reclaims relation recency baselines and deletion/archive tombstones for session ids the runtime no longer references (no relation, raw state, attention state, or activity phase remains), and both are retained for the same deliberate 24-hour stale-event window after they were established (each accepted record restarts its baseline's window, like a fresh terminal restarts a tombstone): a tombstone blocks a stale reconnection from reanimating a deleted/archived session, and an unreferenced recency baseline keeps a root/detach ordering proof alive even with no active raw state so a delayed older child relation record cannot be admitted as fresh and reparent the session; after the window the id is forgotten like any id pruned by the state TTL and a later record is treated as a fresh session. Child-derived parent busy is marked `synthetic`: it is presentation/logical aggregation only and never increments `getActiveSessionCount()` (the operational count that selects the upstream SSE stall timeout) — only raw `busy`/`retry` sessions count, so one actual busy child contributes one active operational session, not two. A parent's own raw busy counts exactly once and is never demoted by a child activating. `session.deleted` is terminal: deleting a parent orphans its outbound child relations first (so the deleted session can never be re-derived from them), deleting a busy child clears the parent's aggregation and relation immediately (a child may never emit the idle event that would normally settle it), a reassigned relation settles the former parent before the child joins the new one, and the deleted session's raw state, attention state, activity phase, and pending cooldown timer are all removed immediately so every snapshot omits the deleted id, the operational count drops its raw-busy contribution, and no timer can recreate the session.
