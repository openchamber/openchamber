...
- `serverBrowserEnabled` and `serverBrowserDebugPort` are instance settings, so project and profile settings cannot override them. `serverBrowserEnabled` defaults to false. `serverBrowserDebugPort` stores `0` for automatic selection or an integer from `1` through `65535` for a fixed loopback CDP port. Settings responses normalize missing or invalid stored ports to `0`; updates with invalid ports return HTTP 400 before writing. Saving a port changes the next Chrome launch without stopping the active process. Disabling the browser stops Chrome before the settings response completes. Live status reports the configured and active ports separately. The process contract belongs to [the browser runtime](../browser/DOCUMENTATION.md).
