...
- `packages/web/server/lib/opencode/path-mapping.js`: host↔remote path translation for external OpenCode servers in another filesystem namespace (Docker). Parses `OPENCODE_PATH_MAP` and exposes the process-wide translator used by every OpenCode-bound directory hint. Resolution order: explicit test injection → active Docker instance mapping (`setActiveInstancePathMapping`, installed by the docker-instances lifecycle manager on switch) → `OPENCODE_PATH_MAP` env → identity. With no active Docker instance the env/identity behavior is unchanged.
- `packages/web/server/lib/docker/runtime.js`: Docker CLI runtime for OpenChamber-managed instance containers (availability probe, image inspect, create/start/stop/remove/inspect, label-filtered listing, explicit image build/pull, bounded log tail). Argument-array `docker` CLI calls with hard timeouts; no socket client and no new dependency.
- `packages/web/server/lib/docker/fake-runtime.js`: in-memory Docker runtime for tests with injectable failures and a call log (rollback assertions).
- `packages/web/server/lib/opencode/docker-instances/store.js`: persistent registry for Docker-backed instances (own `docker-instances.json` next to the other data files; corrupt-file recovery; active-instance pointer).
- `packages/web/server/lib/opencode/docker-instances/mounts.js`: pure mount-plan builder (workspace rw; shared skills dir rw; OpenCode config dir rw when config sharing is opted in — two-way by user-approved risk; credentials ro behind a separate default-off toggle) with fail-closed guards against forbidden host paths (docker socket, SSH, git credentials), covering the workspace and the user-suppliable skills override.
- `packages/web/server/lib/opencode/docker-instances/lifecycle-manager.js`: lifecycle state machine (`creating → starting → probing → running ⇄ stopped → removing → gone`, plus `error`/`removal-failed`), journal-driven rollback scoped to the failing operation, readiness probing on the published loopback port, and the active-upstream switch (network runtime override + per-instance path mapping).
- `packages/web/server/lib/opencode/docker-instances/routes.js`: `/api/docker-instances*` route registration (list/create/status/activate/deactivate/start/stop/remove/cleanup/image build/pull), gated by the `dockerInstancesEnabled` settings toggle (default off).
- `packages/web/server/lib/opencode/network-runtime.js`: OpenCode URL construction, health-probe readiness checks, and API prefix runtime; `buildOpenCodeUrl` consults the optional `resolveUpstreamOverride` first so every OpenCode-bound URL targets the active Docker instance when one is active.
...
- `getPathMapping()`: process-wide translator. Resolution: explicit injection → active Docker instance mapping → built lazily from `OPENCODE_PATH_MAP`. Identity when unset and no instance is active.
- `setActivePathMapping(mapping)`: test/composition injection that overrides the process-wide mapping.
- `setActiveInstancePathMapping(mapping)`: installs/clears the active Docker-backed instance's mapping; `null` falls back to the env/identity behavior.
