...
- `getProviderSources(providerId, workingDirectory)`: Resolves which OpenCode config layers define a provider and returns the winning layer's authored provider block (`custom` > `project` > `user`) alongside the per-layer existence flags, so edit read-back reflects exactly the keys the user wrote instead of catalog-resolved defaults.
