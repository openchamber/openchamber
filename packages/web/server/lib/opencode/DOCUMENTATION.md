...
- `packages/web/server/lib/codex-import/`: isolated Codex import core plus OpenChamber project/settings and HTTP adapters; see its owning `DOCUMENTATION.md`.
...
- Returned API:
  ...
  - `updateSettings(createChanges)`: serializes a read-modify-write update and invokes `createChanges` with the latest persisted settings while holding the settings update queue; returning `null` skips the disk write.
