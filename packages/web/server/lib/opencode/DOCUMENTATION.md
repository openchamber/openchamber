...
Managed health failures are classified as `timeout`, `connection_refused`, `connection_reset`, `invalid_response`, or `error`. The lifecycle retains the latest counted failure with a bounded detail string and source. Managed process wrappers continue capturing a sanitized, bounded stderr tail after readiness and retain exit code/signal. Before replacing a managed process, lifecycle snapshots the reason, latest health failure, process diagnostics/aliveness, busy-session count, and timestamp into `lastOpenCodeRestartDiagnostics`; successful startup does not clear this snapshot, and `/health` exposes it for post-restart diagnosis without process environment or credentials. A threshold restart additionally requires positive evidence that the listener is gone: the managed process must have exited or a TCP connect to `state.openCodePort` must fail. HTTP probe failures against a process whose port still accepts connections are retained as `lastOpenCodeHealthFailure` diagnostics and do not restart it.

Managed startup owns each spawned child from spawn to exit. The child is registered in the managed-process registry as soon as it is spawned (its port is already known), and every failed startup path — listening timeout, unparsable URL, early exit, spawn error, or post-startup readiness failure — terminates the child, awaits confirmed exit, and unregisters it before the error propagates. A process that is alive but has not printed its listening line is no longer abandoned at 30 s: lifecycle keeps probing the allocated port and adopts the server as soon as the port accepts a TCP connection, then continues through the same authenticated health-readiness check. The total wait is bounded by `OPENCHAMBER_OPENCODE_START_TIMEOUT_MS` (default 120000 ms); when the cap is reached the child is terminated. The normal fast path (listening line parsed immediately) is unchanged.
