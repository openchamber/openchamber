...
describe('static routes runtime', () => {
  it('returns API-only HTML fallback for browser UI routes', async () => {
    ...
    expect(response.text).toContain('Sharpie is running in headless mode');
    expect(response.text).toContain('Open it from the Sharpie desktop or mobile app');
  ...
  it('returns API-only info JSON for JSON clients', async () => {
    ...
    expect(response.body).toEqual({
      ok: true,
      mode: 'api-only',
      message: 'Sharpie is running in API-only mode',
    });
  });
  ...
  it('does not intercept API, auth, or health routes in API-only mode', async () => {
    ...
    expect(api.body).not.toEqual({ ok: true, mode: 'api-only', message: 'Sharpie is running in API-only mode' });
    expect(auth.body).not.toEqual({ ok: true, mode: 'api-only', message: 'Sharpie is running in API-only mode' });
    expect(health.body).not.toEqual({ ok: true, mode: 'api-only', message: 'Sharpie is running in API-only mode' });
  });
});
