...
describe('Zhipu AI Coding Plan quota provider — config apiKey resolution', () => {
  ...
  it('expands a {file:...} config apiKey before calling the quota API', async () => {
    const keyFile = '/private/zhipu-key';
    readConfigLayers.mockReturnValue({
      mergedConfig: {
        provider: { zhipuai: { options: { apiKey: `{file:${keyFile}}` } } },
      },
      paths: {},
    });
    vi.spyOn(fs, 'readFileSync').mockImplementation((filePath) => {
      if (filePath === keyFile) return 'sk-zhipu-file\n';
    ...
    expect(fetchMock.mock.calls.at(-1)[1].headers.Authorization).toBe('Bearer sk-zhipu-file');
  });
  ...
  it('prefers the config key over auth.json like OpenCode', async () => {
    ...
    expect(fetchMock.mock.calls.at(-1)[1].headers.Authorization).toBe('Bearer sk-zhipu-config');
  });

  it('does not fall back to auth.json when the configured variable is unset', async () => {
    readAuthFile.mockReturnValue({
      'zhipuai-coding-plan': { type: 'api', key: 'sk-zhipu-auth' },
    });
  ...
  it('does not use auth.json when the provider is disabled', async () => {
    readConfigLayers.mockReturnValue({
      mergedConfig: { disabled_providers: ['zhipuai-coding-plan'] },
      paths: {},
    });
    readAuthFile.mockReturnValue({
      'zhipuai-coding-plan': { type: 'api', key: 'sk-zhipu-auth' },
