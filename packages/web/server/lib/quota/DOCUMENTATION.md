...
| `command-code` | Command Code | `providers/command-code.js` | `command-code`, `commandcode`, `command_code`, or `command code` credential in OpenCode `auth.json`; `COMMAND_CODE_API_KEY`; or `~/.commandcode/auth.json` from `cmd login` |
...
Command Code usage is an optional, best-effort integration over unofficial `/alpha` endpoints. It resolves account scope through `GET /alpha/whoami`, then reads five-hour and weekly limits from `GET /alpha/billing/credits`. Organization accounts add `?orgId=...`. Personal accounts return `org: null` and use the plain credits path. Monthly usage shows as a percentage. Used credits come from `GET /alpha/usage/summary` and remaining monthly credits come from the billing response, and the provider divides used by their sum. The five-hour and weekly windows also show as percentages with no balance labels. Purchased and free credit balances never appear. When the summary request fails or has no usable numbers, monthly usage falls back to the remaining monthly credit balance so the row still shows something. Web/Electron and VS Code read matching OpenCode auth entries, including OAuth `access`, `COMMAND_CODE_API_KEY`, or the `apiKey` written to `~/.commandcode/auth.json` by `cmd login`. Credentials remain in the owning runtime and are never returned to shared UI. Endpoint failures are isolated to the Command Code result and do not block other providers.
