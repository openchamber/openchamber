...
- Zhipu AI Coding Plan (`open.bigmodel.cn`) applied the same `TOKENS_LIMIT` -> `CREDIT_LIMIT` rename on current GLM Coding Plans. Accept both types, surface the credit `valueLabel`, and map `data.level` to `planLabel` exactly like the Z.ai Coding Plan handling. Web and VS Code must stay in sync.
