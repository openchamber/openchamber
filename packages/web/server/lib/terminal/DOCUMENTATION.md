...
- After those deletions, the project's opted-in shell environment (`../projects/shell-env.js`) is overlaid for the session `cwd`, so the terminal and the Project Actions that run through it get the project's dev tools. Resolution is cached and bounded; a failure leaves the augmented-path environment in place and never blocks the spawn.
