...
import { isSafeEndpointUrl } from '../git-providers/config.js';
...
export function createGitLabClient({ token, baseUrl }) {
  ...
  if (!isSafeEndpointUrl(effectiveBaseUrl)) {
    throw new Error('GitLab endpoint rejected: must be HTTPS and not a private/loopback address');
  }
