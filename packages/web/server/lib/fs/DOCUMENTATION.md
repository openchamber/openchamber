...
- `POST /api/fs/clone` reserves the canonical destination through the shared Git execution coordinator. The reservation covers clone processing and partial-clone cleanup; clone children own a detached process group/tree, so cancellation, timeout, and output-limit failures await descendant termination before destination cleanup and scheduler release. Clone stdout and stderr each use the shared 4 MiB `maxBuffer` limit, and an exceeded limit returns an explicit error. Network capacity is released after the full clone termination lifecycle completes.
