...
describe('fs stat directory scope (issue 3019)', () => {
  ...
  it('returns exists:false for a file under the session directory when only lastDirectory resolves the workspace (optional stat)', async () => {
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body).toEqual({ path: '/repo-b/src/index.ts', exists: false });
  });
