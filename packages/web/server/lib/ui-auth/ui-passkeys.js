...
const DEFAULT_RP_NAME = 'ALIAS ADE';
...
const PASSKEY_STORE_FILE = path.join(ALIAS_ADE_DATA_DIR, 'ui-passkeys.json');
...
export const createUiPasskeys = ({
  ...
  const beginRegistration = async (req, { label } = {}) => {
    ...
    const options = await generateRegistrationOptions({
      ...
      rpID,
      userID,
      userName: 'alias-ade-ui',
      userDisplayName: 'ALIAS ADE UI',
  ...
  const finishAuthentication = async (payload) => {
    ...
    if (!passkey) {
      const error = new Error('That passkey is not registered for this ALIAS ADE instance');
      error.statusCode = 404;
      throw error;
    }
