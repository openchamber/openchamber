...
- JSON event IDs stay in event payloads. Only the upstream SSE `id:` field returns to OpenCode as `Last-Event-ID`. Hub-generated replay IDs belong to browser replay and never become upstream cursors. Durable `sync` markers stop at this boundary.
