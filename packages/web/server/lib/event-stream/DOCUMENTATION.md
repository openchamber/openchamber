...
- Global synthetic events such as `openchamber:session-status`, `openchamber:session-activity`, `openchamber:notification`, `openchamber:heartbeat`, `openchamber:scheduled-task-ran`, and `openchamber:session-created` are preserved on the WS path, but heartbeat frames are emitted only while an upstream SSE stream is actively attached.
- Global UI broadcasts are fan-out capable across both SSE and WS clients.
- `openchamber:scheduled-task-ran` and `openchamber:session-created` are emitted by `emitTaskRunEvent` and `emitSessionCreatedEvent` in `packages/web/server/index.js`, which fan out to both the legacy SSE clients (`uiOpenChamberEventClients`) and `broadcastGlobalUiEvent` (WS + notification SSE clients).
- The client-side event bus (`packages/ui/src/lib/openchamberEventBus.ts`) is the delivery contract for `openchamber:*` synthetic events. The event pipeline (`packages/ui/src/sync/event-pipeline.ts`) publishes to the bus when it receives `openchamber:*` events via the WS. Consumers (`openchamberEvents.ts`, `useWebNotificationStream.ts`) subscribe to the bus instead of opening per-tab EventSource connections.
- **WS-vs-SSE delivery guarantee**: the bus is only authoritative when the pipeline is on the WS transport. The SSE transport (`/api/global/event`) is a plain OpenCode proxy that does not carry `openchamber:*` synthetic events. When the pipeline falls back to SSE, `wsActive` is `false` and consumers keep their EventSource fallbacks open. Consumers must not treat `wsActive=true` as proof of synthetic-event delivery unless the pipeline is actually on WS.
