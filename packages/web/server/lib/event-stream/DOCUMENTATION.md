...
- The global hub also keeps a synchronous latest `busy`/`retry` status cache keyed by session ID. The cache is bounded by entry count and approximate bytes, copied status messages and retry-action strings are capped, and updates use oldest-first LRU eviction. It retains a concrete SSE envelope directory, uses `global` for unscoped direct status payloads, preserves a previously known concrete owner, removes entries only for recognized idle/error/deleted states, and clears entries and byte accounting when the upstream hub stops or is rebound. Fresh clients and clients whose replay cursor was evicted receive this cache without an unscoped `/session/status` request; a cursor still present in the replay buffer keeps the existing replay-only behavior.
