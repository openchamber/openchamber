...
- The shared hub preserves legacy event envelopes unchanged and translates the opencode2 events needed by server-side consumers into legacy names and properties. Unsupported opencode2 events retain shallow `{ id, type, data, location }` normalization; the UI does not use that fallback for session text/tool/message events and consumes adapter SSE so the adapter's stateful mapper can translate them completely. The opencode2 event `id` remains available for hub replay.
