...
const createService = (overrides = {}) => {
  const openCodeApi = {
    listSessions: vi.fn(async () => ({ sessions: [], cursor: undefined })),
    getSessionStatus: vi.fn(async () => ({ kind: 'authoritative', status: { type: 'idle' } })),
    listMessages: vi.fn(async () => ({ messages: [], cursor: undefined })),
    waitForSessionIdle: vi.fn(async () => ({ type: 'idle' })),
  };
  ...
  return { service, openCodeApi, sessionService, scheduledTaskService };
};
...
describe('OpenChamber control service', () => {
  ...
  it('resolves the target session directory from the global session list when send omits it', async () => {
    const { service, sessionService, openCodeApi } = createService();
    openCodeApi.listSessions.mockResolvedValue({
      sessions: [
  ...
  it('waits through the OpenCode API and returns the last completed assistant result', async () => {
    const { service, openCodeApi, sessionService } = createService({ now: () => 1000 });
    ...
    openCodeApi.listMessages.mockResolvedValue({
      messages: [{ info: { id: 'msg_new', role: 'assistant', time: { completed: 1500 } }, parts: [{ type: 'text', text: 'done' }] }],
    });
    ...
    expect(openCodeApi.waitForSessionIdle).toHaveBeenCalledWith('ses_1', '/repo', expect.objectContaining({
      requireActivity: true,
      baselineMessageID: 'msg_old',
      startedAt: 1000,
    }));
  });

  it('filters archived sessions and adds directory-scoped statuses', async () => {
    const { service, openCodeApi } = createService();
    openCodeApi.listSessions.mockResolvedValue({ sessions: [
      { id: 'ses_active', directory: '/repo', time: {} },
      { id: 'ses_archived', directory: '/repo', time: { archived: 100 } },
      { id: 'ses_other', directory: '/other', time: {} },
    ] });
    openCodeApi.getSessionStatus
      .mockResolvedValueOnce({ kind: 'authoritative', status: { type: 'busy' } })
      .mockResolvedValueOnce({ kind: 'unavailable', error: new Error('unavailable') });
  ...
  it('names limit in positive-integer validation errors', async () => {
    ...
    expect(openCodeApi.listSessions).not.toHaveBeenCalled();
  });

  it('projects only ordered text parts from session messages', async () => {
    const { service, openCodeApi } = createService();
    openCodeApi.listMessages.mockResolvedValue({ messages: [
      {
