...
vi.mock('../opencode/shared.js', () => ({
  ...
  resolveConfigValue: vi.fn((layers, key) => layers.mergedConfig?.[key]),
}));
...
describe('generateSmallModelText — config-only provider with {env:...} credential (issue #2269)', () => {
  ...
  beforeEach(() => {
    ...
    readConfigLayers.mockReturnValue({ mergedConfig: CONFIG_ONLY_PROVIDER, paths: {} });
  });
  ...
  it('lists config credentials from the requested project directory', () => {
    process.env.OPENCHAMBER_TEST_CUSTOM_KEY = 'sk-valid-key-here';

    expect(listAuthenticatedProviders('/proj')).toContain('custom-provider');
    expect(readConfigLayers).toHaveBeenCalledWith('/proj');
  });
  ...
  it('fails closed when OpenCode config cannot be read', async () => {
    readConfigLayers.mockImplementation(() => {
  ...
  it.each([
    ...
    ['enabled_providers', ['other-provider']],
  ])('does not call a provider excluded by %s', async (key, value) => {
    process.env.OPENCHAMBER_TEST_CUSTOM_KEY = 'sk-valid-key-here';
    readConfig.mockReturnValue({ ...CONFIG_ONLY_PROVIDER, [key]: value });
    readConfigLayers.mockReturnValue({
      mergedConfig: { ...CONFIG_ONLY_PROVIDER, [key]: value },
      paths: {},
    });
