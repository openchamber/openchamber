...
export function registerGitRoutes(app, { broadcastGlobalUiEvent, emitWorktreeChanged } = {}) {
  ...
  const resolveCommitHashQuery = (value, fieldName) => {
    const raw = readSingleQueryString(value);
    if (raw === null || !FULL_GIT_OBJECT_ID_PATTERN.test(raw.trim())) throw new Error(`${fieldName} must be a full commit SHA`);
    return raw.trim();
  };
  const resolveNullableCommitPathQuery = (value, fieldName) => {
    const raw = readSingleQueryString(value);
    if (raw === ROOT_QUERY_MARKER) return null;
    if (raw === null || raw.length === 0) throw new Error(`${fieldName} parameter is required`);
    return raw;
  };
  ...
  app.get('/api/git/commit-files', async (req, res) => {
    ...
    try {
      ...
      const legacyHash = readSingleQueryString(req.query.hash);
      if (req.query.hash !== undefined && !legacyHash) {
        return res.status(400).json({ error: 'hash parameter is required' });
      }

      const result = req.query.hash === undefined
        ? await getCommitFiles(directory, { commitHash: resolveCommitHashQuery(req.query.commitHash, 'commitHash'), parentHash: req.query.parentHash === ROOT_QUERY_MARKER ? null : resolveCommitHashQuery(req.query.parentHash, 'parentHash') })
        : await getCommitFiles(directory, legacyHash);
      res.json(result);
    } catch (error) {
      if (/must be a full commit SHA|parameter is required|is required for non-root commit/.test(String(error?.message || ''))) return res.status(400).json({ error: error.message });
  ...
  app.get('/api/git/commit-file-diff', async (req, res) => {
    ...
    try {
      ...
      if (req.query.hash !== undefined) {
        ...
        result = await getCommitFileDiff(directory, hash, filePath, req.query.binary === 'true');
      } else {
        const request = {
          commitHash: resolveCommitHashQuery(req.query.commitHash, 'commitHash'),
          parentHash: req.query.parentHash === ROOT_QUERY_MARKER ? null : resolveCommitHashQuery(req.query.parentHash, 'parentHash'),
        ...
        if (request.originalPath === null && request.modifiedPath === null) return res.status(400).json({ error: 'originalPath or modifiedPath parameter is required' });
        result = await getCommitFileDiff(directory, request);
      }
      res.json(result);
    } catch (error) {
      if (/must be a full commit SHA|parameter is required|is required for non-root commit/.test(String(error?.message || ''))) return res.status(400).json({ error: error.message });
