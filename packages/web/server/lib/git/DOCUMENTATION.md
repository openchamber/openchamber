...
- `stageFiles` remains supported for callers that need to stage a selected unstaged subset as part of commit. Selected `stageFiles` are staged normally, then the commit uses a temporary index containing only the selected `files` and `stageFiles`; unrelated real-index entries are never unstaged or restaged.
- When an explicit `stageFiles` selection or an oversized `files` selection has paths that need staging, the server stages them in bounded Git argv batches. The selected commit starts a temporary index from `HEAD` (or empty), writes only the selected current index entries into it, and removes selected staged deletions through a NUL-delimited literal pathspec before native `git commit`. This preserves selected staged-only/deleted/untracked content and exact unrelated partial, unmerged, intent-to-add, and newline-path index state. If a staging batch fails, the operation rejects with a completed-path count rather than reporting the full selection as successful.
