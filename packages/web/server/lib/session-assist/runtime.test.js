...
describe('session assist runtime with opencode2 events', () => {
  it('skips generation when normalized idle events arrive without metadata support', async () => {
    const supportsSessionMetadata = vi.fn(() => false);
    const getSmallModelService = vi.fn();
    const buildOpenCodeUrl = vi.fn();
    const assist = createSessionAssistRuntime({
      openCodeApi: { supportsSessionMetadata },
      buildOpenCodeUrl,
    ...
    try {
      hub.start();
      await waitForAssertion(() => expect(supportsSessionMetadata).toHaveBeenCalled());
...
async function fixture(generate = async () => output()) {
  ...
  const runtime = createSessionAssistRuntime({
    openCodeApi: { supportsSessionMetadata: () => true },
