...
Generation requires `openCodeApi.supportsSessionMetadata()`. The pinned V2
contract cannot persist session metadata, so it performs no assist reads or
generation. The supported legacy path retains SDK pagination and cancellation;
freshness checks also reject a protocol switch during generation.
