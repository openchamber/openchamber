...
- Usage: pace and prediction are back on quota rows — the work status panel and the Usage settings page again show whether usage is on track, too fast, or used up, plus the predicted final usage at the end of the window; a "Show predictions" toggle controls the extra rows (thanks to @JElfferich).
