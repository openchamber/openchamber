...
- **Guardian:** cross-platform (Linux/POSIX + Windows). The Linux/POSIX path uses a Unix-domain socket at `mode 0600` (same as before). The Windows path uses loopback TCP at `127.0.0.1:<ephemeral>` with an `icacls`-granted per-user ACL on the discovery file under `%LOCALAPPDATA%\openchamber\managed-opencode-handoff-v2\port`. Process termination on Windows uses `taskkill.exe /F /PID <pid>` (no `/T`).
- **Guardian/downgrade hint:** downgrading from a T2 build to a legacy Linux-only build leaves the discovery file as a harmless orphan; remove it with `rm "%LOCALAPPDATA%\openchamber\managed-opencode-handoff-v2\port"` if desired. No automatic migration is required.
