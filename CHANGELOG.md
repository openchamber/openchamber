...
- **Chat comments:** comments you leave on code now ride along as individual cards above the composer, each editable and removable on its own, instead of a single "Review comments" counter. They are sent as comment cards, so the same conversation shows them as cards in OpenCode Desktop. A comment added to a queued message stays with that message when the queue is sent.
- **VSCode:** code comments are now written in the editor itself. Select lines and pick Add Comment, or use the `+` in the gutter, and a comment box opens on those lines and stays anchored there until the message is sent.
